 ...
 *
 * **呼ぶのは1か所だけ。** PR詳細の「修正Issueを起案」（送り先がセッションなら「修正を依頼」）を
 * 押したとき（`PullRequestFixIssueBar`・#2961・#3009。帯の強さはPR本文の判定だけで決まるため、
 * 表示時には取らない）。Issue詳細のマージ待ち欄からも取っていたが、PRへの修正依頼をPR詳細へ
 * 一本化した（#3333）ため呼ばなくなった。1回で2リクエスト（PR本体＋会話コメント）
