...
export async function POST(request: NextRequest) {
  ...
  const agent = payload?.agent === undefined ? "claude" : parseDispatchAgent(payload.agent);
  const model =
    payload?.model === undefined || payload?.model === null
      ? null
      : typeof payload.model === "string" && payload.model.length > 0 && payload.model.length <= 100
        ? payload.model
        : undefined;
