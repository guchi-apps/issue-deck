...
export type PushNotificationPayload = {
   ...
   *
   * **新しい`public/sw.js`はこれを読まない**（#2196で、表示中かどうかによる抑止ごと
   * 無くなった）。それでも送り続けるのは、**サーバーの更新と端末のService Workerの
   * 更新が同時ではない**ため。古いsw.jsを持ったままの端末では抑止が生きており、
   * `force`の無いテスト通知は設定画面（＝押した時点で必ず表示中）で握りつぶされる。
   * 全端末が新しいsw.jsになったら消してよい。
   */
  force?: boolean;
};
