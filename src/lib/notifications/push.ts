 ...
 * アプリを開いているあいだのトーストは従来どおり画面側が出すので、Service Worker
 * （`public/sw.js`）は**表示中のウィンドウがあれば通知を出さない**。
 *
...
export type PushNotificationPayload = {
  ...
  tag: string;
  /**
   * 表示中のウィンドウがあっても、OSの通知として必ず出すか（#2195）。
   *
   * Service Workerは既定で**表示中なら通知を出さない**（画面内のトーストと二重になるため）。
   * ただしテスト通知は設定画面のボタンからしか送れず、**押した時点でその画面が必ず表示中**に
   * なるので、この判定に当たって毎回握りつぶされていた。届くかどうかを確かめるための通知が
   * 確かめられないのでは意味が無いため、抑止から抜ける口を1つだけ用意する。
   *
   * **`00.check-user`の通知には付けない。** 付けると、アプリを開いている間もトーストと
   * OSの通知が二重に出る。
   */
  force?: boolean;
};
