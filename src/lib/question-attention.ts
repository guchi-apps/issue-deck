 ...
 * レビュー中かどうかは開いた先の一覧の行のバッジ（#2855）と、Issue詳細の「レビュー中」表示で読む。
 *
 * **一覧の行にはバッジが出るようになった（#2855）が、ここは据え置く。** あちらは
 * 「コードレビュー」ビューを開いている間だけ専用の取得口でコメントを取っており、
 * 左メニューの行はその前段（一覧のデータだけ）に居る。合図のためにメニューから
 * 引きに行くと、レビューを見ていない間もGitHubを叩き続けることになる。
 *
