...
function artifactInstructions(labelNames: ReadonlySet<string>): string {
  if (labelNames.has(ARTIFACT_REQUIRED_LABEL)) {
    return [
      ...
      "- **計画のやり取りで見た目の直しを求められても、Plan modeの中では差し替えないでください**（#1745・#2110）。Plan modeで書けるのは計画ファイルだけで、アーティファクトのHTMLは書けません。順序は次の2つです。(1) 直す内容を計画本文へ書き、「公開済みの版は修正前の見た目です」と明記したうえで承認を取る (2) 承認の直後、コードを書く前に**最初と同じファイルパス**で`Artifact`を呼び直して再公開する。パスが同じなら同じURLへ再デプロイされるため、計画コメントに残ったリンクはそのまま新しい見た目を指します。**見た目の直しだけを再承認させるゲートは足しません**（承認の往復が増えるわりに、直す内容は計画に書いてあるので判断材料は増えません）",
