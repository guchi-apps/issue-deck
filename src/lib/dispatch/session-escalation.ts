...
export function buildSessionInterruptedCommentBody(params: {
  ...
  if (reason === "tool_call_stall") {
    lines.push(
      ...
      "「正常に応答した」ように見えます。pollerが下の固定文面を上限回数まで自動で送りましたが、",
      "復帰しませんでした（#2896。`scripts/lib/session-tool-call-stall.sh`）。",
      "",
      "**「進めて」のような曖昧な継続指示は避けてください。** モデルが直前の自分の発言を",
      "「既にツールを呼び出した」という事実だと誤認したまま、`ListAgents`で確認しても見つから",
      "ないのに「まだバックグラウンドで動いている」と誤答して再び止まることを確認しています。",
      "呼ばれていないことを事実として明言し、実際に呼び出し直すよう指示してください。",
      "",
      "```text",
      "直前の応答はツール呼び出し風のテキストを出力しただけで、実際にはツールは呼ばれていません。",
