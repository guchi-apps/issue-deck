 ...
 *
 * **pollerが自動で送るものと同じ1行**（`scripts/lib/session-tool-call-stall.sh`の
 * `SESSION_TOOL_CALL_STALL_BODY`。#2896）。片方だけ書き換えると、自動で送ったものと人が
 * 押して送ったもので文面が食い違い、どちらが効いたのかを後から追えなくなる。
 */
...
const STALL_NOTICES: Record<SessionInterruptedReason, SessionStallNotice> = {
  ...
  tool_call_stall: {
    ...
    detail: [
      ...
      "サブPCのpollerが同じ固定文面を上限回数まで自動で送りましたが、復帰しませんでした（#2896）。同じ1行をここから送り直せます。",
