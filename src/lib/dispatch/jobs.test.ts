...
/**
 * #2994。**一時停止は生きているセッションの判定より前で弾く。** 一括投入は先出し判定を通して
 * からオプションのラベルを書くため、ここで弾かないと一時停止中の一括投入でラベルだけが書かれてジョブが1件も積まれない。
 */
...
/**
 * #1311。**画面側（`resolveDispatchTargetRejection`）だけに置くと、画面を経由しない
 * 積み方が素通りする。** 判定をAPI側へ委ねている経路があるため、省かない。
 */
