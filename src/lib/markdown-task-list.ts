 ...
 *
 * **タスク行の判定はこの1つだけにする**（#1826）。手作業アシスタント
 * （`lib/manual-step-guide.ts`）も手順の切り出しにこれを使う。別の正規表現を書くと、
 * Issue詳細の「タスク 2 / 3 完了」とアシスタントの手順数が食い違う。
 */
export const TASK_LINE_PATTERN = /^(\s*(?:[-*+]|\d+[.)])\s+)\[([ xX])\](\s|$)/;
