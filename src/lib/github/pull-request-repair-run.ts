 ...
 *
 * 終了の報告はワークフローの最後のステップ（`if: always()`）が行うが、実行そのものが
 * キャンセルされた場合・runnerが落ちた場合には届かない。届かなかったときに
 * 「自動修正中」が画面へ残り続ける方が、少し早く消えるより害が大きいため時間で失効させる。
 * 60分はCI失敗修正の実行時間（実測で数分〜十数分）に対して十分な余裕を取った値。
 */
export const REPAIR_RUN_STALE_MINUTES = 60;
