 ...
 *
 * **効くのは初回のバックログだけ。** issue-deck1リポジトリで`/issues/comments`のLinkヘッダは
 * `page=79; rel="last"`を返す（＝約7,900件）。上限が無いとその1リポジトリで79リクエストを
 * 一息に使い、連携済みの全リポジトリぶんではインストール当たり5,000回/時の共有枠
 * （PR一覧・Issue同期・CI状態と同じ枠）を初回だけで大きく削る。
 *
 * 10ページなら1リポジトリ10リクエストで、リポジトリが26なら1巡あたり最大260リクエスト
 * （既定60分間隔で枠の5%程度）。issue-deckのバックログは8巡＝約8時間で読み切る。
 * 読み切った後は`since`で前回以降に更新されたコメントしか返らないため、1リポジトリ
 * 1リクエストに落ち着く。**自動削除は既定OFFなので、読み切るまでの間に何かが消えることはない。**
 */
