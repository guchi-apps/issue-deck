...
describe("IssueSessionStatus のセッション操作", () => {
  it("生きているセッションでは停止と終了を押せる", () => {
    render(<IssueSessionStatus session={makeSession()} dispatch={makeDispatch()} />);
  ...
  it("閉じるは確認してから積む", async () => {
    ...
    fireEvent.click(
      within(confirm).getByRole("button", { name: "セッションを閉じる" }),
    );
  ...
  it("終了済みのセッションでは停止を出さない", () => {
    render(
      <IssueSessionStatus session={makeSession({ state: "EXITED" })} dispatch={makeDispatch()} />,
    );
  ...
  it("消えたセッションには操作を出さない", () => {
    render(
      <IssueSessionStatus session={makeSession({ state: "GONE" })} dispatch={makeDispatch()} />,
    );
  ...
  it("未処理の操作があれば送信済みと出し、重ねて押させない", () => {
    ...
    render(
      <IssueSessionStatus session={makeSession()} dispatch={makeDispatch({ jobs: [job] })} />,
    );
