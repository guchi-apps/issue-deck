 ...
 *
 * そのため、**押す1回で何が流れるのかを押す前に並べる**（手作業アシスタントの承認パネルと
 * 同じ立場。#1869）。振り分けの判定は`buildManualStepSessionPlan`＝代行実行と同じ関数で、
 * 理由の文言も`describeManualStepExecutionRejection`から取る。
 *
 ...
 *
 * **畳まずに全部出す。** 承認の対象は「本文の手順」ではなく「これから実行される文字列」で、
 * 人に頼む手順もその理由とともに同じ並びへ出す——飛ばされないことが分かっていないと、
 * 自分が実行する手順を待たずにセッションが先へ進むと誤解する（承認パネル＝#1869と同じ立場）。
 */
