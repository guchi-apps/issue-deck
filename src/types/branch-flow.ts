...
export type UnreleasedUnits = {
  ...
  /**
   * `main..develop`のマージコミットのメッセージから読んだ、取り込んだブランチ名（#2661）。
   *
   * **「developにはあるがmainにはまだ無い」ことが確定しているブランチの集合。** レーンが
   * どのリリースで本番へ出たかは`resolveReleaseState`がPRのマージ時刻とリリースPRの
   * 作成時刻を比べて推測しているだけで、実際にそのコミットが`main`へ入ったかは見ていない
   * （リリース作業に時間がかかるほど、間に合わせで割り込んだ変更を誤って含めてしまう。
   * 実例: developへ先に入った#2657が、後から作られたv4.69.0のリリースPRに「含まれる」と
   * 誤判定された）。ここに載っているブランチ名は、この一覧と同じ1回の取得だけで分かる
   * 「実際にまだ本番へ出ていない」という裏付けなので、レーンの振り分けはこちらを優先する。
   */
  mergedHeadRefs: string[];
};
