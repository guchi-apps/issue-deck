...
-- GitHub ReleaseはDBに持たないため、リポジトリ＋タグ名＋行の識別子（対応するPRの参照）の組で指す。
CREATE TABLE `ReleaseCheckLine` (
    ...
    UNIQUE INDEX `ReleaseCheckLine_userId_repositoryId_tagName_lineKey_key`(`userId`, `repositoryId`, `tagName`, `lineKey`),
    PRIMARY KEY (`id`)
