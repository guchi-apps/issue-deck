...
6. 全アプリ共通の共有知識（`~/apps/_docs`）を必要な範囲だけ読む。
   ...
   **共有知識は読み取り専用。** 編集・コミットは行わない。知見を得た場合は、このリポジトリの
   `docs/`へ書き、あわせて対応Issueへ「知見メモ」コメント（`<!-- knowledge-candidate -->`）を
   投稿する。共有知識へ格上げするかどうかは判定しない
   （[docs/shared-knowledge.md](../../docs/shared-knowledge.md)「9. 共有知識更新フロー」参照）。
