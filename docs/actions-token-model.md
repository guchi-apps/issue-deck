...
なお、この問題に対して当時採った方針は**案C（issue #360「Claudeアプリの画面が開くボタンを追加」）**
だが、**そのボタンは#1769で削除済み**（質問する・サブPCで実行するで代替）。本ドキュメントは、
その方針を採らなかった場合・および案C ではカバーできない範囲が残った場合に備えた判断材料として残す。
...
遷移し、プロンプトを入力した状態で開始できるようにするもの。**このボタン自体は#1769で削除した**
（質問する・サブPCで実行するで代替）が、「人間がループに入れるセッションで調査する」という
案Cの考え方は、サブPCのローカルセッションを起点として現在も同じように成り立つ。
