...
明細の「内訳」列（入力 / 出力）は、**セッションの金額をトークン数の比で按分してはいけない**
（#2626）。キャッシュ書き込みは入力単価の1.25〜2.0倍、キャッシュ読み出しは0.1倍で、
Claude Codeのセッションはトークンの大半がキャッシュ読み出しになる。トークン比で割ると
入力側が実際より大きく、出力側は桁で小さく出る（opus-5で 入力20k / キャッシュ書き込み(1h)1M /
キャッシュ読み出し20M / 出力200k のセッションだと、実際の 入力$20.1・出力$5.0 が
入力$24.86・出力$0.24 になった）。

**単価表を持っているのは`scripts/lib/session-usage.sh`だけ**なので、そこで金額を割った時点で
`inputCostUsd` / `outputCostUsd`として一緒に出し、報告本文 → `SessionUsage`の列 →
`SessionUsageEntry` → 画面へそのまま運ぶ。`src/lib/session-usage-view.ts`は
**単価を持たず、割り直しもしない**（ファイル冒頭の方針どおり）。

- **内訳を持たない行だけ、トークン比の近似へ落として「約」と断る**（`sessionUsageCostSplit`）。
  該当するのは列を足す前に報告されたローカルセッションと、**GitHub Actionsの行**
- **Actionsの行は今後も近似のまま。** 報告元（`.github/scripts/summarize-claude-usage.sh`）が
  読むのはClaude Codeが出す`total_cost_usd`で、入力側・出力側の内訳が無い。トークン数と
  合計金額だけからは単価が復元できない（未知数2つに対して式が1つ）ため、内訳を出すには
  Actions側にも単価表を置くことになる。単価表を2か所に持つほうが害が大きいと判断した
