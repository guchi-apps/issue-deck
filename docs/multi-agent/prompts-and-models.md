...
- `AI_GATEWAY_API_KEY`がある環境では、まずJev（Vercel AI GatewayのTypeSafe互換API）へChoice質問を
  送り、Sonnet / Opus / Fableを分類する。**確信度が0.60未満・API失敗・応答不正なら採用しない**。
  Jevは文章を生成しない分類モデルなので、実装エージェントとしては使わない
- Jevを採用できなかったときはアプリ内AI（既定はHaiku。`lib/claude/model-pick.ts`）へ渡し、さらに
  **呼べなかった・応答を読めなかったときはラベルと分量からのルールへ倒す**（`pickModelByRule`）。
  ルールでは最上位（Fable）を選ばない——AIが落ちている間ずっと重いモデルで走ることになるため
