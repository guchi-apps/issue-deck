...
言い回しは汎用版より短い。** 揃える対象は規則の中身で、文面の一致ではない。汎用版も#3023で計画の節を`21.plan-required`区間へ入れたが、文面は従来の詳しい書式のまま。
...
| 文脈が伸びた状態で粘らない | 見通しが立たないまま往復が積み上がったらIssueが大きすぎるサイン。区切れる範囲をPRにし、残りは`70.needs-decision`付きの新規Issueへ切り出す |
...
汎用版も#3023で同じ形にし、実測値は[generic-implementation-agent-reference.md](../../scripts/prompts/generic-implementation-agent-reference.md)へ移している。
揃える対象は規則の中身で、文面の一致ではない。
...
- 見た目のアーティファクトの節は、汎用ランチャー・画面の「実装プロンプトをコピー」と同じ文面を
  保つ規約があるため縮めていない
- 汎用ランチャー（`generic-implementation-agent.md`）は#3023で、無人実行（`.github/prompts/implement.md`）は#3024で、
  同じ整理をした（次の2節）
...
（#3245でこの設定はIssue作成のラベル付与にも効くようになった。末尾の「Issue作成のラベル付与にも同じ設定が効く」を参照）
...
#### Issue作成のラベル付与にも同じ設定が効く（#3245）

Issue作成画面の「タイトル自動」（`POST /api/issues/suggest`）が付けるラベルの判定は、
`modelPickEngine`が`jev`のときJevが行う。**タイトルと種別（issue／question）はアプリ内AIのまま**
（Jevは文章を書けず、Issue本文どおりタイトル付与はJevへ移していない）。設定の列は共用で、
UIの名称だけ「判定に使うAI（おまかせ・ラベル付与）」にした（別カラムにするとマイグレーションが要るため）。

- **ラベルは複数付けられるので、1ラベルにつき1つの`noul`（付ける確率）で聞く。** しきい値は
  0.5（`JEV_LABEL_THRESHOLD`）。**どれも届かないときは確率が最大の1つを付ける**（AI経路の
  「種別を必ず1つ」を保つため）
- **優先度（`80.`〜`89.`）だけは`choice`。** 同時に2つ付くと意味が食い違うため。選択肢に
  「付けない」を入れ、本文に緊急性が書かれていなければそれが選ばれる。質問のキーは
  `label_<連番>`と`priority`で、ラベル名（`.`・`:`・空白を含む）をnoulのキーにしない
- 対象のラベルはAI経路と同じ`isAutoAssignableLabelName`の集合（30〜89番台。71・41・72〜75を除く）
- Jevが使えない（キー未設定・失敗・答えが読めない・対象ラベルなし）ときは、AIのプロンプトに
  ラベルの節を戻して従来どおりAIに選ばせる。Jevが判定したときはプロンプトからラベルの節を外す
- Issue編集ダイアログの「付け直す」はタイトルしか使わないので`labels: []`で呼ぶ（Jevを呼ばない）
- 消費量は`issue_suggest`として計上する（`askSystemOne`の`feature`）
...
トークン数は、どちらのプロバイダーも同じ機能別集計として記録する（**画面での表示は#3062で
削除した**。計上だけが残っている）。API換算の目安金額の単価は
