...
| 本文（`message`） | `.github/release-notes.md`の中身（空ならフォールバック文言。#2683） | 付けない |
...
- **本文はバージョンが一致したときだけ載せる。** `.github/release-notes.md`の先頭の見出し
  `# v<バージョン>`と`NOTIFY_VERSION`を突き合わせ、違えば載せない。リリースの流れの外で
  ファイルが取り残されたとき、**古い文面を新しいリリースの通知へ貼るほうが、本文が無いことより
  悪い**ため。上限1500文字で切る（1件で通知一覧が埋まらないように）
- **本文は空欄のまま送らない**（#2683）。見出しの不一致・ファイル不在・中身が空など、理由を
  問わず本文が空になった場合は「今回のリリースでは、表示できる更新内容がありません。」と
  一律のフォールバック文言を載せる。受け取った側が「更新内容が本当に無かった」のか「通知が
  壊れている」のか区別できるようにするためで、理由ごとの出し分けはしない
