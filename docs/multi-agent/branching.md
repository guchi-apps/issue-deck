...
- 開発サーバー（`pnpm dev`）のポートは`start-issue.sh`/`.ps1`が`.env.local`へ自動設定する（issue-deckは`4000 + Issue番号`。例: issue-46 → 4046。Issue番号が帯の幅（2000）を超えるぶんは帯の中で折り返す。#2478）。複数Issueのworktreeで同時に`pnpm dev`を起動しても衝突せず、developへマージする前に人間がブラウザ（`http://localhost:<ポート>`）で直接画面を確認できる。実装エージェントは画面に関わる変更のPRで、このURLを「確認方法」に記載する。
...
- `start-issue.sh`はworktree準備時に`scripts/setup-lan-access.sh`を呼び、Windows側のポートフォワーディング（`netsh interface portproxy`）とファイアウォール許可を自動設定したうえで、`http://<WSL IP>.sslip.io:<ポート>`をあわせて提示する（同一LAN上のスマホ等、`localhost`が使えない別端末からの確認用。詳細はsslip-io-lan-devスキル参照）。WSLのIPはWSL再起動のたびに変わるため、`scripts/dev.sh`経由の通常起動時も含め、devサーバー起動のたびに再設定する。Windowsの管理者権限が必要なためUACダイアログが表示される。ただしワンクリック起動（`start-local-session.sh`）から始まったセッションでは、UAC待ちから戻らずタブが固まる／devサーバーが起動しないため、`ISSUE_DECK_SKIP_LAN_SETUP=1`により`start-issue.sh`・`dev.sh`の双方でスキップする（#1076・#1094。詳細は[local-quick-start.md](local-quick-start.md)）。`next.config.mjs`の`allowedDevOrigins`は個別IPではなく`*.sslip.io`（ワイルドカード）を許可しており、WSLのIPが変わってもコード変更不要。
- worktreeは`scripts/cleanup-worktrees.sh`で掃除する（#1100）。**サブPCではpollerが1時間ごとに`--all-repos --yes`付きで呼ぶ**（#1716・#2123。下記「掃除を回す起点」）。「未コミットの変更が無い」「マージ先のブランチ（実在する`origin/develop`・`origin/main`のうちどれか）に入っていないコミットが無い」「そのIssueのセッション・開発サーバーが動いていない」「ブランチ`issue-<番号>`を開いていて、gitの作業ツリーとして壊れていない」「実行中のworktreeでない」を**すべて**満たすものだけを対象とし、削除対象を一覧表示して確認を取ってから、worktree・ローカルブランチ・そのIssue用の生成物（起動用プロンプト、devサーバーのログ・PIDファイル）を消す。`--dry-run`で判定だけ、`--yes`で確認省略、`--issue <番号>`で対象を1件に絞れる、`--repo <owner/repo>`で対象リポジトリを選べる、`--all-repos`で対応表の全リポジトリを回す、`--size`でディスク使用量も測る。非対話実行で`--yes`が無い場合は表示のみで終了する。リモートブランチには触れない。**worktreeを消す直前に、そのIssueのポートを掴んでいる開発サーバーを止める**（#1524）。上の判定はPIDファイルと`run-issue-session.sh`のプロセスしか見ないため、エージェントが手で起こし直した`pnpm dev`はすり抜け、消えたworktreeを指したまま走り続ける。`--recreate`でworktreeを作り直す`start-issue.sh`も同じ停止を行う。
- **「PRがマージ済みか」は削除の判定に使わない**（#1192）。消して失われるものが無いことは「未コミットの変更が無い」「マージ先に入っていないコミットが無い」の2つで決まり、PRの有無はそこへ何も足さない（マージ先に入っていないコミットが1つでもあれば必ず残る）。逆にPRを条件にすると、**PRが最初から作られないworktreeが永久に消せなくなる**。#1177では起動確認だけして実作業を別リポジトリで行った結果、コミット0件・push無し・PR無しのworktreeが「マージ済みPRが無い（作業中）」として保護され続け、手で`git worktree remove --force`を打つことになった（他に「セッションが途中で落ちた」「Issueが取り下げられた」も同じ形になり、無人実行では失敗ジョブのぶんが誰にも気づかれずに溜まる）。マージ済みPRの番号は削除理由の表示にだけ使うので、`gh`が不通・未認証でも掃除は進む。
...
- **`node_modules`の実消費はパッケージマネージャで桁違いに違う**（#2124）。pnpm・bunは
  `~/.local/share/pnpm/store`のようなグローバルストアからハードリンクを張るのでworktreeを
  増やしても実データが増えない（実測: dayspan 31本で2,830MB＝1本あたり91MB）が、
  **npm・yarnにはこの共有が無く、worktreeごとに実体をコピーする**（実測: ops-dashboard 16本で
  8,566MB＝1本あたり571MB。`node_modules/next/package.json`のリンク数は1）。issue-deck自身は
  pnpmなので気づきにくいが、**汎用ランチャーで起こす他リポジトリの11本はnpm**で、サブPCでは
  `~/apps`配下の`node_modules`だけで51.3GB（ルートFS 98GBの過半）に達した。
  - 対策は2つ。(1)`generic-start-issue.sh`が**installの前に本体の`node_modules`を`cp -al`で
    敷く**（`scripts/lib/node-modules-share.sh`の`seed_node_modules_from_main`。実測で1本あたり
    941MB→12MB、installも数十秒→数秒）。(2)既にコピーで増えたぶんは
    `scripts/dedupe-node-modules.sh`が`hardlink`(util-linux)でまとめ直す（pollerが既定1日1回、
    ポーリングを止めない別プロセスで呼ぶ。実測の回収見込みは合計24.8GiB）
  - **ハードリンク先へその場書き込みをするツールがあると、本体と他worktreeまで壊れる。**
    実測で`prisma generate`は`node_modules/.prisma/client/`の`schema.prisma`・
    `query_engine_bg.wasm`・`query_engine_bg.js`を**inodeを保ったまま**書き換えた（同じ
    ディレクトリの`index.js`等はunlink+再作成でリンクが切れる）。スキーマを変えたブランチで
    generateすると本体の生成物がそのブランチの内容に化け、しかも`node_modules`は
    `.gitignore`対象なのでgitのどこにも差分として出ない。そのため`.prisma`・`.cache`・`.vite`は
    共有の対象から外している（`NODE_MODULES_SHARE_EXCLUDES`）。**同じ形のディレクトリを増やす
    ツールを入れたら、ここへ足す**
...
**起点があっても、範囲が1リポジトリなら残りは溜まり続ける。** #1716で起点を置いたあとも
`cleanup-worktrees.sh`が見ていたのは`~/apps/issue-deck-worktrees/issue-*`だけで、汎用ランチャー
（#1224）で起こした他リポジトリのworktree（`~/apps/<repo>-worktrees/issue-*`）には掃除の起点が
どこにも無かった。他リポジトリ側に同等のスクリプトを持つリポジトリは1つも無く、**166本中153本が
他リポジトリのまま溜まって**ルートFSが91%（残り9.1GB）に達した。#1716で足りなかったのが起点なら、
ここで足りなかったのは範囲。

- pollerは`--all-repos --yes`で呼ぶ。対象は`~/.config/issue-deck/local-repos.conf`に載っていて、
  チェックアウトと`~/apps/<repo>-worktrees`の両方が実在するリポジトリ
- `--all-repos`は**自分自身を`--repo`付きで順に呼ぶ**。判定のループを二重に持たないためで、
  **1リポジトリの失敗（fetchできないclone・`gh`の未認証）で残りを止めない**
- リポジトリごとに変わるのは4つだけ。チェックアウト先（`local-repos.conf`）・worktreeの置き場
  （`~/apps/<repo>-worktrees`という汎用ランチャーと同じ規約）・マージ先のブランチ
  （実在する`origin/develop`・`origin/main`）・開発サーバーのポート帯（`local-repo-ports.conf`）。
  判定そのもの（未コミット・未push・稼働中・経過時間）はリポジトリによらず同じ
- **マージ先は1つに決め打ちできない。** `develop`を持たないリポジトリ（`guchi-apps/docs`・
  `claude-config`）は`main`へ直接マージし、`subpc`・`vps`はGitHubの既定ブランチが`main`である
  一方でPRの宛先は`develop`になる。実在するrefを全部拾い、**どれか1つにでも入っていれば
  「消しても失われない」**とする（`worktree_base_refs`・`worktree_commits_not_in_base`）
- 全19リポジトリ・166本の`--dry-run`が実測42秒（`timeout`は5分）。**リポジトリを増やしたら
  この上限を見直す**（1リポジトリあたり`git fetch`1回と`gh pr list`1回が増える）
...
## 「修正Issueを起案」が対象とする指摘が、まだdevelopに無いコードのことがある（#3005）

「修正Issueを起案」（`buildPullRequestFixIssueDraft`・[code-map.md](../code-map.md)）は、
**マージ済みだけでなく未マージのPRも対象にできる**（本文の`（develop ← issue-2995・未マージ）`が
その印）。この場合、指摘のコードは対象PRのブランチにしかなく、起票された修正Issueは新しい
worktree（develop起点）で開くため、**修正すべき行がそのブランチにまだ存在しない**ことがある。

- **develop起点の自分のブランチへ実装しても意味が無い。** 対象ファイルが無い・機能そのものが
  無いため、修正できないか、develop向けに未マージPRの全内容を重複させたPRになる
- **同じホストに対象PRのセッションが生きていれば、そちらを使う。** 「修正を依頼する」
  （#2919・[gates.md](gates.md)「マージ待ちの修正依頼をローカルセッションへ知らせる」）で
  既存セッションへ知らせ、このIssueでは実装しない
- **生きているセッションが無ければ、対象PRのブランチへ直接pushしてよい。** 対象ブランチを
  ベースにした一時worktree（`git worktree add <一時パス> -b <一時ブランチ名>
  origin/<対象ブランチ>`）を別途作り、そこで直して`git push origin
  <一時ブランチ名>:<対象ブランチ>`する（fast-forwardであることを確認してから）。既存の
  worktree（`issue-deck-worktrees/issue-<番号>`）が残っていてもそこは触らない——別の一時worktreeを
  使えば「他Issueのworktreeの編集」には当たらない。**このIssue自身はdevelop向けPRを持たない**ため、
  完了報告は対象PRへのコメント＋このIssueへのコメント（対象PRのコミットへのリンク）で行う
- この判断は個別ケースの解釈に依存するため、着手前に一度確認を取るのが安全（#3005でも実施前に
  ユーザーへ確認した）
