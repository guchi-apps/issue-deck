...
残ると、新しい実装を起こせなくなる。止め方は4段構えになっている。
...
| 4 | 2・3の在庫に載っていない分を`/proc`の走査で見つけて止める | 同上（#1525） |

4は2・3の重複ではなく**在庫の取り方が違う**。2・3が見るのは`.dev-servers/issue-<番号>.pid`だけで、
**PIDファイルに載らない開発サーバーは存在自体が見えない**。4は`/proc`を走査して、動いている
プロセスそのものから入る。

**周期を足したのではなく在庫を足した。** 呼ぶのは4もpollerの1巡で、systemd timerは新設して
