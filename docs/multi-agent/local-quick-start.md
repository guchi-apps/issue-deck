...
| 5 | 1〜4が届かなかった分を定期的に掃く | `scripts/sweep-orphan-dev-servers.sh`（systemd timerが1時間ごとに呼ぶ・#1525） |

5は1〜4の重複ではなく**入口が違う**。1・2の入口は「ポートを手掛かりに引く」（`dev_server_stop_by_port`・
#1524）で、対象は「セッション終了時」「worktree削除時」というイベント発生時に限られる。3・4の入口は
`.dev-servers/issue-<番号>.pid`だけなので、PIDファイルが残らなかった孤児と、pollerそのものが
止まっている場合を丸ごと取りこぼす。5は**実際にポートを掴んでいるプロセス**から入り、pollerとは
独立した経路で走る。
...
PIDファイルに依らない回収は次の定期掃除が担う（#1525）。

### 定期掃除（`scripts/sweep-orphan-dev-servers.sh`・#1525）

上の回収が届かない孤児を、**PIDファイルに依らず**掃く段。2026-08-15にサブPCで孤児9本が
約5〜6GBを占有しOOMに至った（#1523）ことを受けて足した多層防御で、次のどれにも当たる。

- PIDファイルが書かれる前・消えた後にプロセスだけが残った（SIGKILL・ホストの強制再起動）
- worktreeごと消えたのに、開いていた開発サーバーだけが残った
- **pollerが止まっている**（上の回収はpollerの1巡に相乗りしているため一緒に止まる）

入口は`ss -tlnp`で、**実際にポートを掴んでいるプロセス**を1件ずつ次のように判定する。
1つでも欠ければ**止めない**。

| 材料 | 条件 |
| --- | --- |
| `/proc/<pid>/cwd` | `<repo>-worktrees/issue-<番号>`である（ここからリポジトリ名とIssue番号を決める） |
| プロセスグループ | リーダーが同じworktreeに居る（`scripts/lib/dev-server.sh`と共有） |
| リーダーのcmdline | `pnpm dev`等の開発サーバーの起動コマンドである |
| tmuxセッション | `<repo>-issue-<番号>`が**存在しない** |
| 起動からの経過 | `DEV_SERVER_ORPHAN_GRACE_MINUTES`（既定30分）以上 |

止めた後は、掴んでいたポートの`tailscale serve`も外す（残すと`EADDRINUSE`でそのポートが使えなく
なる。#1403）。記録は`.dev-servers/issue-<番号>.log`と`logger`（syslog）の両方に残す。
...
- **ポート番号からIssue番号を逆算しない。** 実際のポートは「リポジトリごとのベース値 +
  ...
  読める）。`cwd`は対応表を増やさずにリポジトリ名とIssue番号の両方を確定できる。
- **tmuxのセッション一覧は1度だけ取り、取れなければ何もしない。** `has-session`を1件ずつ叩くと、
  tmuxサーバーが一時的に応答しない間の失敗が「セッションが無い（＝孤児）」に見え、生きている
  セッションの開発サーバーを軒並み撃つ。「サーバーが動いていない」だけはセッションが1つも無い
  ことの確定した表明なので、その場合に限り空の一覧として扱う。
- **猶予（既定30分）が唯一の誤爆の防波堤。** 開発サーバーとtmuxセッションのどちらが先に立つかは
  起動経路によって前後し、`next dev`がポートをbindするまでにも数秒〜十数秒かかる。
...
- **worktreeごと消えている場合も拾う**（`readlink /proc/<pid>/cwd`が付ける` (deleted)`の印を
  落として比較する。判定は`scripts/lib/dev-server.sh`と共有）。worktreeを消しても`pnpm dev`は
  ポートとメモリを掴んだまま動き続けるため、孤児のうちでも最も回収したい形になる。
- 判定材料はセッションの有無だけなので、**tmuxセッションの外で手で起こした開発サーバーも、
  対応するセッションが無ければ猶予の後に掃かれる**。止めた理由と起こし方はログの末尾に残る。

手元で確かめるときは`scripts/sweep-orphan-dev-servers.sh --dry-run`。実行の記録は
`journalctl -t issue-deck-dev-server-sweep`。timerの導入手順は
[サブPCへのディスパッチ](subpc-dispatch.md#サブpc側のセットアップ)を参照。
