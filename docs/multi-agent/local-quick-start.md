...
**Athlon 200GE（2C/4T）だった頃**のサブPC実機での実測は**上限3本**（メモリ58%・swap 0）。
メモリより先にCPUが律速し、3本並行では`next build`が35.7秒→88秒（2.5倍）に伸びたため、
**常用は2本まで**が妥当だった。

> **この数字は載せ替え前のもの**（#1791）。サブPCのCPUはRyzen 5 PRO 4650G（6C/12T）へ載せ替え済みで、
> CPU律速という前提は成立しない。代わりに効くのは**メモリ（13Gi・据え置き）**で、旧実測の
> 同時ビルド時1本あたり約2.63GiBから単純計算すると4本で10.5GiB前後になるが、スレッド数が増えたぶん
> 1本あたりのピークも増えうる。測り直し（guchi-apps/subpc#19）が済むまでは、上の「常用2本」を
> **安全側の目安**として使う（3本以上が載らないという意味ではない）。
...
サブPCは**常用2本・上限3本**（前述の載せ替え前の目安）で、`pnpm dev`一式は使用中で0.45〜1.13GiB、アイドルでも
