...
| # | 約束 | 版 | 機械検査 |
| --- | --- | --- | --- |
| 1 | `bash scripts/start-issue.sh <番号>`の1引数で、**そのターミナルのフォアグラウンドで**セッションを開始する（Windows Terminalの新しいタブを開かない） | v1 | できない |
...
| 6 | Windows Terminalが無い環境では、**tmuxの新しいセッション**を出口にする（セッション名は`<リポジトリ名>-issue-<番号>`） | v2 | 部分的（`tmux`の使用有無まで） |
...
python3によるプロンプト生成ブロック全体・`wt.exe`でのタブ起動・tmux出口（出口の判定・
セッション名の組み立て・`bash -lc`・`remain-on-exit`）・`claude --permission-mode acceptEdits`
の起動フラグ。
...
では、この出口だけが使えない。**そこをtmuxの新しいセッションに置き換える**（#1178）。
...
```bash
scripts/start-issue.sh 1178             # 実行環境から自動で選ぶ
scripts/start-issue.sh --tmux 1178      # 必ずtmuxセッション
scripts/start-issue.sh --no-tmux 1178   # 必ず従来の出口
```

| 実行環境 | 単一Issue | 複数Issue |
| --- | --- | --- |
| Windows Terminalがある（メインPCのWSL） | このターミナルのフォアグラウンド（従来どおり） | 新しいタブ（従来どおり） |
| tmuxがある（サブPC・SSH越し） | 新しいtmuxセッション → そのままアタッチ | Issueごとに新しいtmuxセッション（アタッチはしない） |
| どちらも無い | このターミナルのフォアグラウンド | 手動実行コマンドの案内のみ |

**メインPC（WSL）の挙動は変えていない。** `wt.exe`と`WSL_DISTRO_NAME`が揃っていれば従来の経路を
そのまま通る。分岐を足しただけで、既存の経路のコードには手を入れていない。
...
`<リポジトリ名>-issue-<番号>`（例: `issue-deck-issue-1178`）。タブ名（`<リポジトリ名> #<番号>`）と
同じ内容だが、tmuxで使える文字だけに直している。`.`・`:`は`session:window.pane`の区切りとして
解釈されるためセッション名に使えず、空白と`#`は指定のたびにクォートが要る。

**リポジトリ名を含めるのが要点。** サブPCはissue-deck専用機ではなく、他リポジトリの作業セッションも
同じ`tmux ls`に並ぶ。ops-dashboard側でtmuxセッション一覧を出す予定（guchi-apps/ops-dashboard#34）が
あり、そこで見分けがつく必要がある。
...
セッションではなく前回の異常終了の痕跡なので、最後の出力を15行だけ表示してから畳み、作り直す。
...
| 全ペインが死んでいる（前回の異常終了） | 最後の出力を表示 → 畳んで作り直す |
...
| リポジトリ | ベース値 | 例 |
| --- | --- | --- |
| guchi-apps/issue-deck | 4000 | Issue #1178 → 5178 |
| guchi-apps/shopping-list | 5000 | |
| guchi-apps/dayspan | 6000 | |

この表を持つのは受け口（`scripts/start-local-session.sh`）。どのリポジトリがどの帯を使うかは
**定義上どのリポジトリ単独では決められない**ため、全リポジトリを知る唯一の場所に置いている（#1073）。

**あわせて、各リポジトリの`start-issue.sh`側の既定値も同じ値に揃える**（約束7。#1178）。
...
Issue番号は単調増加するので、同じリポジトリ内でポートが衝突することはない。帯の幅（1000）を
...
| 段階 | 設定するもの | 実装 |
| --- | --- | --- |
| タブ（tmuxセッション）を開く | Windows Terminalのタブ名／tmuxのセッション名 | `--title "<repo> #<番号>"`（`issuedeck-protocol.ps1`・`start-issue.ps1`・`start-issue.sh`）／`<repo>-issue-<番号>`（`tmux_session_name`） |
