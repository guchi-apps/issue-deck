...
開発サーバーのポートは`ベース値 + Issue番号`を**帯の中で折り返した値**（#2478）。ベース値と帯の幅は
リポジトリごとに固定する。
...
`issue-deck`は#1224の時点で4000帯（1000ぶん）に収まらなくなっていたため、4000〜5999の2000ぶんを
占める。対応表の3列目（帯の幅・#2478）がその宣言で、省略した行は原則の1000になる。5000帯にいた
`shopping-list`は、この重なりを解消するため7000へ移した（#1224）。

受け口は対応表から引いたベース値を`ISSUE_DECK_DEV_PORT_BASE`で、帯の幅を`ISSUE_DECK_DEV_PORT_WIDTH`で
渡す。幅は3列目がある行だけ渡り、渡らなければ原則の1000に落ちる（契約の約束4はベース値のままで、
版数は上げていない。幅を解釈しないリポジトリでも原則の幅で動く）。
...
### 帯の幅を超えたIssue番号は帯の中で折り返す（#2478）

**Issue番号は単調増加するので、素朴に足すと必ずいつか帯の幅を超え、隣のリポジトリの帯へ食い込む。**
実際にissue-deckの`4000 + 2470 = 6470`が`dayspan`の6000帯へ入り、`dayspan #470`と同じポートに
なっていた。両方のセッションを同時に起こすと後から起こしたほうが`EADDRINUSE`になる。以前は
「帯の幅を超えたら帯を割り直す」運用だったが、**超えたことを検知する仕組みが無かった**ため気付かれ
ないまま食い込んでいた。
...
- **`- 1`が入るのは`ベース値 + 0`を確認環境が使うから**（後述の「developの状態を確認環境で見る」）。
  オフセットは1〜(幅 - 1)を巡回する
- **帯の幅より小さいIssue番号ではこれまでと同じ**（`ベース値 + Issue番号`に一致する）。issue-deckの
  #1999までと、他リポジトリの現在のIssueはポートが変わらない
- **折り返した先は同じ帯の古いIssueのポートと重なる**（issue-deckなら1999番違い。1日あたり約48件の
  ペースなので約42日ぶん離れたIssue）。重なった2つが同時に起動していなければ実害は無く、同時に
  起きた場合は`dev_server_wait_for_port`（#2464）が検出して`dev_server_port_owner_worktrees`
  （#2470）が相手のworktreeを名指しする
- **帯の割り直しは要らなくなった。** 代わりに守るべきは「対応表の帯が重なっていないこと」だけで、
  `local-port-bands.test.ts`が実物の対応表に対して`findOverlappingLocalPortBands`で確かめ、
  あわせて各帯のポートが帯からはみ出さないことをシェルの`dev_server_port_for_issue`で確かめる
- 起動時に折り返しが起きたIssueには、`dev_server_port_note`が理由を1行で出す
  （「Issue番号 2478 は帯の幅（2000）を超えるため、帯（4000〜5999）の中で折り返して 4479 を使います」）

### エフェメラルポート範囲（32768〜60999）へは帯を払い出さない（#2478・#2487）

サブPCの`net.ipv4.ip_local_port_range`は`32768 60999`。**この範囲は外向きの接続が一時的に使う
（エフェメラルポート）ので、開発サーバーのような長時間の待ち受けを置くと、たまたま同じ番号が
使われていたときだけ`EADDRINUSE`になる。** 再現しづらい形で表面化するため、帯はこの範囲に
1ポートも掛けない。
...
払い出し（[`lib/new-app/local-port-bands.ts`](../../src/lib/new-app/local-port-bands.ts)の
`chooseNextLocalPortBase`）はこの範囲に掛かるベース値を飛ばす。**空きは「32768未満」と
「61000〜64999」の飛び地になる。**

- **帯は幅のぶん後ろへ伸びるので、ベース値が32768未満でも掛かる。** 32000は帯としては
  32000〜32999で、32768以降がエフェメラル。判定は`overlapsEphemeralPortRange`が帯の終わりで行う
- `MAX_LOCAL_PORT_BASE`は**64000**（帯の終わりが16bitのポート番号65535に収まる最大のベース値）。
  #2478でIssue番号を帯の中で折り返すようにしたため、「ベース値 + Issue番号」が帯からはみ出す
  ことはなく、60000に抑えておく理由が無くなった
- 現在の帯は4000〜28999。**残っているのは7枠**（29000〜31000の3枠と、61000〜64000の4枠）。
  立ち上げウィザードの下見に「残りの帯は N 枠」として出る（`POST /api/new-app/preflight`）
- **手で行を足すときも同じ範囲を避ける。** 払い出しを通らないため、
  `local-port-bands.test.ts`が`findEphemeralRangeLocalPortBands`で実物の対応表を突き合わせる
- #2478でissue-deckの帯を空き番号へ移さず**帯の中で折り返す**ことにしたのは、これも理由の1つ
  （移すと1リポジトリで数千ポートを占め、飛び地の空きを使い切る）
- **枠を増やすなら`ip_local_port_range`の下限をIANAの動的ポート（49152）へ上げる**と
  32768〜49151が使えるようになる。ただしサブPCの実機設定なので`guchi-apps/subpc`の変更で、
  issue-deck側からは行わない（[CLAUDE.md](../../CLAUDE.md)「VPS・サブPCの設定ファイルの変更は、
  管理リポジトリのIssueへ切り出す」）
...
- 開けるポートが見つかるまで**1ずつ繰り上げる**（6000なら6001）。下げると隣の帯
  （前のリポジトリのIssue #999）へはみ出す。Issueごとのセッションでは**帯の端まで来たら
  `ベース値 + 1`へ折り返す**（`dev_server_band_safe_port`。#2478。繰り上げで隣の帯へ出ないため）
- 新しい帯の払い出し（`chooseNextLocalPortBase`）もブロック対象のベース値を飛ばす
- **Issueごとのセッション（ベース値 + Issue番号）も同じ繰り上げを通す**（#2470）。ブロック対象は
  6000だけではないため、`6566`（dayspan #566）・`6665`〜`6669`（dayspan #665〜#669。IRC）・
  `10080`（clip-hive #80）のように当たりうる

#### 繰り上げの計算は`dev_server_port_for_issue`だけに置く（#2470）

**片側にだけ繰り上げを入れると、起こしたセッションを止められなくなる。** 止める側が繰り上げ前の
ポートを探しに行くため。採番も停止も
[`scripts/lib/dev-server.sh`](../../scripts/lib/dev-server.sh)の`dev_server_port_for_issue`を
呼び、自前で`ベース値 + Issue番号`を計算しない（`local-port-bands.test.ts`が呼び出し側も見る）。

| 呼ぶ場所 | 用途 |
| --- | --- |
| `scripts/start-issue.sh` | issue-deck自身のセッションの採番と、`--recreate`でworktreeを作り直す前の停止 |
| `scripts/generic-start-issue.sh` | 汎用ランチャー（#1224）の採番 |
| `scripts/cleanup-worktrees.sh` | worktreeを消す前の停止（#1524） |

**繰り上げた先は同じ帯の別Issueのポートと重なる**（`6566`→`6567`は#567、`6665`〜`6669`はまとめて
`6670`で#670とも重なる）。これは避けようがない——帯の中のどのオフセットも別のIssue番号でありうる
ため、`ベース値 + Issue番号`を保ったまま衝突しない写像は作れない（ブロック対象より後ろを全部
1つずつずらせば衝突しなくなるが、今まで動いていたIssueのポートまで動く）。**重なった2つが同時に
起動していなければ実害は無い**ので、次の2つで受ける。

- 起動側: ポートを掴めなかったことは`dev_server_wait_for_port`（#2464）が検出して警告する。
  そのポートを別のworktreeが掴んでいれば`dev_server_port_owner_worktrees`が相手のworktreeを
  警告へ添えるので、「`PORT`が渡っていない」（#2464）と見分けられる
- 停止側: `dev_server_stop_by_port`はcwdで対象worktreeに絞るため、同じポートで待ち受けている
  別Issueの開発サーバーを巻き込むことはない
...
| 5 | Issueに`11.local`が付いていない。**見るのは条件6の引き渡し済みの経路だけ**（#2474） | 実装エージェントが引き渡し時に自分で外すラベル。付いている間はローカルで作業中。**CLOSED・マージ済みの経路では見ない** |
...
- **条件5を見るのは引き渡し済みの経路だけ**（#2474）。以前は条件6より手前に置いていたため、
  引き渡し時に`11.local`を外し忘れたセッションは**PRがマージされても永久に残った**。しかも
  期限もリトライも無い`hold`なので畳む経路も猶予も決まらず、`.reap`が書かれないぶん
  **画面に自動終了の残り時間すら出なかった**（実例: `guchi-apps/dayspan#467`。IssueはOPEN・
  `70.needs-decision`と`11.local`付きのまま、ブランチ`issue-467`のPRはマージ済みだった）。
  成果物が本流へ入っている以上、そのworktreeでローカル作業を続ける理由は無い。人が触っている
  最中に消える心配は他の条件が受け持ち（条件3の`Stop`＝承認待ち・作業中は畳まない、条件7の
  cleanでpush済み）、畳んでもworktreeは残るので画面から起動し直せば会話の続きから再開する。
  **引き渡し済みの経路（条件6の3つ目・4つ目）だけは従来どおり見る**——あちらの判定は
  「`11.local`を外した＝もう作業しない」という宣言を前提に組んであり、無視すると
  「PRを出しただけでまだローカルで続けている」セッションが猶予5分で畳まれてしまう。
  引き換えに、`11.local`付きでIssueがOPENのセッションでは毎巡`gh pr list`（マージ済み）を
  1回余計に叩く。
...
- **PRを人の指示で作るリポジトリは、条件6の3つ目・4つ目を通さない**（#2499）。一覧は
  [scripts/local-repo-pr-policy.conf](../../scripts/local-repo-pr-policy.conf)（今は
  `guchi-apps/ideas`だけ）で、判定は`scripts/lib/pr-policy.sh`が持つ。載っているリポジトリでは
  「PRが無い＝もう作業しない」が成り立たない——構想は一度で仕上がらず、同じセッションで何度も
  練り直す。**`issue-<番号>`のPRができるまでは期限もリトライも無い`hold`で残す**（`.reap`も
  書かないので、画面に終了予告も出ない。指示を出せばまだ続けられるセッションが「まもなく終了」と
  見えるのを避ける）。PRができた後は3つ目の経路でこれまでどおり畳まれ、IssueのcloseとPRの
  マージも従来どおり効く。畳みたくなったら画面の「終了」を押す。
  **主たる仕組みは起動プロンプト側（`{{PR_POLICY_INSTRUCTIONS}}`）の「PRを作るまで`11.local`を
  外さない」**で、条件5のholdが先に効くため、この分岐まで来るのは**その指示どおりに
  ならなかったとき**だけ。それでも置くのは、求められているのが「PRができるまで続く」という
  条件そのもので、指示の遵守に賭けると直そうとした事象（会話が5分で消える）がそのまま残るため。
  引き換えに、PRを作らずに終える構想のセッションは自分では畳まれない。
...
来て同じセッションを再利用した実績があるが、これは畳まれた後でも起動し直しで同じことができる。

**生きている間の追加指示は画面から送れる**（#1012）。Issue詳細のセッション表示の「追加指示」で、
...
**Codexで起こしたセッションへは`codex queue`で積む**（#2519）。走っているターンは止まらず次の
ターンの頭に届き、起動直後のディレクトリの信頼確認に答えるまでは押せない（[codex.md](codex.md)）。

**`11.local`を外さないまま止まったセッションは、引き渡し済みの経路では畳まれない。**
`00.check-user`で人へ引き上げて止まっているセッションが該当する。これは意図した挙動で、
人が見るまで残す。**ただしIssueがCLOSEDになった・`issue-<番号>`のPRがマージされた場合は、
`11.local`が付いたままでも畳む**（#2474）。
...
- **届かなかったときに貼り直せる。** 数KBのプロンプト全文を貼り直すのは現実的でない。起動前に
  この文面を必ず端末へ表示しておく
- **2つ目の段落（丸投げの禁止）は#2896で足した。** これを外すと、キックオフへの最初の応答で
  作業まるごとを`Agent(fork)`へ委任しようとし、その呼び出しがtool_useにならずテキストで出て
  セッションが止まる現象が戻る（直近45日で31件・全件`sonnet-5`。
  [session-notify.md](session-notify.md)「起こしているのは`sonnet-5`の…」）。再開
  （`--continue`）の文面にも同じ段落が付く
...
**ファイルを読むための`Bash`の規則は入れない。** `cat`・`grep`のような規則は前方一致で当たるため、
`cat > file`のようなリダイレクトによる書き込みまで通してしまう。読む・探すのは`Read`・`Grep`・
`Glob`を使う方針（各実装プロンプトの「調査は往復を減らす形で行う」）で、この3つは**作業
ディレクトリの中なら**承認を挟まない。

#### 起動時に読むworktree外のファイルは`Read`規則で渡す（#2778）

`Read`が承認不要なのは作業ディレクトリの中だけで、**外のファイルは`Read`でも1件ずつ承認を
求められる。** キックオフ文面は「あなたへの指示は`<worktree親>/.prompts/issue-<番号>.md`にあります」
という1文なので、セッションを開いた直後に必ず「Readが次へのアクセスを要求しています」が出て、
無人に近い起動ではそこで止まっていた。

そこで`run-issue-session.sh`は、worktreeの外にあってセッションが読むファイルの絶対パスを
`agent_allowed_tools`へ渡し、`Read`規則として足す。渡すのは次の2つで、性質が違う。

- **指示ファイル**（`.prompts/issue-<番号>.md`）— 起動直後に必ず読む。承認プロンプトが実際に
  出ていたのはこちら
- **開発サーバーのログ**（`.dev-servers/issue-<番号>.log`）— 繋がらないときだけ見る。場所が載る
  だけで読めという指示は無いので、ついでに許可しておくもの。開発サーバーを使わない起動
  （ポート`0`の横断質問セッション）では渡さない

**許可されるのは渡した絶対パスだけで、`.prompts`ディレクトリごとは開かない**（他Issueの
指示ファイルは従来どおり承認が要る）。`--add-dir`で`.prompts`を渡す案は、そこが書き込み可能に
なり他Issueの指示ファイルまで触れるので採っていない。規則にできるのは**絶対パスで、空白・
カンマ・`*`を含まないもの**だけ（置き場所は`ISSUE_DECK_WORKTREE_BASE`などで差し替えられるため
素通しにしない）。`--allowedTools`は "Comma or space-separated list"（`claude --help`）なので、
空白やカンマを含むパスをそのまま足すと1つの規則が2つに割れる。

**絶対パスはスラッシュ2つで始める。** 1つだけだと設定ファイルからの相対として扱われ、規則が
当たらない。`claude -p`でworktree外のファイルを読ませて確かめた結果が次のとおり
（`--permission-mode default`は規則に無いものが即座に承認待ちになるため、規則の当たり判定だけを
見るのに使う）。

| 渡した規則 | 権限モード | 結果 |
|---|---|---|
| `Read(//home/guchi/apps/issue-deck-worktrees/.prompts/issue-2778.md)` | `default` | 読めた（規則が当たっている） |
| `Read(/home/guchi/apps/issue-deck-worktrees/.prompts/issue-2778.md)` | `default` | 承認待ちで停止（スラッシュ1つでは当たらない） |
| 規則なし | `default` | 承認待ちで停止（規則の効果であることの対照） |
| `Read(//…/issue-2778.md)` | `auto` | 読めた |
| 規則なし | `auto` | 承認待ちで停止（**実際に報告された症状はこれ**） |

最後の2行が示すとおり、**`auto`のままでも規則があれば通り、無ければ止まる。** worktree外の
`Read`はクラシファイアのブレではなく毎回承認が要るので、#2762の`Bash`側より確実に効く。

#### 添付画像の保存先はディレクトリごと渡す（#3039）

#2967で画像の配信に認証が要るようになり、ローカルセッションは`fetch-issue-images.sh`で画像を
`/tmp/issue-deck-images/`へ落としてから`Read`で開く形に変わった。**ここもworktreeの外なので、
画像を開くたびに承認待ちになっていた。** 「今日はReadの許可を頻繁に求められる」という報告
（#3039）を、会話転記の`tool_use`→`tool_result`の時間差（ファイル系ツールは通常1秒未満）で
追うと、9/17〜18に承認待ちになった`Read`はほぼ全件がこの画像で、実装・横断質問のどちらの
セッションでも、issue-deck以外のリポジトリでも起きていた。権限モードは全ターン`auto`のまま
（`permission-mode`レコード）で、設定ファイルも変わっていなかった。

画像のファイル名は起動時に決まらないため、ここだけ**末尾`/`付きのディレクトリとして渡し、
`Read(//tmp/issue-deck-images/**)`にする**（`--permission-mode default`で、規則ありなら読め、
無ければ拒否されることを確かめた）。スクリプトの既定の保存先は`TMPDIR`で変わるので、
`run-issue-session.sh`が`ISSUE_DECK_IMAGE_DIR`をセッションの環境へ明示し、規則と保存先を
同じ値に揃えている。ルート（`/`）を渡しても規則にはしない。無人実行（Actions）は
`--out /tmp/issue-images`で別の場所へ落としており、この規則の対象外。

**承認が増えたと報告されたら、まず転記の時間差を見る。** 承認プロンプトそのものは転記に
記録されないが、承認を待った呼び出しは`tool_result`が数十秒〜数十分遅れて返るので、
遅れた呼び出しの対象パスを並べれば、どこへのアクセスが承認を求めているかが分かる。
...
- 暴発の歯止め（`70.needs-decision`を付ける・1回に目安3件まで・起票しても実装しない）は
  プロンプト側にある（#735・#1528）
