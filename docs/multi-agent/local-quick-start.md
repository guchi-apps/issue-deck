...
- 現在の帯は4000〜26999で、次に払い出されるのは27000。**32767までに残っているのは6枠**
- `MAX_LOCAL_PORT_BASE`（`lib/new-app/local-port-bands.ts`）は60000のままで、この範囲を
  またいで払い出せてしまう。埋まる前に、帯の上限を下げるか`ip_local_port_range`の下限を
  上げるか（`guchi-apps/subpc`側の設定）を決める必要がある
- #2478でissue-deckの帯を空き番号へ移さず**帯の中で折り返す**ことにしたのは、これも理由の1つ
  （移すと1リポジトリで数千ポートを占め、32768未満の空きを使い切る）
