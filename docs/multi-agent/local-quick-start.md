...
| 4 | 2・3が届かなかった分を定期的に掃く | `scripts/sweep-orphan-dev-servers.sh`（systemd timerが1時間ごとに呼ぶ・#1525） |

4は2・3の重複ではなく**入口が違う**。2・3の入口は`.dev-servers/issue-<番号>.pid`だけなので、
PIDファイルが残らなかった孤児と、pollerそのものが止まっている場合を丸ごと取りこぼす。4は
**実際にポートを掴んでいるプロセス**から入り、pollerとは独立した経路で走る。
