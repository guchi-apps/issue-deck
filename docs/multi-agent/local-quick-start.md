...
そこで`run-issue-session.sh`は、そのセッションが必ず読む2ファイル——指示ファイルと開発サーバーの
ログ（`.dev-servers/issue-<番号>.log`）——の絶対パスを`agent_allowed_tools`へ渡し、`Read`規則として
足す。**許可されるのはその2ファイルだけで、`.prompts`ディレクトリごとは開かない**（他Issueの
指示ファイルは従来どおり承認が要る）。`--add-dir`で`.prompts`を渡す案は、そこが書き込み可能に
なり他Issueの指示ファイルまで触れるので採っていない。

**絶対パスはスラッシュ2つで始める。** 1つだけだと設定ファイルからの相対として扱われ、規則が
当たらない。クラシファイアを外した`--permission-mode default`（規則に無いものは即座に承認待ち）で
worktree外のファイルを読ませて確かめた結果が次のとおり。

| 渡した規則 | 結果 |
|---|---|
| `Read(//home/guchi/apps/issue-deck-worktrees/.probe.txt)` | 読めた（規則が当たっている） |
| `Read(/home/guchi/apps/issue-deck-worktrees/.probe.txt)` | 承認待ちで停止（スラッシュ1つでは当たらない） |
| 規則なし | 承認待ちで停止（規則の効果であることの対照） |
