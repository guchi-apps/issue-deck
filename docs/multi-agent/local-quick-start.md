...
| 5 | 3・4の在庫に載っていない分を`/proc`の走査で見つけて止める | 同上（#1525） |

5は3・4の重複ではなく**在庫の取り方が違う**。3・4が見るのは`.dev-servers/issue-<番号>.pid`だけで、
...
| 3 | プロセスグループ | リーダーが同じcwdに居て、リーダー自身も1を満たす |
| 4 | tmuxセッション | `<repo>-issue-<番号>`が**存在しない** |
| 5 | 起動からの経過 | `DEV_SERVER_ORPHAN_GRACE_MINUTES`（既定30分・**0で無効**）以上 |

- **コマンドラインの部分一致で探さない。** `claude`はプロンプト全文をargvに持ち、そこには
  Issueの本文がそのまま入る。#1523の調査で`ps -eo pid,args | grep next-server`を叩いたところ、
  **#1524を担当している`claude`プロセス自身がヒットした**（本文中の`next-server`という記述に
  当たったため）。同じ罠は[scripts/lib/worktree-status.sh](../../scripts/lib/worktree-status.sh)にも
  書かれている。`/proc/<pid>/cmdline`はNUL区切りなのでargvを要素として取り出せ、要素そのものが
  `dev`と一致することを求めれば本文の文字列では偽装できない
