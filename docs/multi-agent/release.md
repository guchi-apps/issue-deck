...
3. バンプのステップが前回のバンプを`git revert -m 1`で取り消し（版・更新履歴・
   `.github/release-notes.md`がバンプ前へ戻る）、そのうえで通常どおりバンプする。上げ幅・
   更新履歴・使い方・対象一覧は修正込みの差分から作り直される
4. **版は必ず取り消した版より上にする**（判定がv6.4.0でもv6.4.1）。同じ番号にすると差し引きで
...
「後に入った変更」を出し、0件なら押せなくする。ダイアログを開いたときに1回だけ取る
...
- **上げ幅を引数に取るbump-command**（signalyの`bump_version.py`）では、版を繰り上げた場合に
  書き換え結果が一致せず失敗する。そのリポジトリでは上げ幅を1段上げて作り直す
