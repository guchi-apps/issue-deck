...
- **close済みのレビューも並べる**（`LABEL_FILTER_PRESETS`の`code-review`が`state: "all"`）。
  ...
  **左メニュー・スマホのビュー切替の数字はopen（未完了）のぶんだけを出す**（#3081。
  `computeNavCountsForFilters`）。一覧の行数のままだと、読み終えたレビューまで数えて
  「あと何件読めばよいか」が読めない。ヘッダーの「未完了N件」と同じ数字になる
- **並べるのは「未完了は全部＋完了した新しい20件」**（`limitCodeReviewHistory`。
  `src/lib/issue-stats.ts`）。**窓を持たせないと、一覧・下の取得の2つが
...
レビューしていないリポジトリがどこか読み取れなかったため。見出しの「レビューを実行」は
従来の緑の帯（#698の起動口）を統合したもので、Issueが0件でも出る。
...
- **実施日はレビューIssueの作成日時で数え、一覧の20件上限より前の全件から数える**
