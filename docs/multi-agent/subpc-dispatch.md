...
#### `claude --resume`によるsession idの管理は持たない

Phase 3 のもう1つの論点だったが、**採らない**と結論した。tmuxでプロセスを生かしている限り
contextはプロセス内にあり、`--resume`は「畳んだ後に文脈を戻す」ための手段になる。#1256が畳むのは
7つの条件を**すべて**満たしたセッション（IssueがCLOSEDかPRがマージ済み・`11.local`が外れている・
worktreeがcleanでpush済み）だけで、そこまで進んだセッションの文脈を戻す価値は薄い。
session idをDBに持たせると、issue-deckがサブPCのローカルファイル（`~/.claude/projects/…`）の
識別子を持つことになり、`hosts/route.ts`の「ホストの内部状態は持ち込まない」取り決めにも触れる。
畳まれた後に追加指示が要る場合は従来どおり画面から起動し直す（worktreeは残るため再開扱いになる）。
**必要性が実測で見えてから足す。**
