...
既定の2は**Athlon 200GE（2C/4T）だった頃の実測**（#1177）にもとづく（並行3本が上限。
`next build`単体で4スレッド中2.6を使い切るため、実運用の快適さでは2本）。

> **この2は載せ替え前の値のまま**（#1791）。サブPCのCPUはRyzen 5 PRO 4650G（6C/12T・L3 8MiB）へ
> 載せ替え済みで、上の「4スレッド中2.6」というCPU律速の根拠はもう成立しない。代わりに上限を
> 決めるのは**メモリ（13Gi・載せ替えでは変わっていない）**になる。旧実測の同時ビルド時1本あたり
> 約2.63GiBをそのまま当てると4本で10.5GiB前後だが、スレッド数が増えたぶん`next build`の並列度が
> 上がって1本あたりのピークも増える可能性があり、**何本まで載るかは実測でしか分からない**。
> 測り直し（guchi-apps/subpc#19）の結果が出るまで既定値は2に据え置く。
>
> **上げてみるだけならコード変更は要らない** — アプリ設定ダイアログから3以上を選べる（上限は8）。
> ただし上げるときは`DISPATCH_MAX_JOBS`（既定1）も合わせて見直すこと。1巡で1本しか取りに
> 行かないため、同時実行数だけを3にしても3本目が立つのはポーリング間隔（既定30秒）の2巡後になる
