...
- **その手順を実行する端末がサブPCであること**（`resolveManualStepDevice` →
  `isSubpcManualStepDevice`）。VPS・1Password・GitHub App・ブラウザでの設定はissue-deckから
...
### 埋め戻しが完走しないと、直近のセッションほど届かない（#2870）

「AI使用量」画面の直近日だけがGitHub Actionsばかりになり、Claude・Codexのローカル実装が
反映されない不具合を追った（#2870）。原因は表示側ではなく`report_session_usage`
（`scripts/subpc-dispatch-poller.sh`）の送信の詰まり方にあった。

- `session-usage-phase-backfill.stamp`・`session-usage-code-review-backfill.stamp`
  （`$HOME/.local/state/issue-deck/`）は、**対応する4つの埋め戻し印が全部揃うまで
  「通常モード」（直近2日だけ）へ切り替わらない。** #2779・#2832でこの2つの印が増えて
  以降、一度も完走していないホストは**毎巡ずっと「埋め戻しモード」**（既定30日ぶん・
  このホストの実測で約1,690セッション）のままになる
- 埋め戻しは`session_usage_report_payload`が200件ずつのチャンクへ分けてPOSTする。
  受け口（`storeSessionUsage`）は1件ずつ`upsert`する設計のため、チャンクによっては
  `curl --max-time 30`のタイムアウトに達しうる
- `report_session_usage`はチャンクが1つでも失敗したら残りを送らず関数を抜け、
  **タイムアウト（ステータス`000`）は`404`と同じく黙って見送っていた**ため、
  journaldにも痕跡が残らなかった
- 転記は`session_usage_transcripts`が**最終更新の古い順**に開くため、チャンクも
  古い順になっていた。**途中のチャンクで詰まると、それより新しいセッションは
  一度も送られないまま、次の巡回でも同じ場所で詰まり続ける**

対策は2つ。`session_usage_report_payload`がチャンク化する直前に終了時刻の新しい順へ
並べ替え（送信順だけの変更で、`session_usage_aggregate`側の重複除去の基準＝転記を
読む順は変えていない）、途中で詰まっても新しいセッションだけは先に届くようにした。
あわせて`000`も`404`と分けてログへ残すようにした（`report_api_failure`）。

**埋め戻しが長期間完走しないホストが今後も出うる**（新しいフィールドを足すたびに
埋め戻し印の名前を変える設計のため）。「4つの印が全部揃っているか」は
`$HOME/.local/state/issue-deck/session-usage-*.stamp`で確認できる。
