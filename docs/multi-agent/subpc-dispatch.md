...
**ヘッダーではなく一覧の上のバー**（`issue-list.tsx`。手作業アシスタント・「次にやること」と
同じ位置）。スマホの一覧は`IssueList`のヘッダーを出さない（`showHeader={false}`）ため、
ヘッダーに置くとPCからしか押せない。当初はPCのヘッダーにチェックボックスのアイコンだけを
置いていたが、**スマホから使えず、PCでもラベルが無くて見つけられなかった**。

**出すのは積めるIssueが2件以上あるときだけ**（`lib/dispatch/bulk-dispatch.ts`の
`bulkDispatchableIssues`）。closeしたIssue・既に走っている（積んである）Issueは数えない——
押しても何も起きない件数を入口に出さないため。1件しか無いなら個別の「実装を開始」で足りる。

### まとめて積むときのオプションは1回だけ選ぶ（#1993）

**選んだ条件は、選んだIssueすべてへ同じように付く。** まとめて実行したいのは「同じ条件で
流したいIssueが複数ある」ときで、条件をIssueごとに選び直せる必要が無い。

> **#1266では「オプションを選ばせない」としていた**（Issueごとに要否が違うものを一括で
> 決める方が事故になる、という理由）。選べないと、計画を挟みたいIssueだけ個別に積み直すことに
> なり、まとめて積む意味が薄れていたため方針を変えた。事故を減らすための縛りを3つ置いている。

- **既定は全部OFF。** 種別ラベルからの既定（`startImplementationOptionsFromLabels`）は
  Issueごとに違うため、一括では当てない。押したものだけが付く
- **既に付いているラベルは外さない**（足すだけ）。**オプションのラベルは積むより先に付ける** —
  `21.plan-required`等はランチャーが起動直後に読む（`scripts/start-issue.sh`）ので、積んだ後に
  付けると払い出しがその隙間に入ったときに読まれない（「実装を開始」ダイアログも同じ順序）。
  `11.local`は従来どおり**積めたときだけ**なので、書き込みは2回に分かれる
- **選んだIssueで共通して選べるものしか出さない**（`commonStartImplementationOptions`）。
  実行先はサブPC固定なので撮影（`24`。Actions専用）は出ず、さらに**選んだIssueのリポジトリ
  すべてに定義があるラベル**だけを残す。ラベルの付与は存在しない名前を渡すと**その場で
  作ってしまう**ため、片方にしか無いラベルを一括で配ると押した覚えの無いラベルが増える
  （リポジトリのラベル定義は`use-repository-label-names.ts`が`/api/issues/meta`から
  リポジトリごとに1回だけ取る）

**確定するまでチップを出さない**（#1666と同じ理由）。取得の途中で出すと、押そうとしたチップが
指の下で入れ替わる。
...
**押せるかどうかは「選んだうち1件でも積めるか」で決める**（#1993）。先頭のIssueだけで判定すると、
そのリポジトリがサブPCにcloneされていないだけで、他のIssueまで積めなくなる。
