...
**`MANUAL_STEP`はセッションにも tmux にも触らない**（枠外で先に配る点だけ制御ジョブと同じ）。
詳細は[手作業アシスタントからの代行実行](#手作業アシスタントからの代行実行1828)。
...
- **自動実行の状態を持つのは画面だけ**（`hooks/use-manual-step-autorun.ts`）。サーバーにもDBにも
  「自動で進めている」という状態を置かない——置くと、画面を閉じた後・別の端末から開いたときに
  誰が続きを進めるのかを決める必要が出る。**ダイアログを閉じれば止まる**
