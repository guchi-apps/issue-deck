...
- 起票当時、**VPSはtailnetに参加していなかった**（push型を採るならVPSをtailnetに入れる工程が
  前段に丸ごと増える状態だった）

**VPSはその後tailnetへ参加したが、pull型のままにしている。** forced commandが無いのは変わらず、
issue-deck（VPS）からサブPCを叩く経路を作ると、サブPCが落ちている間のジョブの置き場が要る。
到達できるようになったことは**逆向き（サブPC → VPS）にだけ使っている**——手作業の代行実行で、
`（VPS）`と書かれた手順をサブPCからSSHして実行する（後述の[VPSの手順を代行する](#vpsの手順を代行する2901)）。
...
**`REBOOT`もセッションにも tmux にも触らない**（#2496。`SELF_UPDATE`と同じ枠外）が、
...
そのボタンは確認環境の画面には無く、[画面から更新して再起動する](#画面から更新して再起動する1875)の
とおり実行キュー（PCはトップバー右のアイコン／スマホはヘッダーの「実行状況」）を開いた先の
サブPCのカードにしか出ない。**理由を出す画面と押す場所が違うとき、ボタン名だけでは探せない。**
あわせて、リポジトリを選び直しても変わらない理由（申告なし・応答なし・poller非対応。
`isHostWidePreviewRejection`）は行ごとに繰り返さず、ホストの見出しの下へ1回だけ出す
（十数行に同じ長文が並ぶのと、リポジトリ名の幅が潰れるのを避ける）。

**中断（`MANUAL_STEP_ABORT`）は代行実行とは別の申告（`manualStepAbort`）で配る。** 代行実行を
実行できるpollerでも、止める側の実装（poller v13以降）が入っているとは限らない。未対応のホストへ
配ると未知の種別として`failed`になり、画面には「中断できなかった」だけが残る——そこは
「打ち切り（5分）まで待つことになります」と先に案内する方が正しい。
...
#### 「停止」と「セッション終了」の使い分け（#1557）

画面のボタンは`INTERRUPT`が「停止」、`KILL`が「セッション終了」（#3108までは「セッションを閉じる」）で、**止まる範囲が違う**。

| | 停止（`INTERRUPT`） | セッション終了（`KILL`） |
|---|---|---|
...
この違いは、かつて停止と終了が並んでいるときだけ添える1行として画面に出していた（#1557。
**ボタンの文言だけでは押すまで分からず、実際に問われた**）。**#3108でその1行は外した**——スマホの
Issue詳細でカードの縦幅を食っていたため。違いを読める場所は、この節と「セッション終了」の
確認ダイアログだけになっている。

#### 操作は畳み、出口と押した結果は畳まない（#1676）

Issue詳細の実行中カード（`IssueStatusCard`）では、**停止・追加指示・セッション終了の3ボタン
（#3108で1行に収めた）を「操作」トグルの中へ畳んでいる**。起動ジョブの行（「サブPCで起動しました」）も、起動が
成功してセッションが立っていればセッションの行へ畳む（起動時刻と`tmux attach`のコピーは
展開側へ移る）。スマホではこのカードだけで14行を占め、Issue本文が初期表示から押し出されていた。
...
- **押した操作の結果**（送信済み・pollerが見送った理由・失敗）。pull型で届くまで最大1分あり、
  送り直してよいかの判断がここにしかない。未処理の制御ジョブがあるあいだは操作も開いたままにする
...
そこでissue-deck側が、IssueがcloseされたときにIssue詳細の「セッション終了」と同じ`KILL`
...
#### 畳んだセッションは画面から復旧する（#1830）

再開の仕組み（`--continue`）は上のとおり前からあるが、**押す場所が終了したセッションの行に無かった**。
終了すると行は「サブPC・終了」だけになって操作が消え、起動は別の場所にある「サブPCで開始」（新規に
始めるのとまったく同じ文言）を探すしかない。回答が遅れて畳まれた人が見ているのは終了した行の方なので、
Issue詳細の「いま何が起きているか」カードの、その行のすぐ下に**「セッションを復旧」**を置いた
（`src/components/dashboard/session-recovery-button.tsx`。PC・スマホ共通）。

- **積むのは起動ジョブ（`LAUNCH`）そのもの**で、「サブPCで開始」と同じ手順を共有する
  （`src/hooks/use-local-session-launch.ts`。`11.local`を付け直すところまで同じ）。
  **issue-deck側は「もう一度起動する」以上のことをしない**——session idもホストの内部状態も
  持たないという上の取り決めは変えていない
- **終わり方を書き分ける。** `WAITING_INPUT`のまま消えたセッションは「回答前に終了」（琥珀色）と出し、
  復旧ボタンを主導線（塗りつぶし）にする。役目を終えて畳まれたのか、こちらの回答を待ったまま
  消えたのかは、押す前に区別できないと判断できない。**`WAITING_INPUT`を「今も待っている」意味で
  使っているわけではない**（待つ相手はもういない）。終了時点の記録として読んでいるだけで、
  答えた後に作業が進めば`WORKING`／`RESPONDED`へ移る
- **押せない理由（サブPCが未申告・未処理の起動ジョブがある・もう立ち上がり直している）は
  起動ジョブと同じ判定**（`resolveDispatchTargetRejection`）を使い、ボタンは消さずに理由を下に出す
- **横断質問セッションも復旧できる**（#3033）。ただし積むのは`LAUNCH`ではなく質問ジョブ
  （`CROSS_REPO_QUESTION`）で、`11.local`は付けず、押せない理由も横断質問と同じ判定
  （`resolveCrossRepoQuestionRejection`）を使う。**`--continue`は使えない**（cwdが質問Issue間で
  共有されるため別の質問を拾う。#1648）が、ランチャーが質問Issueごとに控えたsessionIdで
  `--resume`するので同じ質問の会話へ戻れる（[後述](#畳んだ質問セッションは同じ質問の会話へ戻れる3033)）。
  判定は、そのセッションを立てた質問ジョブが画面に残っているかどうかで行う
- **自動では復旧しない。** 落ちたセッションを勝手に立て直す実行体は作らない（[gates.md](gates.md)）

### APIエラーで中断したセッションの自動再開（#1971）

**画面から何も押されなくても、pollerが1巡ごとに行う唯一の送出**（`resume_interrupted_sessions`）。

Claude Codeはサーバー側の一時エラー（529 Overloaded など）を再試行し、使い切ると
...
- 上の「追加指示」も段1aで弾かれ、**人が画面から送ることすらできない**

2026-08-18 16:12〜16:37（UTC）に6セッション（issue-deck #1947・#1956・#1962・#1964、aide #35・#89）が
同時にこれで止まり、人が端末から「続けて」と打つ 21:55 まで5時間半動かなかった。転記の最後は
例外なく `isApiErrorMessage: true` のレコードで終わっていた。
...
時点で禁止の側へ戻る（変えたい文面があるなら、それは人が「追加指示」で送るもの）。

**「落ちたセッションを自動で立て直さない」（上の`WAITING_INPUT`のまま消えた場合）とは別の話。**
あちらは死んだセッションを新しく起こす判断で、こちらは**生きているセッションが自分の続きを
...
コマンドを実行して、終了コードと出力を画面へ返す**。ユーザーは内容を見て承認するだけで手順が進む。

```text
手作業アシスタントの手順画面「承認して実行」
  ↓ POST /api/dispatch（kind=manual_step / 送るのは「どの手順か」＋「承認したコマンド」）
  ↓ サーバーが手作業Issueの本文から**抽出し直した**コマンドをジョブに載せる
（キュー: DispatchJob）
  ↑ claim（manualStepCapable を申告したホストにだけ払い出す）
scripts/subpc-dispatch-poller.sh
  ↓ GitHubの本文を読み直して照合（ラベル・コマンドが本文にあるか）
  ↓ systemd-run --user --collect（**pollerとは別のcgroup**）
scripts/run-manual-step.sh → コマンドを実行 → POST /api/dispatch/report（終了コード＋出力）
```

**これは`gates.md`の「実行体が判断して組み立てた文字列の送出」に当たらない。** 文面を決めるのは
本文を書いた人で、実行するかどうかを決めるのも押した人。実行体が状況を読んで内容を組み立てる
経路はここにも作っていない（[gates.md](gates.md)「やらせないこと」の例外である「人が押した
1回の操作」の3つ目にあたる）。

#### 実行できるのは本文に書かれたコマンドだけ（照合を2回する）

| どこで | 何と照合するか | 通らなかったら |
|---|---|---|
| issue-deck（`enqueueManualStepJob`） | **DBのIssueキャッシュの本文**から抽出し直したコマンドと、画面が送ってきた「承認したコマンド」 | 積まない（`body_changed`。押した人に理由が返る） |
| サブPC（`run_manual_step_job`） | **GitHubから読み直した本文**に、受け取ったコマンドがそのまま含まれるか＋`71.manual-step`が付いているか | 実行しない（`skipped`） |

**画面から届いた文字列は照合にしか使わない。** ジョブに載るのはサーバーが抽出したもので、
画面から任意のコマンドを流す経路にはならない。**サブPC側の照合はGitHubを一次情報にする**ので、
issue-deckのDBだけを書き換えても本文に無いコマンドは実行されない。

照合できないとき（`gh`が無い・未認証・Issueを読めない）は**実行しない側へ倒す**。この仕組み
全体で一貫している向きで、pollerは`gh`が使えなければ`manualStepCapable`を申告しない。

#### 対象はサブPC・VPSで実行する手順だけ

- **その手順を実行する端末がサブPCかVPSであること**（`resolveManualStepDevice` →
  `resolveManualStepRunTarget`）。1Password・GitHub App・ブラウザでの設定はissue-deckから
  到達できない。**判定は手順ごと**（#2052）で、手順の文頭に`（サブPC）`と書かれていればそれ、
  無ければ`## 前提条件`の「実行するデバイス」を既定値として使う。デバイス欄に端末が複数
  書かれている本文では既定値が決まらないので、デバイスの無い手順は代行しない
  （`## 完了の確認方法`は節に手順が無いため、常に既定値で判定する）
- `## やること`の1手順の中に、**シェルのコードブロックがちょうど1つ**あること
  （`extractManualStepCommands`）。0個・2個以上はどちらも代行しない——2つあると「どちらを
  実行したのか」がチェック1つに対応せず、片方だけ実行して成功した状態でチェックが付く
- 言語の指定が無いコードブロック（```だけ）も対象外。出力例・設定の抜粋が同じ書き方で置かれる

**`## 完了の確認方法`も対象**（#1869で入れた。#1828の時点では対象外だった）。ただし手順とは
条件を変えている。

| | `## やること`の手順 | `## 完了の確認方法` |
|---|---|---|
| コードブロックの数 | **ちょうど1つ**のときだけ代行する | **複数あれば全部**、上から順に代行する |
| 指す行番号（`manualStepLine`） | `- [ ]`の行 | そのコードブロックの**開きフェンスの行** |
| 成功したら | その手順にチェックを付ける | **何も書き換えない**（チェックもクローズもしない） |
| 出力の表示 | 成功したら畳む | **成功でも開いたまま出す** |

手順を1つに限っているのは「どれを実行したのかがチェック1つに対応しない」ためで、**確認節には
チェックが無いのでその問題自体が起きない**。逆に、確認は**終了コードしか見ない**（本文の
「期待する出力」との照合はしない）ため、0で終わっても完了とは限らない——だから成功しても
出力を畳まず、クローズは人が押す。

#### 対話が要るコマンドは代行しない（#2025）

代行実行は標準入力を閉じて走らせるため、**サインインの答えを渡せない**。そのまま積むと失敗するか
打ち切り（5分）を待つだけになり、画面には「失敗しました」しか残らない。そこで、コマンドに次のものが
含まれる項目は**押す前から「あなたが実行」として並べ**、自動実行はその手前で止まる（`USER`で止まる
ので、ホスト側の事情で止まったのとは別物として扱われる）。

| 対象 | なぜ代行できないか |
|---|---|
| `op signin` | 1Passwordの個人アカウントへのサインイン。セッションは実行したシェルの環境変数にしか残らない |
| `gh auth login`・`gh auth refresh` | ブラウザでコードを入力する対話的なログイン・スコープ追加 |

判定は`findInteractiveCommand`（`lib/manual-step-command.ts`）1か所が持ち、画面（実行計画）と
API（`enqueueManualStepJob`）が同じ関数を通る。**必ず対話が要るものだけを挙げる**——「設定に
よっては対話になる」ものまで広げると、いま代行できている手順が押せなくなる。挙げそこねたものは
失敗として止まるだけで、人が手元で実行すれば進む。

- **`op signin`を含む手順は、その手順まるごとを人が実行する。** サインインしたセッションは
  そのシェルにしか残らないため、`op signin`だけを人が実行しても、続きのコマンドを代行実行の
  シェルへ渡すことはできない（同じコードブロックに書く）
- **ただし、`op signin`はそもそも書かないで済む**（#2401）。サブPCには書き込み権限つきの
  サービスアカウント（`~/.config/issue-deck/op-writer.env`）があり、1Passwordへの書き込みも
  読み取りも非対話で通る（#1874）。手作業Issueは`scripts/provision-secret.sh`を使う形で書く
  （[labels.md](labels.md)「1Passwordを扱う手順に`op signin`を書かない」）。この経路で書かれて
  いない本文には、本文検査（`manual-step-body-check.ts`）が起票・編集のたびに指摘を返す。
  **代行実行のシェルが読み込むのは`~/.config/issue-deck/dispatch.env`だけ**なので、
  `op read`しかしないコマンドでも、そのトークンを自分で読み込まないと動かない
- 止まった後は、手順なら「実行した・次へ」（チェックが付いて計画から外れる）、
  `## 完了の確認方法`のコマンドなら「自分で実行した・続きへ」で先へ進む。確認にはチェックが
  無いので、**止まると同時に流し終えた扱いで記録する**（記録しないと同じ項目で止まり続ける）

#### プレースホルダを含むコマンドは代行しない（#2051）

`## やること`には、**人が値を埋めてから実行するコマンド**が書かれることがある。それは実行できる
コマンドとまったく同じ`bash`フェンスで書かれるため、それまでの歯止め（本文との照合・
`findInteractiveCommand`・チェック済みは計画から外れる）はどれもこれを止められず、**穴が空いた
まま実行しに行っていた**（実例: guchi-apps/aide#103）。
...
実害は「失敗して止まる」だけではない。上の`=<控えたkey>`は**シェルのリダイレクトとして解釈
されうる形**で、意図しない失敗の仕方をする。そこで判定を`findPlaceholder`
（`lib/manual-step-command.ts`）へ置き、`interactive_command`と同じ層で「あなたが実行」として
並べる（`placeholder_command`）。

| 拾う | 拾わない |
|---|---|
| `<控えたkey>`・`＜番号＞`（角括弧。**内側が空白で始まらない・終わらないものだけ**） | `cmd < in.txt > out.txt`・`cat <<EOF > f`・`diff <(a) <(b)`（実在のリダイレクト・ヒアドキュメント・プロセス置換） |
| `***`（伏せ字） | `*.log`・`**/*.ts`（グロブ） |
| `…`（全角の三点リーダ） | `git diff main...HEAD`（ASCIIのピリオド3つ） |
| `xxx`・`XXXX`（語として現れたときだけ） | `0xxxabc`・`fooxxxbar`（語の一部） |
| | `$HOME`・`${REPOSITORY}`・`"$CHILD_ID"`（**実在の環境変数参照と区別できない**） |
| | `（詳細はPRを参照）`（全角丸括弧。日本語を含む実在のコマンドに現れる） |

**`findInteractiveCommand`とは倒す向きが逆**なので、そこだけ注意する。あちらは「必ず対話が要る
ものだけを挙げる／迷ったら挙げない」で、挙げそこねても失敗して止まるだけだった。こちらは
取りこぼすと**穴あきのコマンドがそのまま走る**一方、誤検知しても「あなたが実行」になるだけで
人が実行すれば進む。被害が非対称なので**迷ったら拾う側**へ倒す。

止まった後の進め方は対話が要るコマンドと同じ（手順は「実行した・次へ」、確認は「自分で実行した・
続きへ」）。

#### 埋めた値を差し込んで代行する（#2403）

#2051は穴の空いたコマンドを止めるところまでで、**埋める場所が画面に無かった**。手作業を実際に
進めるのはスマホでissue-deckを開き、Tailscale越しのターミナルアプリへ切り替えて戻ってくる人で、
前の手順が出した値（トークン・ID）を次の手順へ渡すのが往復のいちばん詰まるところだった。
画面に埋める欄を置き、**埋まった手順はサブPCで代行実行できるようにする**。

```text
画面（<控えたkey> へ値を入力・前の手順の出力から選ぶ）
  ↓ POST /api/dispatch（送るのは「どの手順か」＋「承認したコマンド（テンプレート）」＋「値」）
  ↓ サーバー: 本文から抽出し直したコマンドと照合 → 値を差し込む → 穴が残れば拒否
（キュー: DispatchJob.command はテンプレートのまま／値は placeholderValues）
  ↑ claim（manualStepValuesCapable を申告したホストにだけ払い出す）
poller
  ↓ GitHubの本文と照合（**テンプレートのまま**。値を含む文字列では照合が成立しない）
  ↓ 自分でも差し込み → サーバーの resolvedCommand と突き合わせ → 穴が残れば実行しない
scripts/run-manual-step.sh
```

**画面から送るのは値だけで、コマンドの文字列は送らない。** 差し込むのは
`scripts/lib/manual-step-fill.sh`と`lib/manual-step-command.ts`が持つ同じ規則で、値を
**単引用符で包む**（`'` は `'\''` で継ぎ足す）。単引用符の中はシェルが一切解釈しないため、
値は常に**リテラルの1語**にしかならず、コマンドの構造を変えられない。この経路を
「画面から任意のコマンドを流せる口」にしないための歯止めは、#1828のまま崩れていない。

守っていること。

- **差し込むのは照合の後。** 順序が逆だと、値を含む文字列を本文と突き合わせることになり、
  本文照合そのものが成立しない
- **引用の規則は2実装あり、突き合わせて一致しなければ実行しない。** 重複ではなく2枚目の壁で、
  ずれたときの結果は「誤ったコマンドの実行」ではなく「実行しない」になる（本文照合を2回
  行うのと同じ形）。同じ表を両方へ通す回帰テストが`scripts/manual-step-fill.test.mjs`
- **代行できるかを「山括弧を全部埋めたか」で判定しない。** 埋められるのは名前の付く`<…>`だけで、
  上の表の残り3種（`***`・`…`・`xxx`）は名前が無く埋めようがない。判定は**差し込んだ後の
  文字列へ`findPlaceholder`を掛け直す**（`hasUnfilledPlaceholder`）。個数で数えると、
  残り3種の穴が空いたまま素通りする
- **pollerの申告とセットにする**（`DispatchHost.manualStepValuesCapable`）。古いpollerは知らない
  フィールドを黙って無視し、穴が空いたままの`command`をそのまま実行してしまう。
  `manualStepAbortCapable`（#1882）と同じ作法で、申告が真でないホストへは配らない
...
- **`GET /api/dispatch`（画面が読む方）は`placeholderValues`・`resolvedCommand`を常に`null`で返す。**
  値を必要とするのは`POST /api/dispatch/claim`で取りに来るpollerだけ

##### 本文の書式検査（#2048）には規則として足さない

**プレースホルダを含む手順は、書式が崩れているのではなく正しく書かれている。** 値を人が埋める
手順は本来そう書くしかなく、`manual-step-body-check.ts`の規則として指摘すると「直しようのない
指摘」がすべての正しい手作業Issueに出る。あの検査は「崩れたときにどの機能が落ちるかを言える
もの」だけを持つ決まりなので、ここは対象外にした。

**雛形側でフェンス言語を`bash`以外にする案も採らなかった。** `text`にすれば`extractShellBlock`が
最初から拾わないが、(1) コピーしたときのシンタックスハイライトが失われる、(2) 起票する側が
規約を覚えていることに賭ける形になる——#2048で分かったのは、まさにそこが揃わないという事実
だった。代わりに雛形へ「埋める値は`<…>`で書く」とだけ足し、**判定は本文の書き方に頼らずに
行う**。

#### VPSの手順を代行する（#2901）

`（VPS）`と書かれた手順は、**サブPCからSSHしてVPSで実行する**。手作業Issueで残っていた作業の
多くはVPS側（`.env`の追記・PM2の再起動・DBの作成）で、そこが人の手に残っている限り
「承認1回で最後まで流す」（#1869）は途中で必ず止まっていた。

**新しい実行の口は作っていない。** ジョブも本文照合もpollerも#1828のままで、最後の1歩の
走らせ先だけが変わる。

```text
（画面・自動実行・定期巡回はどれも従来どおり）
  ↓ enqueueManualStepJob が本文から解決した実行先をジョブへ載せる（manualStepRunTarget）
scripts/subpc-dispatch-poller.sh（照合は従来どおりGitHubの本文と2回）
  ↓ payload に runTarget を足して渡す
scripts/run-manual-step.sh
  ├ subpc … これまでどおり bash -c で実行
  └ vps   … ssh -o BatchMode=yes <MANUAL_STEP_VPS_SSH_TARGET> 'bash -s' へコマンドを標準入力で渡す
```

- **接続先はissue-deckが持たない。** ジョブに載るのは`subpc`／`vps`の2語だけで、SSHの宛先は
  サブPCの`dispatch.env`（`MANUAL_STEP_VPS_SSH_TARGET`）にある。**リポジトリはPUBLIC**なので、
  接続先の構成情報（ホスト名・ユーザー名）をGitに置かない（`previewAction`が3語だけを渡し、
  ポートと起動の仕方をpoller側が持つのと同じ作法）
- **鍵は配っていない。** VPSはtailnetに参加していてTailscale SSHが有効なので、認証はtailnetの
  ACLが行う（`authorized_keys`を経由しない）。したがって「サブPCからVPSへ入れる」ことを支える
  のはtailnetの参加であって、issue-deckが配る秘密ではない
- **pollerの申告とセットにする**（`DispatchHost.manualStepVpsCapable`）。古いpollerは
  `manualStepRunTarget`を黙って無視し、**VPSで実行するはずのコマンドをサブPCで実行してしまう**
  ——`manualStepValuesCapable`（#2403）と同じ「配ってから`failed`で返る」では済まない非対応。
  申告は`refresh_manual_step_vps_capable`が**実際にSSHして疎通を確かめる**（`gh`の有無で
  `manualStepCapable`を決めるのと同じ立場で、実行できないまま申告しない）。ただし**毎巡は
  繋がない**（既定5分でキャッシュ。`MANUAL_STEP_VPS_PROBE_INTERVAL_MINUTES`）——申告は30秒ごとに
  走るので、VPSが落ちているときに毎回`ConnectTimeout`ぶん待つと申告そのものが遅れる。
  接続先の設定が消えたときだけはキャッシュを見ずに落とす（落とす方向は待たせない）
- **読み取れない実行先はサブPCへ倒す。** ジョブの列も、pollerが受け取る語も、runnerが読む
  ペイロードも、既知の2語以外はすべて`subpc`になる。#2901より前に積まれたジョブ（列が`null`）が
  そのまま流れるための既定でもあり、**読み取れなかったときに向こう側へ倒さない**ため
- **`sudo`を含むVPSの手順は代行しない**（`findInteractiveCommand`に実行先を渡す）。VPSには
  NOPASSWDの設定が無く（`github-user`・`guchi`のどちらでも`sudo -n`はパスワードを求める）、
  SSH越しの実行には端末も標準入力も無いので必ず落ちる。**サブPCの手順では従来どおり対象外**
  ——あちらはNOPASSWDが効くため、広げると今できている代行が押せなくなる（#2025の「必ず対話が
  要るものだけを挙げる」に沿う）
- **cwdは接続先のホーム。** ローカル実行と同じで、手順はテンプレートどおり自分で`cd`する。
  サブPCのパスを前提に書かれた手順はそのまま失敗し、チェックは付かない
- **コマンドはSSHのargvに載せず標準入力で渡す**（`bash -s`）。ローカル実行と同じ理由で、
  VPS側の`ps`にも出さない
- **打ち切り（5分）はVPS側にも掛ける**（計画レビューG1・指摘1）。**PTYを取らないSSHでは、
  サブPC側の`ssh`をTERMで殺してもリモートのコマンドは走り続ける**（実測で確認した）。
  ローカルだけに掛けると、打ち切りも「中断する」（`systemctl --user stop`）もサブPC側の`ssh`を
  消すだけになり、本番サーバーでコマンドが走り続ける。そこで`ssh <target> "timeout … bash -s"`の
  形にし、**先に切れるのは必ずVPS側**にする（ローカル側は接続待ちのぶんだけ長くしておく）
- **したがって「中断する」はVPS側を即座には止めない。** 止まるのはVPS側の打ち切りが来たときで、
  画面はそれを押す前に出す（`manual-step-run-panel.tsx`の`vpsAbortNote`）——「止めました」だけを
  返すと、本番サーバーで走り続けていることが画面のどこにも出ない
- `## 完了の確認方法`の定期巡回（#2008）も同じ判定で広げた。**VPSの確認コマンドは、そこへ
  到達できるホストが居るときだけ候補にする**（`findNextPatrolTarget`）——積んでも
  `manual_step_vps_unsupported`で弾かれ、巡回の記録だけが失敗として残る

**代行できる端末が増えても、`71.manual-step`の起票の判断は変わらない**（[labels.md](labels.md)）。
VPS・サブPCの**設定ファイル**の変更は従来どおり`guchi-apps/vps`・`guchi-apps/subpc`のIssueへ
切り出す（実機を直接書き換えるとGitに残らない。#2021）。ここで代行できるようになったのは、
**リポジトリ管理外の実機操作**（`.env`の追記・PM2の再起動・DBの作成など）の側。

#### 実行は別のcgroupへ逃がす

pollerは`systemd-run --user --collect --unit=issue-deck-manual-step-<ジョブID>`で
`scripts/run-manual-step.sh`を起こし、**待たずに次のジョブへ進む**。

**サブPCの手作業でいちばん多いのが「`git pull`してpollerを再起動する」**で、pollerの
cgroupの中で`systemctl --user restart issue-deck-dispatch-poller.service`を実行すると、
実行しているプロセスごと殺されて結果を返せない（systemdの既定は`KillMode=control-group`で、
...
- cwdは**ホーム固定**。手作業のコマンドはテンプレートどおり自分で`cd`する。前提条件の
  ...
  分からなくなる
- 標準入力は閉じる（対話を求めるコマンドが答えを待ち続けない）
- `timeout`は5分（`MANUAL_STEP_TIMEOUT_SECONDS`）。ジョブのheartbeatタイムアウト（10分）より
  短くしてあるので、打ち切った結果を必ず報告できる

#### 出力は画面にだけ出す

終了コードと出力（末尾8000字）は`POST /api/dispatch/report`で返し、`DispatchJob`に残る。

- **GitHubのIssueにも通知にもpollerのログにも出さない。** 手作業のコマンドの出力には
  シークレットが混ざりうるうえ、このリポジトリはPUBLICでIssueもActionsのログも誰でも読める
- 画面（ログイン必須）では既定で畳み、**失敗したときだけ開いて出す**（何が起きたのかを
  見ずに次へ進める状態にしない）
- 「誰がいつ何を実行したか」はジョブの行（コマンド・終了コード・実行者・時刻）に残る。
  **手順ごとにIssueへコメントはしない**（手順の数だけコメントが増える）

#### 失敗したらチェックを付けない

終了コードが`0`のときだけ、画面がその手順の`- [ ]`にチェックを付ける（書き換えるのは
既存の`useIssueTaskList`で、**チェックの実体はIssue本文のまま**）。失敗・打ち切り・見送りでは
チェックを付けず、出力を出して人に委ねる（もう一度実行する／手元で実行して「実行した・次へ」）。

**成功しても画面は次の手順へ自動では進まない。** 実行できたことと、出力を見て次へ進んでよいと
判断することは別で、勝手に進むと結果を読む前に画面が変わる。

#### 承認1回で最後まで流す（#1869）

**押す1回の射程は「1手順」ではなく「1つの手作業Issueの手順列＋完了の確認」。** 最初の画面
（`manual-step-autorun-panel.tsx`）に**これから実行されるコマンドを畳まずに全部並べ**、承認すると
上から1件ずつ流す。実行の経路は#1828のまま（`POST /api/dispatch` → `enqueueManualStepJob` →
poller）で、**新しい実行の口は作っていない**。

- **自動実行の状態はDBが持ち、進めるのはサーバー**（#1882で変更。`lib/manual-step-run.ts`・
  `ManualStepRun`）。#1869の時点では画面だけが状態を持ち、**ダイアログを閉じれば止まる**作り
  だった（「誰が続きを進めるのか」を決めずに済ませるため）。「自動実行中に画面を閉じたい・
  そのとき進行状況を確認したい」という要望を受けて、進める役をサーバーへ移した。詳細は
  [画面を閉じても進む](#画面を閉じても進む1882)
- **積むのは1件ずつで、前の1件が終わってから次を積む**（`activeKey`はIssue単位のまま）
- **止まる条件は3つ**。どれも「勝手に進まない」側へ倒している
  - 代行できない項目に来た（人が実行して「実行した・次へ」を押すと続きから流れる）
  - 実行が失敗した（チェックも付けず、次へも進めない）
  - 積めなかった（起動先が居ない・本文が変わった等。理由はパネルに出る）
- **Issueをまたいで自動では進まない。** 次の手作業はもう一度承認する（承認1回の射程を、押した人が
  画面で見た範囲に留める）
- **最後まで流してもクローズしない。** 完了の確認の画面で止まり、押すのは人

#### 画面を閉じても進む（#1882）

**進めるのはサーバーで、画面は状態を読んで出すだけ。** ブラウザを閉じても・別の端末から開いても、
追っているのは同じ1本の実行になる。

```text
画面「承認してN件を自動実行」
  ↓ POST /api/manual-steps/run（action=start）
ManualStepRun（1 Issueにつき1行）
  ↓ サーバーが先頭の1件を enqueueManualStepJob で積む（#1828と同じ経路）
  ↑ 結果の報告（POST /api/dispatch/report）
  ↓ 成功なら本文のチェックを付けて次の1件を積む／失敗ならそこで止める
  …（最後まで流したら FINISHED。クローズは人が押す）
```

- **動かす契機は2つだけで、常駐プロセスは置かない**（`expireStaleDispatchJobs`と同じ方針）。
  代行実行の結果報告と、画面が状態を読むとき（`listManualStepRunViews`）。後者が要るのは、
  取り消し・タイムアウトで終わったジョブには報告が来ないため
- **進める条件と止める条件は`syncManualStepRun`の1か所**。報告からも画面からも同じ関数を通す
- **自動実行中のチェックはGitHub App名義で付く**（押した人がその場に居ないため）。人が押した
  「実行した・次へ」は従来どおり本人の名義
...
- **止まったままの実行は、Issueがcloseされた時点で片付ける**（#2073・`sweepClosedManualStepRuns`）。
  `PAUSED`のまま居座るとダッシュボードの自動更新が5秒間隔から戻らない。`RUNNING`は対象外で、
  走っているジョブを止めるのは従来どおり「中断する」の仕事
- **中断はいつでもできる**（`action=stop`）。次を積まないだけでなく、走っている1件も止める——
  順番待ち・払い出し済みは取り消し、走り出したものは`MANUAL_STEP_ABORT`で
  `systemctl --user stop`。**止められないホストでも中断そのものは成立し**、走っている1件が
  打ち切り（5分）まで続くことを画面に出す

#### 失敗したらClaudeが原因と修正案を出す（#1869）

失敗した実行について、**原因の説明と修正コマンド案**を出す（`lib/claude/manual-step-fix.ts`・
`POST /api/manual-steps/fix`）。いちばん多い失敗が「本文に書かれたコマンドが実際の環境と
食い違っている」（ユニット名・パスの誤り）で、これは出力を読めば直せる。

```text
失敗（終了コード != 0）
  ↓ POST /api/manual-steps/fix（送るのは**ジョブのidだけ**）
  ↓ サーバーがDispatchJobとIssueキャッシュからコマンド・出力・手順を読み直す
  ↓ Claude API（claude-haiku-4-5）→ {kind, cause, command, advice}
画面に差分（いまの本文 → 修正案）を出す
  ↓ 人が「修正を適用して実行」を押す
  ↓ PATCH /api/issues（**本文のコードブロックだけを差し替える**）
  ↓ POST /api/dispatch（＝#1828と同じ経路。サーバーは書き換わった本文から抽出し直す）
```

- **Claudeが書いたコマンドを、押さずに実行することはない。** 提案は差分で出し、適用するのは人。
  これが「必要に応じてユーザーに承認を求める」の中身で、**本文に元から書かれていた手順は自動で
  流し、Claudeが書いた文字列だけは必ず承認を通す**という線の引き方になっている
- **適用は本文を書き換えてから実行する**（`replaceManualStepCommand`）。実行するのは常に
  本文に書かれたコマンド、という歯止め（[gates.md](gates.md)）をこの経路でも崩さない。
  副産物として、次に同じ手作業を開いた人も直ったものを読む
- **本文へ入るのは提案されたコマンドだけ。** 原因の説明も実行時の出力も入れない（このリポジトリは
  PUBLICで、出力にはシークレットが混ざりうる）。置換はコード側が行い、モデルに本文全体を書かせない
- **書き換え先を一意に特定できなければ適用しない。** 書き換えた本文からコマンドを取り出し直して
  同じ行で読めることまで確かめてからGitHubへ送る
- **出力をClaudeへ送ることには同意を求める**（承認パネルのチェック。既定オン）。外すと自動では
  調べず、失敗の表示に出る「原因を調べる」を押したときだけ送る
- **読めない応答・使えない修正案（空・フェンス入り・長すぎ・元と同じ）は`manual`へ倒す。**
  コマンドを提示できないことより、壊れたコマンドを提示することの方が悪い

#### 想定外だったら「うまくいかない」から相談する（#2299）

**#1869が拾えるのは、代行実行が0以外で終わったときだけ。** けれど手作業の多くは代行できない
（ブラウザでの操作・メインPCやVPSでの作業・プレースホルダを含むコマンド）。人が自分で実行した
手順は終了コードも出力も画面に届かないので、「コマンドの出力が手順書と違う」「外部ツールの画面が
手順書と違う」が起きても、押せるものは「実行した・次へ」と「あとで」しか無かった。**気付いたことは
どこにも残らず、手順書は実態とずれたまま次の人へ渡る。**

手順と`## 完了の確認方法`の画面に「うまくいかない」を置き、押すと**人が状況を書くパネル**が開く
（`manual-step-trouble-panel.tsx`・`lib/manual-step-trouble.ts`）。

```text
「うまくいかない」（代行実行の成否によらず、どの手順からでも押せる）
  ↓ 分類（4つから任意）＋起きたこと（必須）＋出力・画面の文言の貼り付け（任意・同意制）
  ├─ 「原因を調べる」→ POST /api/manual-steps/fix（#1869と同じ口。**jobIdの代わりに行と報告を送る**）
  │     ↓ Claude API → {kind, cause, command, instruction, advice, steps}
  │     ├─ kind: "command" … コマンドの差分 → 適用（本文を書き換えてから実行。#1869のまま）
  │     ├─ kind: "instruction" … **手順の説明文の差分** → 適用（本文の1行だけ書き換え・実行しない）
  │     └─ kind: "retry" / "manual" … **この後にやること**（steps。#2310）＋助言
  └─ 「Issueに記録して次へ」→ POST /api/issues/comments（`<!-- manual-step-trouble:… -->`）
        ↓ 次に同じ手作業を開いた人の最初の画面へ「過去に報告されたつまずき」として出る
```

- **直せる先が2つある。** 外部ツールの画面が変わったときにずれているのは**コマンドではなく
  手順の文言**で、`replaceManualStepCommand`では届かない。`replaceManualStepInstruction`が
  `- [ ]`の後ろの1行だけを差し替える（チェックとリストマーカーは残す）。歯止めは#1869と同じで、
  **書き換えた本文から同じ手順とコマンドを読み直せることまで確かめてから**GitHubへ送る
- **文頭のデバイスの印を落とす直し案は適用しない。** 手順の実行端末は`（ブラウザ）`のような
  括弧書きだけから読み、書かれていない手順は`## 前提条件`の既定値へ落ちる（#2052の
  `resolveManualStepDevice`）。印が消えると、既定値がサブPCのIssueでは**ブラウザ手順が代行実行の
  対象へ反転する**——代行の可否は`ManualStepRunEntry.device`ひとつで決まるため。コマンドを
  引き直せるかの確認ではこの反転を検出できないので、書き換え後の`device`と`checked`が
  変わっていないことを別に確かめる（変わっていたら適用しない）
- **文言を直しても実行はしない。** コマンドの修正案は「適用して実行」まで続けられるが、文言の
  直しは本文が変わるだけ。直った手順を読んでから人が実行する（文言を直したことは、その手順が
  済んだことを意味しない）
- **Claudeへ送る材料は、コマンドが無ければ`command`を選択肢から外す**（`buildManualStepFixPrompt`）。
  実行していない手順について修正コマンドを出させると、そのまま本文へ入る提案になる
- **貼り付けた出力・画面の文言はIssueへ入れない。** このリポジトリはPUBLICで、手作業の出力には
  シークレットが混ざりうる。Issueに残るのは**分類・自由記述・手順の番号**だけ。Claudeへ送るかどうかも
  貼り付け欄のチェックで毎回決める（**既定オフ**。承認パネルの同意が既定オンなのは、押した1回の
  射程の中で起きた失敗に限られるため）
- **`00.check-user`は付けない。** 手作業Issueには承認して再開させる相手が居ない
  （[labels.md](labels.md)）。気付く経路はIssueのコメント欄と、次に開いた人の最初の画面の2つ
- **記録しても手順にチェックは付かない。** 実行できていないまま次を見に行く形にする
  （進捗のうえでは「やっていない手順」のまま残る）
- **確認節で直せるのはコマンドが1つに定まるときだけ**（`onlyVerificationLine`）。複数並んでいる
  ときは原因の説明と記録まで（どれについてのつまずきかを画面で選ばせるほどのものではない）

##### 直せないときは「この後にやること」を出す（#2310）

**手順書を直しても進めない（`manual`）ときに返していたのは、原因と自由記述の助言だけだった。**
助言は「データベーススキーマが正しく初期化されているか確認してください」「必要に応じて
マイグレーションを実行してください」のように書かれがちで、**読んだ人が次に何を押すか・何を打つかが
決まらない**。画面の締めも「手元で対処してから、もう一度実行するか『実行した・次へ』で進めて
ください」で、対処そのものはどこにも書かれていなかった。

`ManualStepFixResult`に`steps`（1件＝説明文1行＋コマンドまたは`null`）を足し、画面の
「この後にやること」へ番号付きで並べる。

- **`manual`・`retry`では必ず1件以上出させる。** `command`・`instruction`のときは、
  画面のボタンで直る分は書かせない（残る手元の作業があるときだけ）
- **原因を特定できなかった場合は、「何を調べれば分かるか」を手順にする。** 状況を集めるコマンドを
  1件目に置き、最後を「その出力を貼り付け欄に貼って、もう一度『原因を調べる』を押す」にする。
  `steps`が空で返ってきた場合も、画面が同じことを促す文へ切り替える
- **`steps`のコマンドは本文へ入らず、issue-deckからも実行しない。** できるのはコピーまで
  （`StepCommand`）。実行できるのは変わらず本文に書かれたコマンドだけ、という歯止め
  （[gates.md](gates.md)）はここでも動かさない。**押せば実行できるボタンを置いた時点で、
  Claudeが書いた文字列を本文を経ずに実行する経路になる**
- **手順は1件ずつ形を見て、使えないものだけ落とす**（`pickSteps`）。直し案と違って本文へ
  書き戻さないので、1件が壊れていても全体を`manual`へ倒す必要はない。コマンドだけが使えない
  （フェンス入り・長すぎ）場合は説明文だけを残す
- **選べない種別の名前はプロンプトに出さない**（`fixableKinds`）。代行実行が無い手順で
  `command`という語を出すと、実行していない手順について修正コマンドを書き始める（#2299の線）。
  **この歯止めを守っているのは`expect(prompt).not.toContain("`command`")`という否定アサーション
  だけ**なので、プロンプトへ節を足すときは同じ条件で出し分ける必要がある。無条件に書くと、
  テストが無ければ気付けないところで#2299の線が消える

#### 完了の確認方法を定期巡回する（#2008）

**openな手作業Issueの`## 完了の確認方法`を、人の操作なしに1日1回実行する。** 全部が終了コード0で
終わったIssueには「完了済みの可能性」の印が付き、Issue一覧の行とIssue詳細の手作業パネルに出る。

これまでの完了判定は「人が実行して『手作業を完了してクローズ』を押す」ことだけに依存していて、
**実行したのに押し忘れると誰も気づけなかった**（#1994は本文の完了条件を満たしているのにopenの
まま残っていた実例）。「ユーザーの作業待ち」にも通知ベルにも並び続け、実行する人はコマンドを
打って初めて「もう終わっていた」と分かる。

```text
GET /api/dispatch（画面が20秒ごとに読む）
  ↓ runManualStepVerificationPatrol（lib/manual-step-verification-patrol.ts）
  ↓ 対象を1件選び、確認コマンドを1つずつ enqueueManualStepJob で積む（#1828と同じ経路）
  ↑ 結果の報告（POST /api/dispatch/report）
  ↓ 0で終わったら次の1件、0以外ならそこで FAILED（＝まだ実施されていない）
  …（全部通ったら PASSED＝「完了済みの可能性」。クローズは人が押す）
```

- **実行の口は増やしていない。** 積むのも照合の二重の歯止めも#1828のまま。巡回が渡すのは
  「どの行のコマンドか」だけで、実行するのはサーバーとサブPCが本文から抽出し直したもの
- **常駐プロセスは置かない**（`expireStaleDispatchJobs`・`ManualStepRun`と同じ方針）。動かす契機は
  画面の読み取りと結果報告の2つで、どちらも`syncCheck`の1か所を通る
- **同時に走らせるのは1件だけ。** 確認コマンドもサブPCの実行キューを通るため、まとめて積むと
  人が押した実行がその後ろに並ぶ。急ぐ理由が無いぶん巡回が譲る
- 記録は`ManualStepVerificationCheck`（1 Issueにつき1行）。**出力はここに持たない**——
  置き場は`DispatchJob.commandOutput`の1か所に留める（上記「出力は画面にだけ出す」）

##### 巡回するのは「読み取りだけだと読める」確認コマンドに限る

人が押す代行実行との違いは、**押す人の目という歯止めが1つ外れる**ことに尽きる。`## 完了の
確認方法`が読み取りだけであることはテンプレート上の期待にすぎず、本文で強制されてはいない。
そこで`isReadOnlyVerificationCommand`（`lib/manual-step-verification.ts`）を通ったものだけを巡回する。

- パイプ・`&&`・改行で割った**各区間の先頭語**をすべて見る（引用符の中では割らない。
  `jq -r '.projects | keys[]' … | grep …`のような書き方が実際にある）
- 許すのは**引数によらず読み取りだけのコマンド**（`cat`・`grep`・`jq`・`test`など）と、
  **サブコマンドまで見て読み取りだと分かるもの**（`systemctl is-active`・`git status`・`gh view`など）
- `>`（`2>&1`は除く）・`$(`・`` ` ``・単独の`&`・閉じていない引用符があれば対象外。
  **静的に読み切れないものは通さない**
- **1つでも読めないコマンドがあればIssueごと対象外。** 確認は全部流して初めて結論が出る
- 実行先の判定は代行実行と同じ（`resolveManualStepRunTarget`）。**VPSの確認コマンドも巡回
  できる**（#2901）が、そこへ到達できるホストが居るときだけ候補にする

**判定は「安全なものを見つける」側で書く**（危険なものを列挙する形にしない）。後者にすると、
知らない書き方が既定で通ってしまう。対象から外れても損は小さい——印が出ないだけで、
その手作業はこれまでどおり人が実行できる。

##### 終了コード0を完了と見なさない（#2011と共通の結論）

**自動でcloseはしない。** 「期待する出力」との照合はしないので、0で終わっても完了とは限らない
（`extractVerificationCommands`の取り決めそのまま）。画面に出すのは「完了済みの可能性」までで、
押すのは人。

これは**#2011（Issueをまたいだ全体実行）とも共通の未決事項**だった。同じ「確認が通った状態」に
対する画面の扱いが両者で食い違わないよう、先に着手したこちらで決めている。#2011でも、確認まで
通ったIssueは同じ印として積み上げ、closeは人が押す。

##### 人が触っているIssueは選ばない

`activeKey`はIssue単位なので、巡回が積んだ1件が残っていると人の実行が`already_queued`で弾かれる。

- 走っている自動実行（`ManualStepRun`）があるIssue・未処理のジョブが積まれているIssueは選ばない
- 選んだ後に人が自動実行を始めたら、**巡回のほうを取りやめる**
  （`abandonManualStepVerificationCheck`。積んだ1件は5分で打ち切られるので取り消さない）
- 同じIssueを次に巡回するのは24時間後。結果は上書きで、履歴は`DispatchJob`側に残る

#### 未処理の代行実行はIssueに1件まで

`activeKey`は`manual_step:owner/repo#番号`で、**手順単位ではなくIssue単位**。順番に実行する
...
scripts/subpc-dispatch-poller.sh → scripts/start-manual-step-session.sh
  ...
  本文の手順を上から順に実行（聞かない）→ 終了コード0なら - [ ] にチェック（gh issue edit）→ 次へ
  ↓ 止まるのは4つだけ（代行できない手順／失敗／本文に無いコマンド／クローズの可否）
...
- **本文に書かれた手順は、起動を押した1回で最後まで流す**（#2830で変更）。#2771では手順ごとに
  「実行しますか？」「次へ進みますか？」と2回ずつ聞いており、5手順のIssueで10回答えることに
  なっていた。**線を引き直したのは「本文に書かれたコマンドかどうか」**で、実行体が組み立てた
  文字列（調査・修正案）は読み取り専用でも全文を`AskUserQuestion`で示して毎回承認を取る——
  ここを外すと、押す1回の射程が「本文から抽出済みのコマンド列」から「実行体が実行する任意の
...
- **止まる条件は4つだけ。** 代行できない手順（サブPC・VPS以外・`<…>`・対話が要るコマンド・
  コマンドが1つに定まらない）／実行の失敗／本文に無いコマンドが要るとき／クローズの可否。
  答えるとそのまま残りが流れる
- **止まったときは、いまの位置も出す**（#2830・`lib/manual-step-question.ts`の`progress`）。
  自動で流すと人が呼ばれるのは自分にしかできない手順だけになるため、質問パネルに
  「手順Nまで自動で実行しました／残りM件（うちあなたが実行 K件）」を添える。**数えているのは
  本文のチェックと残りの手順の代行可否**で、セッションの実況は読まない（質問と実行を結ぶ
  データが無いのは#2820と同じ）
...
- **代行できない手順では、質問パネルにその手順の中身を出す**（#2820）。
  ...
  引用された手順名から本文の手順を当て（`lib/manual-step-question.ts`）、
  ...
  **当てる軸は手順名で、番号は食い違いを弾くのにだけ使う。** プロンプトは同時に
  「未チェックのものだけ進める」とも言っているため、残りを1から数え直されると
  **番号だけが当たって中身は別の手順**になる（番号がN、手順名が別の手順を指していたら出さない）。
  代行できない理由の文言は`describeManualStepExecutionRejection`から取り、
  手作業アシスタントの`ManualStepRunPanel`と同じ文にする
...
#### 参照先は本体チェックアウトではなく`origin/develop`のスナップショット（#1583）

**本体チェックアウト（`~/apps/<repo>`）をそのまま`--add-dir`で渡さない。** 本体チェックアウトを
更新する仕組みはどこにも無く（`start-issue.sh`は「本体の作業ツリーには触れない」ことを設計上の
約束にしている）、**誰も`git pull`しない限り古いまま**になる。2026-08-17の実測では、`git fetch`
し直した後でも shopping-list が29コミット、car-care が24、dayspan が21、issue-deck自身は`main`に
乗ったまま67コミット遅れていた。質問の主用途はコード調査で、回答は「いまのdevelopがどうなって
いるか」を前提に読まれる。**古いコードを根拠に自信を持って答えるのは、いちばん困る壊れ方**
（#1290がStep 4「チェックアウトの鮮度」として挙げていた懸念がそのまま現実になった）。

ランチャーは起動時に各リポジトリを`fetch`し、**リポジトリごとに固定のdetached worktree**を
`origin/develop`（無ければ`origin/main`）へ合わせて、そこを渡す
（[scripts/lib/question-refs.sh](../../scripts/lib/question-refs.sh)）。
...
- **質問ごとには分けず、リポジトリごとに固定する。** `_session-<repo>`（#1529）と同じ理由に加え、
  ...
  `checkout`すら走らない。同時に走る質問セッションどうしのために`flock`で囲む
- **fetchは全リポジトリぶんを並列で行う**（直列だと1件0.5秒でも十数秒かかる）。失敗・タイムアウト・
  `origin/develop`も`origin/main`も無いといった場合は、**そのリポジトリだけ本体チェックアウトへ
  落として続ける**。参照先が古いことより、質問に答えられないことのほうが困る
...
（プロンプトにも、その旨と「聞かれたらスナップショットに無いことを断る」ことを書いてある）。

#### ランチャー自身も同期コピーから実行し直す（#1583）

pollerはsystemd unitの`ExecStart`で本体チェックアウトの`scripts/subpc-dispatch-poller.sh`を起動し、
そこから呼ぶ`start-cross-repo-question.sh`も本体のものになる。#1438の同期コピーで新しくなるのは
**その先の`run-issue-session.sh`と`prompts/`だけ**だったため、**ランチャーの修正はdevelopへ
マージしても人が`git pull`するまで効かなかった**（#1529のcwd固定がサブPCで効いていなかったのは
これが原因）。

そこで`start-cross-repo-question.sh`は、同期コピーが使えるなら**自分自身をそこから実行し直す**
（`ISSUE_DECK_LAUNCHER_REEXEC`で1回だけ）。**対象は横断質問のランチャーに限る。** 実装セッションの
`start-local-session.sh`経路は影響範囲が広いため、これまでどおり本体の作業ツリーから動かす。

あわせて、同期コピーを使うかどうかの判定も直した。`resolve_launcher_scripts_dir`は
「HEADが`origin/develop`の祖先か」で「単に古いだけ」を判定していたが、**本体チェックアウトが
`main`に乗っていると`main`のマージコミットは`develop`に含まれないため必ず外れ、同期コピーが
丸ごと無効化されていた**（サブPCは`main`（v4.0.0）に乗ったまま67コミット遅れており、#1438が
入っているのにセッション側のスクリプトもプロンプトも古いままだった）。判定は
**「HEADがリモート追跡ブランチのどれかに含まれているか」**に変えてある。未コミットの変更があれば
作業ツリーを優先する、という「人が今書いているものを黙って無かったことにしない」約束はそのまま。
...
| | 実装セッション | 横断質問セッション |
|---|---|---|
| worktree | 作る | **質問用には作らない**（cwdは`~/apps/issue-deck-worktrees/.questions/_session-<repo>`の空ディレクトリ） |
...
#### 畳んだ質問セッションは同じ質問の会話へ戻れる（#3033）

実装セッションは畳まれても`--continue`で前回の続きから再開する（#1541）が、**質問セッションでは
`--continue`を使わない**。`claude --continue`が拾うのは「そのcwdで最後に動いた会話」で、質問セッションの
cwdは**質問Issueごとではなくリポジトリごと**に固定されている（#1529）。使うと、同じリポジトリへの
2回目以降の質問が**別の質問Issueの会話の続き**として始まる（#1648で`ISSUE_DECK_CLAUDE_RESUME=0`を
固定して塞いだ）。

そのため畳まれた質問セッションを画面から続けられなかった（「新しい質問として聞き直す」しかなかった）。
#3033で、cwdを分けずに**会話だけを質問Issueごとに特定する**形にした（Codexの`.codex-thread`と同じ手口）。

```text
SessionStart フック（scripts/session-notify.sh）
  ↓ kind=question かつ Claude Code のときだけ、フックのJSONの session_id を控える
~/.local/state/issue-deck/sessions/<リポジトリ名>-issue-<番号>.claude-session   ← tmuxセッション名がキー
  ↓ 畳んだ後、画面の「セッションを復旧」（CROSS_REPO_QUESTION ジョブ）で起動し直す
scripts/run-issue-session.sh
  ↓ 控えたIDと履歴（~/.claude/projects/<cwd>/<id>.jsonl）が揃えば `claude --resume <id>`
```

- **`--continue`は質問セッションでは一切使わない。** 控えが無い・履歴が消えている場合は新しい会話で
  始め、古いIDは消す（安全側。**別の質問の会話へ戻るより、新しい会話の方がまし**）。新しい会話でも、
  起動プロンプトには質問Issueの全コメント（前回の回答を含む）を載せているので、経緯は読める
- **再開時のキックオフ文面は質問セッション専用**。cwdがgitリポジトリではないため`gh issue view`に
  `--repo`（`ISSUE_DECK_REPO_SLUG`）を付け、「作業の続き」ではなく「追い質問（未回答のコメント）への回答」を
  指示する。実装セッションの再開文面（`--repo`なし）をそのまま使うと、質問Issueを引けない
- **控えるのは質問セッションだけ**（記述子の`kind`）。実装セッションはworktreeごとに`--continue`で戻れる
  ので、ファイルを増やす理由が無い。Codexは従来の`.codex-thread`
- `session_state_remove`は`.claude-session`を消さない（畳んだ後にこそ要るため）
- **画面の入口は「終了したセッションの行」**（`SessionRecoveryButton`）。行は24時間残るので、それを過ぎると
  入口が無い（控えたIDはホストに残るが、押す場所が無い）。積むのは`LAUNCH`ではなく質問ジョブで、
  `11.local`は付けない。説明文は「記録が残っていない場合は新しい会話で始まる」まで書く
- 再開後もsessionIdが変わるかどうかは実機依存だが、変わってもフックが起動のたびに最新のIDで上書きする
  ので復旧の妥当性は変わらない
...
呼ぶだけにする（判定を2か所に分けない）。閾値は`DEV_SERVER_IDLE_MINUTES`（既定20・0で無効。#2076で60から短くした）。
...
**同じ穴はセッションの側にもある（#2744）。** 掃除する側だけでなく、**セッションの中で走る
エージェントが打つ`pkill -f`も他セッションに当たる。** 2026-09-02、あるセッションが自分の
開発サーバーを入れ替えようとして`pkill -f "pnpm dev"`を打ち、同じホストで走っていた3セッション
（myroom #302・#343、asset-manager #329）の`claude`がSIGTERMで落ちて終了コード`143`の異常終了
として引き上げられた。当たったのは**キックオフ文面がargvに入っていた**ためで、開発サーバーを
起動せずに立てたセッションの引数には「worktreeで pnpm dev を実行すると起動します」という
一文がそのまま載っていた（3件とも作業自体は終わった後だったため成果は失われていない）。
対策は次の3つで、**どれか1つでは塞がらない**（照合の語はいくらでもあるため）。

- `scripts/run-issue-session.sh`が`--disallowedTools "Bash(pkill:*),Bash(killall:*)"`を
  **常に**渡す（`ISSUE_DECK_DISALLOWED_TOOLS`の指定があれば結合する）。複合コマンド
  （`pkill … || true; …`）でも拒否されることは実測済み。**Codexでは強制できない**ので、
  起動ログに効いていない旨を出す（#2377と同じ扱い）
- `scripts/lib/kickoff-prompt.sh`の開発環境の案内に**実行可能なコマンドの文字列を置かない**。
  起こし方は指示ファイル（`.prompts/issue-<番号>.md`）にあり、そちらはargvに載らない
- 実装エージェントのプロンプト（`scripts/prompts/implementation-agent.md`・
  `generic-implementation-agent.md`）の禁止事項に書く。自分の開発サーバーを止めたいときは
  `lsof -ti :<ポート>`で引いたPIDだけを止める

**ただしこのホストの`lsof -ti :<ポート>`は、待ち受けているプロセスがあっても空を返す**（#2914で実測）。
自分で起動した`next-server`が`127.0.0.1:4915`をLISTENしている状態でも終了コード1・出力なしになり、
「誰も掴んでいない」と読める。**PIDが要るときは`ss -ltnp`で引く。**

```bash
ss -ltnp | grep "127.0.0.1:<ポート>"   # users:(("next-server (v1",pid=3907310,fd=22))
kill <PID>                             # 引けたPIDだけを止める
```

空振りを「掴んでいるプロセスは無い」と解釈して`pkill -f "pnpm dev"`へ逃げると、上の巻き添え事故に
まっすぐ戻る（`pnpm dev`が`EADDRINUSE`で落ち続けるので、止められていないことはすぐ分かる）。

**巻き添えの異常終了は、引き上げの側からは正常な異常終了と区別が付かない。** 送り主は
`pane_dead_status`（`143`）に残らないため、原因を辿るには`journalctl`の同時刻と、
他セッションの転記に残った`Bash`の履歴を突き合わせるしかない。
...
閾値は`SESSION_IDLE_MINUTES`（既定5・0で無効）と`SESSION_HANDOFF_IDLE_MINUTES`（既定5・#1541）、
横断質問セッション専用の`QUESTION_SESSION_IDLE_MINUTES`（既定30・#1648）。**3つとも経路ごとに
独立していて、掛け合わせない**（#1649）。
...
- **猶予は別建て**（`SESSION_HANDOFF_IDLE_MINUTES`・既定5・**0でこの経路だけ無効**）で、
  **`SESSION_IDLE_MINUTES`とは独立**（#1649。この経路ではこちらだけを見る）
...
#### 猶予は経路ごとに独立させ、既定を短くする（#1649）

畳めるのに猶予で残っているセッションは、そのぶん本数の上限（#1361）を埋めて**新規セッションの
待ち時間になる**。#1541・#1600を入れた時点では、引き渡し済みの経路は共通猶予（`SESSION_IDLE_MINUTES`）
を通過したうえでさらに`SESSION_HANDOFF_IDLE_MINUTES`を見るAND判定で、**実効が
`max(SESSION_IDLE_MINUTES, SESSION_HANDOFF_IDLE_MINUTES)`**だった。

- **既定の60/30では、30が一度も効いていなかった**（実効60分）。「引き渡し済みにだけCI失敗の指摘が
  返る余地があるので長めに取る」という段差も、既定値では成立していない（どちらも60分）。
  サブPCが`SESSION_IDLE_MINUTES=5`を設定して初めて30が効き、実効30分になった
- **経路ごとに独立させる。** CLOSED・マージ済みは`SESSION_IDLE_MINUTES`だけ、引き渡し済みは
  `SESSION_HANDOFF_IDLE_MINUTES`だけを見る。横断質問セッション（#1648）がすでに
  `QUESTION_SESSION_IDLE_MINUTES`で共通猶予を置き換える方式だったので、そちらに揃えた形になる。
  早い段階の猶予チェックは**短い方での足切り**として残す（`gh`を叩く前に落とすため）
- **段差そのものも取り下げ、既定を60/30から5/5へ下げた。** 根拠はサブPCのpollerログの実測
  （2026-08-14〜15の1.7日・134件を回収して、**畳んだ後に呼び戻したケースが0件**）。
  畳んでもworktreeは残り、`--continue`で前回の会話の続きから再開できる
- **上限で待つ時間がこれで全部消えるわけではない。** 同じ実測で上限（12/12）に達していた22巡は、
...
稼働開始（2026-08-13）から3日で**181本・38GB**まで溜まり、ルートFSが77%（残り23GB・約12〜13GB/日で
増加）に達したが、**そのうち177本は削除対象になりうる状態**だった。

**これだけは毎巡呼ばない。** 掃除は`git fetch`と`gh pr list`を1回ずつ叩き、worktreeの本数ぶんの
走査を行う（170本で数十秒。#1680の改善後）ため、30秒ごとには重い。
`WORKTREE_CLEANUP_INTERVAL_MINUTES`（既定60分・**0で無効**）の間隔でだけ実際に走り、前回の実行時刻は
`~/.local/state/issue-deck/worktree-cleanup.stamp`のmtimeで持つ。**印は走らせる前に置く**
（`gh`の未認証のように毎回失敗する理由だと、30秒ごとに数十秒の走査を繰り返すことになる）。
固まってもポーリングごと止まらないよう`timeout`（5分）を被せてある。

**systemd timerは新設していない**（開発サーバー・セッションの回収と同じ理由。[関門と計器](gates.md)
「計器」）。間隔をunitに持たせると、変更のたびに編集と`daemon-reload`が要り、他の回収の設定と
置き場所が分かれる。判定はすべてスクリプト側にあり、poller側は「一定間隔で呼ぶ」だけを持つ。
...
`71.manual-step`を付け直すのも同じ巡回で、**全指摘が対応済みになったコードレビューIssueを閉じる**
のも同じ巡回（#3216。[code-review.md](code-review.md)「全指摘が対応済みなら自動でcloseする」）。
ログに出すのは進めた・通知した・ラベルを付けた・閉じたときだけで、
...
戻すか・完全に削除するかの判定もissue-deck側が持つ。**他の巡回より間隔が長いのは、初回の
バックログでリポジトリあたり最大1,000件のIssueコメントを読むため**（読み切った後は前回以降に
更新されたコメントしか返らないので1リポジトリ1リクエストに落ち着く）。ログに出すのは
ファイルを動かしたときだけで、`--dry-run`では呼ばない（ファイルの移動・削除という取り消しの
効かない副作用があるため）。設計は[../code-map.md](../code-map.md)
「『使っていない画像』は参照の索引で判定する」。
...
置いている。サブPC（13.9GB）では#1361の時点の実測で1セッション約390MB（2026-08-22の再実測では
約0.5GB）。

> **「開発サーバーが最大3本」という当初の見積もりは実態と違っていた**（#2076）。開発サーバーは
> **セッションごとに1本ずつ**起動され（`run-issue-session.sh`・`ISSUE_DECK_DEV_SERVER`）、
> アイドル回収まで残る。本数を抑えているのは`DISPATCH_MAX_SESSIONS`ではなく
> `DEV_SERVER_IDLE_MINUTES`のほうである。セッション本体・開発サーバー・テストとビルドの
> 実測値は[立った後のセッションが走らせるものには上限が無い](#立った後のセッションが走らせるものには上限が無い2076)を参照。
...
- **当初は使用率で何も止めなかった。** 人が判断するための計器（[gates.md](gates.md)）として
  出すだけで、起動を止めているのは`maxSessions`と同時実行数だけ、という線を引いていた。
  **これは#2095で引き直した**（次節）。同じ数字をpoller側が見て起動ジョブを見送る

**スマホのホームに戻したカードは#1933で縮めた**（`DispatchHostPanel`の`compact`）。#1690で
ホームへ戻した時点のカードは使用率4行＋動いているセッション数行で縦242pxあり、それだけで
メニューの1行目を画面の外へ押し出していた。ホームは開く場所ではなく常に目に入る場所なので、
**ホームは使用率だけのサマリ（CPU・メモリ・SWAP・ディスクを横並び、実数は落とす）、
ヘッダーの実行状況シートは全部（実数・動いているセッション・キュー）**という切り分けにする。

- **カード全体が実行状況シートを開くボタン**（`onOpenDetail`）。ヘッダー右上のアイコンと同じ
  シートが開くだけで、中身は1つのまま。開閉の状態は`mobile-home-screen.tsx`が持ち、
  `MobileDispatchStatusButton`へ`open`/`onOpenChange`で渡す（渡されなければボタンが自分で持つ）
- **セッションの行にあった導線（Issue詳細・Remote Control・開発サーバー）はシート側に残る。**
  ホームからは1タップ増えるが、同じ一覧を2か所に持たない
- **入力待ちのセッションの本数だけは、縮めた側の見出しにも残す**（`countWaitingSessions`）。
  一覧ごと落とすと「シートを開くべきとき」がホームから分からなくなる——「セッション 6/12」は
  本数でしかなく、「いまの状況」の「実行中」はIssueの進捗（`navCounts["in-progress"]`）で
  セッションの活動状態とは連動しない。**#1638で「ホームから消えて分からなくなった」のが
  再発しないのは、消えるのではなく1タップ先へ移るのと、この印が残るため**
- **スクリプトの版（#1612）は遅れているときだけ残す。** 常に出すと縮めた意味が無い一方、
  遅れは「developへマージしたのに効いていない」ことに気付く唯一の手掛かり。
  「更新して再起動」（#1875・#1927）は押した結果を出す先ごとシート側に置く——カード全体が
  ボタンなので中にボタンを重ねられない（`compact`では`jobs`・`onRequestSelfUpdate`を渡さない）
- **`compact`を使うのはスマホのホームだけ。** PCの実行キューは押して開く場所で、縦に伸びても
  読む手間にならないため従来のまま
...
#### メモリ・SWAPが逼迫している間は起動を見送る（#2095）

`DISPATCH_MAX_SESSIONS`が見ているのは**本数だけで、実際の空きメモリではない**。12本に届いて
いなくても重い作業（テスト・ビルド）が重なっていれば、そこへさらにセッションを足してしまう。
#2076で重いコマンドの同時本数は機体全体で2本まで絞ったが、あちらは**走り出した後**の話で、
こちらは**入口で実際の余力を見る**という別の対処。片方だけでは残る。

pollerは1巡の入口で集めた`metrics`を見て、メモリ・SWAPの使用率が閾値以上なら
その巡の起動ジョブを`maxJobs: 0`で見送る（本数の上限に達したときとまったく同じ形で、
**停止・追加指示などの制御ジョブは従来どおり取りに行く**）。判定は
[`scripts/lib/launch-hold.sh`](../../scripts/lib/launch-hold.sh)の`resolve_launch_hold`が持つ。

- **閾値は`dispatch.env`の設定値**（`DISPATCH_MEMORY_HOLD_PERCENT`・既定85、
  `DISPATCH_SWAP_HOLD_PERCENT`・既定50。どちらも**0で無効**）。コードに埋め込まないのは
  `DISPATCH_MAX_SESSIONS`と同じ理由で、ホストの搭載メモリで決まる値だから（#1176）
- **メモリの既定85%は、画面の使用率が赤くなる境目**（`host-metrics.ts`の`CRITICAL_PERCENT`）
  に合わせてある。見た目の警告と実際の見送りを同じ線に置くためで、別の値にすると
  「赤いのに起動する」「赤くないのに止まっている」が起きる。**片方を変えるときは両方見る**
- **判定に使うのは申告する数字そのもの。** 画面に出ている使用率と、見送りの理由に出る使用率が
  同じ巡の同じ値になる。別々に測ると「画面は80%なのに90%で止まっている」という説明の
  付かない状態ができる
- **判定はclaimの直前ではなく`announce`の中で行う**（`resolve_launch_hold`の呼び出し位置）。
  **`DispatchHost`へ書けるのは`POST /api/dispatch/hosts`＝`announce`だけ**なので、claimの直前で
  決めると、その理由を画面へ渡す口がその巡には無い。**画面へ出す必要があるpoller側の判定は、
  申告と同じ場所で決める**（新しい絞りを足すときも同じ）。1巡の入口で決めた結果を使う
  ぶん、直後の回収（`reap_sessions`）で空いたメモリは次の巡まで反映されないが、ずれは最大で
  ポーリング間隔（既定30秒）——`liveSessions`の申告と同じ性質の遅れ
- **どちらも超えていれば理由はメモリ。** SWAPが増えるのはメモリが足りなくなった結果なので、
  原因の側を出した方が読み手の次の行動（何を畳むか）に繋がる
- **SWAPを持たないホスト（総量0）ではSWAPを見ない。** 割合を出せず、0%として扱うと
  「常に空いている」と読める（画面の`describeSwapRow`と同じ向き）
- **使用率を取れなかった巡は見送らない。** 余力が分からないことを止める理由にすると、`/proc`が
  読めないだけで起動が永久に止まる。**この判定が壊れたときに倒れる向きは「起動する」側**
- **取りに行かなくてもジョブは消えない**（本数の上限と同じ。`queued`は期限切れの対象外）

**見送っていることは画面に出す。** 出さないと「順番待ちのまま進まない」としか見えず、pollerが
落ちている状態と区別が付かない（#1394とまったく同じ問題）。pollerは申告に`launchHold`
（`{reason, percent, thresholdPercent}`）を載せ、`DispatchHost`の3列へ写す。

| 場所 | 出すもの |
|---|---|
| 実行キューのポップオーバー・スマホのホームのホストのカード | `メモリ 92%（上限 85%）のため、新しいセッションの起動を見送っています` |
| 実行キューの順番待ちが進まない理由 | 同じ内容（`describeDispatchQueueStall`） |
| Issue詳細のジョブの状態 | 押した本人が見ている場所に同じ理由（`describeDispatchJobWaitReason`） |

- **issue-deck側は閾値を持たず、判定し直さない**（`maxSessions`とまったく同じ立場）。
  受け取った結果が入っているかどうかを見るだけ。2か所に閾値を持つと必ずずれる
- **3列はまとめて入るかまとめて`null`か。** 部分的に採用すると「理由は分かるが割合が0%」という、
  読んでも判断できない表示になる
- **毎巡上書きし、前回の値を残さない。** 残すと、余力が戻って起動を再開した後も
  「見送っています」が出続ける
- **本数の上限にも達しているホストでは、本数の方を理由に出す。** どちらも「取りに行かない」と
  いう同じ結果だが、待っている人にできること（セッションを畳む／重い作業が終わるのを待つ）が
  違い、畳む方が具体的
- **応答していないホストでは出さない**（使用率のメーターと同じ）。落ちているホストは
  「余力が無くて待っている」のではなく「取りに来られない」
...
（#1176）に従い、アプリ設定ダイアログから変更できる。CPUの載せ替えで適正値が変わるため。

既定の3は**Ryzen 5 PRO 4650G（6C/12T・L3 8MiB）での実測**（guchi-apps/subpc#19・#1812）にもとづく。
「worktreeごとに`pnpm dev`を常駐させ、全数で同時に`next build`を回す」という最も重い条件で測った。

| 並行数 | 同時ビルド時のピーク | swap | ビルド所要 | CPU合計 |
| --- | --- | --- | --- | --- |
| 2本 | 5.55GiB | 0 | 28.9秒 | 629% |
| 3本 | 8.36GiB | 0 | 39.0秒 | 753% |
| 4本 | 10.61GiB | **3.0GiB** | **85.6秒** | 465% |

**上限は3本。決めているのはCPUではなくメモリ**（13Gi・載せ替えでは変わっていない）。12スレッド
（1200%）に対しCPUは3本でも頭打ちしておらず、4本で使用率が下がるのはswap待ちの空転による。
4本でもOOM Killerは出ず全て成功はするが、所要時間が2.2倍に伸びる。

> **既定値を2から3へ上げたのが#1812。** 旧既定の2は載せ替え前（Athlon 200GE 2C/4T）の実測
> （#1177）で、`next build`単体が4スレッド中2.6を使い切る**CPU律速**を避けるための値だった。
> 12スレッドになってその律速が消えたため、実測の上限と既定値が一致した。
>
> **`DISPATCH_MAX_JOBS`（既定1）は据え置き**（#1812）。1巡で1本しか取りに行かないため、まとめて
> 投入すると3本目が立つのはポーリング間隔（既定30秒）の2巡後になる。ただし起動そのもの
> （worktree作成・依存インストール）は同期的に数分かかり、1巡で2本取っても2本目はその後に
> 直列で走るだけなので、短くなるのはポーリング間隔ぶんに留まる。一方で取得中は申告・回収・
> 停止ジョブの受け取りが待たされるため、割に合わないと判断した
> （[deploy/subpc/dispatch.env.example](../../deploy/subpc/dispatch.env.example)）。

ホスト側が`maxConcurrency`を申告している場合は**小さい方**を採る。

### 立った後のセッションが走らせるものには上限が無い（#2076）

**上の3本は「ジョブの払い出し」にしか効かない。** ジョブはtmuxが立った時点で`succeeded`になり、
そこから先——12本（`DISPATCH_MAX_SESSIONS`）のセッションが**それぞれ好きなときに走らせる
`pnpm test:unit`・`pnpm build`**——は誰も数えていなかった。#1812が測ったのは「3本の同時ビルド」で、
その3本が守られるのは**起動が重なったとき**だけである。

2026-08-22のサブPC（12スレッド／13.9GiB／SWAP 4GiBのファイル）での実測。

| 項目 | 実測 |
| --- | --- |
| `claude`本体 1本 | 約0.5GiB（12本で約6.1GiB） |
| `pnpm test:unit` 1本 | ワーカー11個・ピーク**3.24GiB**・70秒（12スレッドを使い切る） |
| 同上（ワーカー6個＝現在の既定） | ピーク**2.10GiB** |
| 同上（ワーカー4個） | ピーク1.20GiB・165秒 |
| `next dev` 1本 | 起動直後0.4GiB → コンパイル後**1.6GiB** |

セッション本体だけで6GiBを使っているところへテストが2本重なるとSWAPへ落ちる。実際、
`earlyoom`（サブPCに常駐）が2026-08-16以降7回発火し、**すべて`next-server`をkillしている**。
#1361のような「ホストごと固まる」事態はearlyoomが防いでいるが、代わりに**開発サーバーが黙って
消える**（画面確認をしようとしたエージェントには理由が見えない）。

そこで**機体全体で重いコマンドを同時2本まで**に絞る
（[scripts/heavy-command.sh](../../scripts/heavy-command.sh)）。`package.json`の`test:unit`・`build`が
このスクリプトを経由し、`flock`で枠を取ってから走る。あわせてvitestのワーカーを1本あたり6個に
制限し（[vitest.config.ts](../../vitest.config.ts)）、12スレッドを6×2本へ割り当てている。

- **枠の置き場は`${XDG_CACHE_HOME:-~/.cache}/heavy-command`**（リポジトリの外）。制約はリポジトリ
  ではなく**機体**にあるため、他リポジトリのセッションが同じ仕組みを入れれば枠を共有できる
- **待たせきりにしない。** 待ちが`HEAVY_COMMAND_TIMEOUT_SECONDS`（既定15分）を超えたら警告を出して
  そのまま実行する。絞るのは山を削るためで、作業を止めるためではない
- **待機中は標準エラー出力へ理由を出す。** 出さないと固まったように見え、エージェントが待ちを
  異常とみなして中断する
- **`pnpm dev`は通さない。** 動かし続けるものなので枠を返さない。開発サーバーの本数は
  `DEV_SERVER_IDLE_MINUTES`（既定20分・#2076で60分から短縮）のアイドル回収が抑える
- **`build:ci`は通していない。** CIは1ジョブ1VMで競合しないため、通しても待ちは発生せず、
  CIの挙動を変えないほうが読み違えが少ない
...
### 枠外のジョブだけは軽い巡回で取りに行く（#2413）

`~/.config/issue-deck/dispatch.env`の`DISPATCH_FAST_POLL_INTERVAL_SECONDS`（既定3秒・**0で無効**）。

**1巡は「間隔」より重い。** `run_once`は回収（開発サーバー・セッション・worktree）・申告・
巡回検知（コンフリクト・デプロイ失敗・進捗・本番マージ待ち）を通ってから`claim`へ着くため、
起動ジョブが無い巡でも実処理だけで実測約14秒かかる（起動ジョブがある巡はさらに長い。後述）。ここに効いていたのが**手作業アシスタントの代行実行**で、
[承認1回で最後まで流す](#承認1回で最後まで流す1869)は**1手順ごとにジョブを積み直す**
（前の1件の報告が届いてから次を積む）。1手順あたり平均22秒待つので、手順が5つある手作業Issueでは
実行そのものより待ち時間の方が長くなっていた。

`DISPATCH_POLL_INTERVAL_SECONDS`を短くする形は採らない。**重い処理まで一緒に速くなる**ためで、
回収も巡回検知も30秒より短い周期で回す理由が無い。代わりに、重い巡回の**待ち時間をこの間隔で
...
- **`fast: true`を添えて、相乗りしている定期処理を省いてもらう。** `claim`は確認待ちの
  Push通知の巡回（#838）を「30秒に1回来る唯一の定期経路」として相乗りさせており、そのまま
  10倍回すと前提が崩れる。**払い出しそのものは何も変えない**
- **ジョブが無いときも失敗したときも何も出さない。** 3秒ごとに1行出すとjournalが10倍に膨らみ、
  重い巡回の記録が読めなくなる。失敗理由は同じ宛先を叩いている重い巡回が出す
- **重い巡回が走っているあいだは取りに行けない**ままなので、最悪の待ちはそこに残る。
  実測約14秒というのは**起動ジョブが無い巡の値**で、`run_once`は`launch_and_report`の
  `timeout "$LAUNCH_TIMEOUT" …`（既定15分）を前景で待つため、**セッションの起動と重なった巡は
  分単位で延びる**。worktreeの掃除（`WORKTREE_CLEANUP_TIMEOUT_SECONDS`・既定300秒）が
  当たった巡も同じ。したがって速くなるのは「起動が走っていないあいだ」で、そこでの待ちが
  22秒前後→3秒前後になる。並行して走らせると同じジョブを二重に掴む経路ができるため、
  そこまでは踏み込んでいない
- 画面の文言もこれに合わせて「数秒〜30秒（セッションの起動中はさらに待つことがあります）」へ
  直した（`manual-step-run-panel.tsx`・`manual-step-run.ts`・`host-checkout.ts`）。
  **秒数を申告に載せる形は採っていない。** `announce`はポーリング間隔を送っておらず、
  載せるには`DispatchHost`の列を増やすことになる——DBスキーマの変更に見合う正確さではない
- **上の間隔と同じか長い値は無効に倒す**（刻まないのと同じ意味のため）
- 新旧どちらの組み合わせでも壊れない。古いissue-deckは`fast`を未知のキーとして無視するだけ、
  古いpollerはこの巡回を持たないので従来どおり動く
...
### エージェントは「実装を開始」ダイアログでだけ選ぶ（#2505）

サブPCを実行先に選ぶと、Claude CodeとCodex CLIのどちらで起こすかを選べる（`DispatchJob.agent`。
既定は`claude`）。**出すのは`codexCapable`を申告しているホストのときだけ**で、選んだ時点で
Codexで効かなくなる連携（通知・計画の承認・Remote Control）を同じ場所に出す。詳しくは
[codex.md](codex.md)。

**ここ（「サブPCで開始」ボタン・「セッションを復旧」）には足していない。** どちらもホストごとの
メニュー項目で、エージェントを掛け合わせると項目数が倍になる。**同じ選択を2か所に持つより、
選ぶ場所を1つに決める方が、押した結果が読める。**
...
- **オプション（`21.plan-required`等）を選ぶのはこのダイアログだけ**（#1580）。以前は作成
  ...
  「計画を立案」の初期値は作成時に付けた種別ラベル（`50.feature`等）から決まるので、
...
### 実行先が確定するまで選択肢を出さない（#1666）

`GET /api/dispatch`が返るまで`useDispatchState`の`hosts`は空で、**「まだ取得していない」と
「申告しているサブPCが1台も無い」を受け取る側から区別できなかった**。そのため実装開始の
ダイアログは、サブPC抜きの選択欄（既定がGitHub Actionsへ落ちた状態）を先に描いてから、
届いた時点でサブPCを足して既定も移していた。実行先で出るオプションまで変わる
（撮影⇄アーティファクト）ので、押そうとしていたものが指の下で別物に入れ替わる。

- フックに**`isLoaded`（最初の取得が終わったか）**を持たせ、ダイアログは`false`の間
  実行先もオプションも出さずスケルトンだけを出す（「開始する」も押させない）。
  骨組みの見出しと枚数は確定後と同じにして、確定した瞬間に高さが変わらないようにする
- **取得に失敗しても`isLoaded`は立てる。** 20秒間隔で取り直すが、届かない間ずっと待たせるより、
  従来どおり（サブPCの選択肢が出ないだけ）で操作できる方が害が小さい。
  ポーリングの2回目以降で`false`へ戻さない（戻すと20秒ごとに選択肢が消える）
- Issue詳細の起動ボタンの文言（「サブPCで開始」／「GitHub Actionsで開始」）も同じ理由で、
  確定するまでは実行先を名乗らず「実装を開始」と出す
...
**API側（`enqueueDispatchJob`）の判定を省かない。** 「次にやること」の自動開始
（`issue-order-dialog.tsx`）は判定をAPI側へ委ねているため、画面だけに置くとそちらが素通りする。
...
### 押した直後に画面が知っているのは`11.local`だけ（#1815）

積んだジョブ・立ったセッションは**pull型で遅れて届く**が、`11.local`は積むより先に付けている
（`ensureLocalLabel`・「実装を開始」ダイアログ・サブPC側の`start-issue.sh`のいずれも同じ）。
そのため**押した直後にIssueへ付いている唯一の合図がこのラベル**になる。

- **開始の主導線（塗りつぶしのボタン）は`11.local`が付いた時点で引っ込める**
  （`isIssueExecutionStarted`）。実体（ジョブ・セッション）だけを見ていた頃は、押した後も
  押す前とまったく同じ「サブPCで開始」が残り、効かなかったようにしか見えなかった
  （Issueを作成して続けて起動した場合、詳細画面はジョブを積んだダイアログとは別の取得口を
  持つため、次のポーリングまで最大20秒何も変わらない）
- **起動そのものは塞がない。** ラベルはセッションが外すまで残るので、これで積めなくすると
  落ちたセッションを立て直せなくなる。立て直しの判定は従来どおり実体だけを見る
  （`isIssueExecutionPending`）ため、`11.local`だけが付いた状態では
  枠線の「サブPCで開始」（`StartLocalSessionButton`）が唯一の導線として残る
- **`11.local`が付いている間はGitHub Actionsの導線も出さない。** 無人実行はこのラベルで
  skipし「対応しません」とコメントするだけなので（`reusable-issue-dispatch.yml`）、
  出しても押した人に何も返らない
- ボタンを消したぶんは`IssueStatusCard`が「ローカルで対応中」の1行で埋める。
  消して空白になるのは、それはそれで「押しても何も起きなかった」に見える
- **積む・取り消す・送るは、同じ画面の他の`useDispatchState`にも取り直させる**
...
判定は`isActionsRunInProgress`（`completed`以外は進行中）。塞ぐのは積む導線2つ。
...
### バッジの数字はサブPCで生きているセッション本数（#2265）

**元は積まれているジョブの件数（実行中＋順番待ち＝`activeCount`）だった。** ジョブは
tmuxセッションが立った時点で`succeeded`になるため、**10本走っていても数字は0〜1にしかならず**、
サブPCがどれだけ埋まっているかはポップオーバーを開くまで分からなかった。ホストが申告する
`liveSessions`（`*-issue-<番号>`のtmuxセッション数）へ切り替え、上限（`DISPATCH_MAX_SESSIONS`・
既定12）まで数字が伸びるようにした（`countDispatchQueueBadge`）。

- **応答している（`online`）ホストの合計だけを出す**（`describeDispatchQueueStall`と同じ絞り方）。
  ホストの行は`listDispatchState`が`findMany`で全件返して消えないため、絞らないとpollerが
  止まったホストの`liveSessions`が最後の値のまま残り、**バッジが古い数字（例: 12）で固まる**
- **本数を申告するホストが1台も無ければ（古いpollerだけ・pollerが止まっている間）、従来どおり
  ジョブの件数を出す。** 判定材料が無いことを理由にバッジを消すと、ジョブを積んだこと自体が
  画面から消える
- **見るべき失敗が残っていれば、その数字を赤くする（`bg-destructive`）。** 数字と失敗の印を
  排他にしない——#1519は「動いていない かつ 失敗がある」ときだけ赤いドットを出す作りだったが、
  数字がセッション本数になると走っている間はずっと数字側が勝ち、**失敗が閉じたボタンから
  読めなくなる**。色で出す形は通知ベルのバッジ（`NotificationBadge`）と同じ規則
- **何も動いていないのに失敗が残っているときは、従来どおり赤いドット。**
- **セッションが0本でジョブだけ積まれているときは、数字ではなく点（primary色）。** 数字の意味を
  「セッション本数」に固定するため。この状態は起動待ちの数十秒か、pollerが取りに来ていないかの
  どちらかで、件数より「開いて理由を読め」という合図の方が合う（理由は
  `describeDispatchQueueStall`が出す）
- `title`（スマホはシートの説明）の先頭に`セッション 10/12`を出し、押さずに数字の意味を
  確かめられるようにする。続く`実行中 n/m`はジョブと同時実行数の話で、**セッションが立った
  時点で数から外れる**ため別の項として並べる

### 閉じているボタンでも失敗に気づける（#1519）

件数バッジは動いているときにしか出ないため、**失敗だけが残っている状態はボタンが無印**で、
ポップオーバーを開くまで気づけなかった。動いていない かつ 失敗があるときは、件数の代わりに
赤いドットを出す。件数ではなくドットなのは、押して確かめてほしいのが「何件あるか」ではなく
「何が失敗したか」のため（件数は`title`に入れる）。

**数字が出ている間は、その数字を赤くする**（#2265）。数字がセッション本数になり、走っている
あいだは常に数字側が出るようになったため、ドットと排他のままでは失敗が読めなくなる（上の節）。
...
> **#2772で「時刻が来たら起動する」仕組み自体は入れた**（かつての「夜間実行」。時計で決まる
> 窓で、次枠実行と同じ表・同じ画面を使っていたが#3019で機能ごと削除した）。#1541の時点で
> 断った理由は「VPSにcronやワーカーを増やす」ことで、その後に入った巡回の作法（#2294。
> **pollerが30秒ごとに叩き、時刻・間隔の判定はサーバー側の純関数が持つ**）ではその前提が
> 消えている。**同時実行数を時間帯で切り替える**ことは引き続き持たず、次枠実行も
> `dispatchConcurrency`にそのまま従う。

### 次枠実行（#2995）

「次の5時間枠」に積んだIssueを、**いまのClaudeプラン枠の残りが設定ぶん（既定60分）を切ってから**
サブPCへ1件ずつ起動する。起動後はいつもの経路（PR作成→自動レビュー→developへ自動マージ）で
「本番反映待ち」まで進み、結果は左メニュー「予約実行」（スマホはホームのメニュー）で結果を5つ
（本番反映待ち／確認が必要／実行中／止まった／見送り）に分けて見る。

```text
「実装を開始」→ 実行先「次の5時間枠」   POST /api/nightly-run（NightlyRunEntry・QUEUED）
...
- **予定は`DispatchJob`と別の表（`NightlyRunEntry`）に持つ。** ジョブは積んだ瞬間から払い出しの
  対象で、予定は窓が開くまで払い出されてはいけない。窓が開いた時点で`enqueueDispatchJob`を
  **通して**変換する（積めない組み合わせの判定を2か所に持たない）。`activeKey`（未処理の間だけ
  `owner/repo#番号`）で同じIssueの二重投入を防ぐ
...
- **積んだ後に手動で実装開始されたIssueの予定は、枠を待たずに自動で取り消す**（#3274）。
  以前は`11.local`が付いていても枠が開くまで予定として画面に残り、起動する時点でようやく
  「見送り」に落ちていた。claimの巡回が、次枠実行の起動より先に`cancelManuallyStartedScheduledRuns`
  （`nightly-run-launch.ts`）を回し、**DBに同期済みの`11.local`と、積んだ後に作られた生きている
  起動ジョブ**（判定は純関数`decideManualStartCancel`）のどちらかがあれば`CANCELED`＋`skipReason`にする。
  「実装を開始」もターミナル・VS Codeからのローカル着手も、必ず`11.local`を付けるので同じ判定で拾える
  （個々の開始APIへフックを足す形にしない）。**`nightKey`が入った予定は対象外**——起動処理が席を
  取った後で、自分で作るジョブ・`11.local`を手動着手と取り違えないため。次枠実行がOFFでも回す。
  画面は`QUEUED`・`LAUNCHED`・`SKIPPED`しか描かないので、`CANCELED`にした予定は一覧から消える
...
- **残りが下限を下回っている間は起動を見送る**（#3100）。5時間枠・週間枠のそれぞれに「起動しない
  残り枠の下限」（0＝制限しない〔既定〕・10〜50%）を持たせ、**残り＜下限の間は待つ**。日々の作業用に
  取っておく枠を、無人で走る予約実行が食い切らないための歯止め。判定は
  `resolveNextWindowRunQuotaBlock`（純関数）で、`decideNextWindowRunLaunch`が`wait`にする
  - 使用量は**すでに取得している応答**（`anthropic-ratelimit-unified-7d-*`が5時間枠のヘッダと同じ
    応答に載る）から読むので、**探りの送信は増えない**（`toClaudeWindowReading`）
  - **5時間枠は`open`（終わり際）のときだけ見る。** `idle`のスナップショットは終わった前の枠の
    使用率か、取得の拍子に開いたばかりの0%で、これから起動する枠の残りではない。**週間枠は
    リセット時刻を過ぎていれば見ない**（前の週の値）。週間枠の使用率が取れないときも見ない
    （取れない日があっても、下限を設けない場合と同じ動きにとどめる）
  - **5時間枠の下限は「いまの枠を多く使っている＝人が作業中」とみなして起こさないための目安**
    （起動するのは枠の終わり際で、残りはどのみち捨てられるため、次の枠を直接守るものではない）。
    週間枠の下限は、週の残りを取っておくためのもの
  - **見送りではなく待つだけ**で、予定は残り、残りが下限を上回れば次の巡回で再開する。ただし**24時間の期限は
    延びない**（起動できる状態に戻った直後に期限切れで見送るのを避けるため、待ちの長さで数え直す
    仕組みも持たない）。週間枠のリセットまで数日あるときは期限切れで「見送り」になり、その理由は
    サブPCの不調と区別して「枠の残りが下限を下回っていた」と出す（積み直しは人が行う）
  - 画面は「予約実行」の枠メーターに週間枠を足し、下限を斜線で示す。止まっているときは見出しに
    「起動を見送り中」と出し、理由（どの枠が残り何%で下限何%）を1行で出す
- **承認・回答を代わりに押す経路は持たない**（[gates.md](gates.md)）。「計画を立案」のIssueは
  計画の投稿で止まり、人が承認する
- **人が居ないと進まないオプションは積ませない**（計画レビューG1の指摘）。`23.preview-required`・
  `25.artifact-required`が付いているIssueは、積む時点（ダイアログ・API）と起動する時点（窓が開くまでに
  付いたもの）の両方で弾く。判定は**Issueの実ラベル**で、ダイアログのチェックの状態ではない
  （`applyOptionLabels`は足すだけで、外しても残るため）。`22.merge-confirm-required`（自分の目で
  通す札）は積める
- **確認待ちPushは止めない。** 枠のリセット時刻は時計と無関係で、昼に起動することもあり、
  伏せる理由が無い
- **GitHubへの読み書きは積んだ人のトークン**（`withUserGithubToken`）。ラベルの付与を人の操作として
  残すためで、インストールトークンにすると`issue-deck[bot]`が着手したように見える。切れていれば
  リフレッシュトークンで延長して続け、延長にも失敗したときだけ「ログインし直して」の理由で見送る
  （#3148。以前は401が延長に届かず、夜の枠の起動が一律「認証が切れている可能性」で見送られていた）
- **既定はOFF。** 無人で実装が進む処理なので、画像の自動削除と同じく人が納得してから有効にする。
  設定（有効／無効・残り時間・間隔）は「予約実行」画面で**切り替えた時点で保存**し、
...
- **積まれていることはIssue一覧・Issue詳細でも分かる**（#2866）。積んでも**ラベル・ジョブ・
  ...
  並んでいた。一覧の行は進捗バーの左に「次枠 07:40〜」のようなチップ、Issue詳細は「実装を開始」と
  同じヘッダーの中に注釈（起動する見込み・「予約実行を見る」・「予定を取り消す」）を出す。
  **目印を出すのは`QUEUED`の予定だけ**で、起動した後は従来どおり進捗バー・セッションの表示が
  受け持つ（[code-map.md](../code-map.md)「同じ状態を2か所で言わせない」）。次枠実行がOFFのときは
  積んであっても走らないので、チップの字を「次枠実行OFF」に変え、色も確認待ちと同じamberにする
  ——人が設定を戻すまで進まない状態で、この行のamberの意味（人の対応待ち）と一致する。
  材料は左メニューの件数と同じ`useNightlyRun`の結果（`selectScheduledRunQueuedMarks`）で、
...
| 場所 | 役割 |
|---|---|
| `src/lib/nightly-run.ts`（＋test） | 結果5分類・画面の目印（#2866）の共有純関数。ファイル名は歴史的なもの（#2772で夜間実行として追加し、#3019で夜間実行専用部分だけ削除した） |
...
| `src/lib/nightly-run-db.ts` | `Issue.id`とは別の軽量な鍵（`nightlyRunIssueKey`）だけを持つ |
| `src/lib/next-window-run-db.ts` | 次枠実行の設定・5時間枠の取得・直前の起動時刻（#2995） |
| `src/lib/nightly-run-launch.ts` | 予定をジョブへ変換する共有コア（`launchScheduledRunEntry`等）。claimからは`next-window-run-launch.ts`経由で呼ぶ |
...
上のコマンドを人が思い出して打たない限り気付けない、という状態は#1612で解消した。pollerは
申告（`POST /api/dispatch/hosts`）に**自分が動かしているチェックアウトの版**を載せ、実行キューと
スマホのホーム画面のホストの行に出る。

```
● サブPC                        セッション 3/12
  スクリプト develop fbb809d     97コミット遅れ・2日前
```
...
### チェックアウトが最新でも、走っているコードは古いことがある（#2815）

**`git pull`はpollerの中身を入れ替えない。** bashは実行中のスクリプトのファイルを開いたまま持ち、
gitは新しいファイルを作ってrenameするため、**プロセスは起動時のコードのまま動き続ける**
（sourceした`scripts/lib/*.sh`も同じで、読み込み済みの関数定義が残る）。

上の`behindCount`はチェックアウトのHEADと`origin/develop`しか比べていないので、
**チェックアウトが追い付いた瞬間に「最新」へ戻り、古いコードが走っていることは画面から消える。**
`SELF_UPDATE`（次節の「更新して再起動」）はpullと入れ替えを1組で行うので食い違わないが、
手元での`git pull`や他のセッションによる更新ではこの状態になる。

2026-09-02 15:13から2026-09-04まで、サブPCのpollerが`6e4a38e`のまま走り、チェックアウトだけが
`6f54ae50`へ進んでいた。#2779（AI使用量の「実装」をフェーズ4分割する）の集計側
（`scripts/lib/session-usage.sh`の`researchCostUsd`ほか）が一度も動かず、本番には表示側だけが
入っていたため、直近のセッションが全部「実装（フェーズ未集計）」で並んだ
（[guchi-apps/question#61](https://github.com/guchi-apps/question/issues/61)の調査で判明）。
しかも「一度だけの埋め戻し」は再起動後に初めて走るため、放置していた期間ぶんは保持期間の中で
埋まらないまま流れていく。

そこでpollerは**起動した時点のコミット**（`startedCommit`）も申告する。

```
● サブPC                        セッション 3/12
  スクリプト develop 6f54ae5     再起動待ち・起動時 6e4a38e で動作中・2日前
```

- 判定は`isDispatchHostPollerRestartPending`（[src/lib/dispatch/host-checkout.ts](../../src/lib/dispatch/host-checkout.ts)）。
  **短縮SHAの桁数は`git rev-parse --short`が状況に応じて決める**ため、片方がもう片方の前置なら
  同じコミットとして扱う
- **遅れ0でも橙にする。** 縮めた版のカード（スマホのホーム）が出す条件は`tone !== "normal"`なので、
  ここを`normal`のままにすると、いちばん気付いてほしい状態がホームから消える
- 遅れてもいれば1行にまとめる（`5コミット遅れ・再起動待ち`）。押すボタンは「更新して再起動」の
  1つで、`SELF_UPDATE`は追い付いていても`exec`で入れ替わるため、**どちらの状態もこれ1回で解ける**
- journaldにも出る（`申告しました: subpc（セッション 3/12・スクリプト develop 6f54ae5（再起動待ち（起動時 6e4a38e で動作中））)`）
- **`startedCommit`を申告しないpoller（#2815より前）では`null`で、これまでどおりの表示になる。**
  検知が効き始めるのは「更新して再起動」を1回押した後（＝新しいpollerが上がった後）
...
**この作業は手作業Issueにしない**（#2009）。ボタンが入った後も「pullしてpollerを再起動する」だけの
`71.manual-step` Issueが立ち続けたため、起票する側の基準に「画面から実行できる操作は起票しない」を
書いた（[labels.md](labels.md#起票しない条件2009)）。案内するときはIssueではなく、PR本文の
「注意点」か起点Issueのコメントに「マージ後に画面から更新して再起動する」と書く。

### 画面からホストごと再起動する（#2496）

サブPCのカーネル更新は`/var/run/reboot-required`が立ったまま適用されず、2026-08-29の調査時点で
9日・稼働13日だった（[guchi-apps/question#52](https://github.com/guchi-apps/question/issues/52)・
`guchi-apps/subpc#68`）。落とすには毎回sshして`tmux ls`でセッションの有無を数える手作業が要り、
`README`が`sudo reboot`を「自動化しない残り」に置いていたぶん、誰も落とさないまま差が開いていた。

数える材料（セッション本数・応答・使用率）は**pollerの申告として既に届いている**ので、確認と
ボタンを実行キューのサブPCのカードへ置く。押すと`REBOOT`のジョブが積まれ、受け取ったpollerが
`sudo -n /usr/sbin/reboot`を打つ。

- **「更新して再起動」（#1875）とは別のジョブにする。** あちらが畳むのはpollerのプロセスだけで、
  `exec`で入れ替わるため走っているセッションは残る（#1927）。こちらは**OSごと落ちる**ので
  セッションは全部消え、会話も戻らない。同じ経路に相乗りさせると、pollerが「更新のつもりで
  ホストを落とす」ことになる
- **出すのは「再起動が要る」ときだけ**（`/var/run/reboot-required`があるとき）。落とす理由が
  無いときに取り返しのつかないボタンを常設しない——[question#52](https://github.com/guchi-apps/question/issues/52)の
  「毎週無駄に落とさない」という結論と同じ向き
- **押せるのはセッションが0本のときだけ。** 本数を申告していないホストでも押させない
  （「たぶん0本」で落とす判断はしない）。押せないときは**理由をボタンの下に必ず出す**——
  押せる条件がチェックアウトの遅れではないぶん、理由が無いと何を直せばよいのか分からない
- **確認を1枚挟む。** 押し間違えても戻せない（GUIが無いので、上がってこなければ物理コンソールが
  要る）。確認はカードの中へその場で出し、どのホストを落とすのかを読み直させない
- ジョブは`activeKey`（`reboot:host:<ホスト名>`）で1件に絞る

**置き場所は設定画面ではなく実行キューのカード。** 当初の計画は設定→「フリート運用」だったが、
設定画面はホストの状態を一切持っておらず、セッション本数・応答・押せない理由を出すには
`useDispatchState`の取得口をもう1本増やすことになる（#2022でこの区分は「開いただけで取得が
走る」のをやめている）。判断に要る計器が既に揃っている場所へ寄せた。

#### セッションが0本であることは、issue-deckとpollerが独立に2回確かめる

issue-deckが見ている本数は**最大30秒古い申告**で、押してから届くまでの間に新しいセッションが
立ちうる。pollerは受け取った時点で`tmux ls`を数え直し、0本でなければ実行せず失敗として返す
（[手作業の代行実行](#実行できるのは本文に書かれたコマンドだけ照合を2回する)が本文の照合を
2回行うのと同じ作法で、**取り返しのつかない操作の最終的な判定は実機側に置く**）。

計画レビューの1本目・コードレビューのセッションも数える。あちらはセッション名を`-issue-`の
規約から外してあるぶん`DISPATCH_MAX_SESSIONS`の計上に入らないが、落とせば同じように消える。

#### 落とす前に入口を閉じる

`REBOOT`が積まれている間、issue-deckはそのホストへ**起動ジョブを払い出さない**
（`claimDispatchJobs`）。閉じないと、押してから届くまでの数十秒に新しいセッションが立ち、
pollerの側の確かめ直しに引っかかって再起動そのものが失敗する（押した人からは「押したのに
落ちない」に見える）。止めるのは起動ジョブだけで、制御ジョブ（`C-c`・セッションを閉じる）は
むしろ再起動へ近づける操作なので塞がない。

**報告した後の穴はpoller側で塞ぐ。** `sudo reboot`はシャットダウンを予約して即座に戻るため、
成功を報告した時点でissue-deck側の見送りは解ける。pollerはその後`sleep`に入って次のジョブを
取りに行かないので、pull型である以上そこから起動が始まることはない。落ちきった後は
「誰も取りに来ない」状態そのものが入口を閉じ続ける。

#### サブPC側にsudoの許可が要る

pollerは`sudo -n -l`の一覧から**`NOPASSWD:`が付いた行だけ**を取り出し、そこに
`/usr/sbin/reboot`があるときだけ`reboot`を申告する。**許可が入るまで申告は偽のままで、画面にも
ボタンが出ない。** 配ってから`failed`で返す形にすると、押した人には「押したのに落ちなかった」
しか残らない。

##### `sudo -n -l <コマンド>` は「パスワード無しで通るか」の判定に使えない（#2496）

最初はこちらで判定していたが、**`sudo -l <コマンド>`が見るのは「そのコマンドが許可されて
いるか」だけ**で、`(ALL : ALL) ALL`のような**パスワードを要する許可でも成功する**。サブPCの実機では

```
$ sudo -n -l /usr/sbin/reboot   # → /usr/sbin/reboot（rc=0）
$ sudo -n true                  # → sudo: a password is required（rc=1）
```

となり、そのままでは「対応している」と申告しながら実行時に必ず失敗する——この申告が防ごうと
していることそのものになっていた。`sudo -n -k -l`（タイムスタンプを捨ててから引く）でも同じで、
`-l`はNOPASSWDかどうかを返さない。

代わりに`sudo -n -l`の出力を読む。**パイプへ流す限りsudoは行を折り返さない**ので、
`NOPASSWD:`を含む行を`grep -F`で絞ってからコマンドの文字列を突き合わせれば足りる。
`NOPASSWD: ALL`のような書き方では文字列が現れず`false`になるが、**取りこぼす側は
ボタンが出ないだけで害が無い**（誤って`true`にする側が困る）。

`sudo -n true`で「そもそもパスワード無しのsudoがあるか」を見る手もあるが、あれはNOPASSWDの
範囲を問わないため、別のコマンドにだけ許可がある状態で真になってしまう。

許可そのものは`/etc/sudoers.d/`にあり`guchi-apps/subpc`の管理下なので、issue-deck側からは
足せない（[labels.md](labels.md)「実機の設定ファイル変更は、管理リポジトリのIssueへ切り出す」）。
`sudo -n -l`が突き合わせるのは**許可された表記そのもの**なので、sudoersに書く形と
`REBOOT_COMMAND`（`/usr/sbin/reboot`）を1文字も違えない。`/sbin/reboot`のような別名では一致しない。
...
**ジョブの結果は、画面へ報告した`message`と同じ文字列がjournalにも出る**（#1228）。出すのは
`report_job`の入口1か所だけで、分岐ごとにログを書いていない（同じ理由を2か所に持つとずれる。
かつては起動前に早期returnする経路——不正な値・cloneが無い・同じIssueのセッションが既にある——が
報告だけで手元に何も出さず、`ジョブ <id>: <repo> #<番号>`の1行から`tmux ls`と突き合わせるまで
起動できたのか見送ったのか分からなかった。#1224で実際に起きている）。**ストリームは報告した
状態で決まる**（`failed`は標準エラー、見送り・成功・実行中は標準出力）ため、画面の色と
ログの出方が一致する。基本は1行で、起動失敗だけはランチャー出力の末尾がそのまま続く。

**代行実行（`MANUAL_STEP`）の結果はこの例外で、journalには出ない。** pollerが`report_job`で
報告するのは`running`までで、成否は別ユニットの`scripts/run-manual-step.sh`が自分で報告する。
コマンドの出力にはシークレットが混ざりうるため、行き先はログイン必須の画面だけに限る取り決めで
（上記「手作業の代行実行」）、**「報告と同じものをログにも」をあちらへ広げてはいけない**。
