...
**いつ選ぶか**（#1317・#2775）。**issue-deckでは`merge-policy: relaxed`にしたため、変更の種類では
...
**他リポジトリ（`merge-policy: strict`）では従来のまま**で、上のカテゴリはラベルが無くても機械的
判定が止める。そちらで選ぶ意味があるのは、どのカテゴリにも当てはまらないのに自分の目で通したい
変更に限られる。

- ラベル `22.merge-confirm-required` の有無で分岐する。
  - **ラベルなし（デフォルト）**: 従来どおり、`risk-check`（機械的判定）・`claude-review`
    （意味的判定）の結果のみで自動マージ可否を判断する。issue-deckでは`merge-policy: relaxed`の
    ため、機械的判定は変更カテゴリではマージを止めない（#2775）。
...
### 変更カテゴリで止めるかは`merge-policy`で切り替える（#2775）

上のカテゴリのうち`develop`→`main`を除く7つは、**共有ワークフローの`merge-policy`入力で「マージを止める理由」として扱うかどうかを切り替えられる。**

| 値 | 挙動 | 使っているリポジトリ |
| --- | --- | --- |
| `strict` | 既定。カテゴリに該当したら`00.check-user`＋`01.check-merge`を付け、自動マージしない | issue-deck以外のすべて |
| `relaxed` | カテゴリでは止めず、そのまま自動マージへ進む | issue-deck |

issue-deckを`relaxed`にしたのは、**`develop`はリリース前の統合先**で、本番へ出るには`develop`→`main`のリリースPR（自動マージ不可カテゴリのまま・人がマージする）をもう1回通るため。develop向けPRの側で毎回「確認待ち」の札を積んでも、押すのは同じ人・同じ変更に対する2回目の確認にしかならなかった。
