...
| マージ前に確認が必要 | `22.merge-confirm-required` | developへのマージ前 | 出来上がった差分を自分で読んでから入れたい（issue-deckでは変更の種類では止まらないため、DB・Actions・認証の変更もここで指定する。#2775） |
...
- **「作る前に方向を合わせる」ものと「出来上がったものを確認する」ものに割れる。** 前者が「計画が必要」（作り方）と「アーティファクトで見た目を出す」（見た目。#1540）、後者が残り2つ。バグ修正のように直す場所が決まっているIssueで計画を挟んでも、承認待ちの往復が増えるだけで判断は変わらない。
- **「開発環境を起動する」は、サブPC実行・ローカル実行が対象。** `tailscale serve`で開発サーバーそのものをtailnetへ出せる（#1265）ため、外出先のスマホからでも実物を触れる。
- **「アーティファクトで見た目を出す」はローカル実行専用**（#1473）。アーティファクトの公開はローカルセッションのツールで無人実行からは作れないため、実行先にGitHub Actionsを選んでいるときだけ隠す。**止まるのは実装着手前**で、`23`とは時間軸が違うため択一ではない（#1540）。`25`が効くのは**実データが無くて実画面が空になる場合**・複数案を見比べたい場合・作り始める前に見た目を固めたい場合で、**「計画が必要」と併用すると計画と見た目を1回のやり取りで承認できる**。
- **「マージ前に確認が必要」は、自分の目で通したい変更を止めるためのもの。** `merge-policy`の既定が`relaxed`のため、**変更の種類（認証・認可、DBスキーマ・マイグレーション、本番環境の設定、GitHub Actions・デプロイ設定、Secrets・環境変数、課金・決済、大規模な依存更新）では自動マージが止まらない**（#2775・#2790。「変更カテゴリで止めるかは`merge-policy`で切り替える」参照）。それらをマージ前に読んでおきたいときは、このラベルを明示的に付ける。ほかに、影響範囲が読みにくいリファクタリング、共通コンポーネント・共通関数の書き換え、Issueの文面だけでは仕様が決まりきっていない実装、他リポジトリへ配る雛形の変更も出番。一度きりの`00.check-user`の手動付与と違い、**PRが何度pushされても毎回止まる**のが実務上の違い。**この緩和はcallerを持つ全リポジトリに効く**ので、どのリポジトリでも同じ基準で選んでよい。
- **チェックを外すと、既に付いている実ラベルも「実装を開始」を押した時点で外れる**（#2884）。選んだオプションどおりにラベルが付き、その内容で実行されるようにするための対称な動作で、`applyOptionLabels`（`start-implementation-dialog.tsx`）が追加・除去の両方を同じPATCHでまとめて反映する。**種別ラベル由来の既定（「計画が必要」「アーティファクトで見た目を出す」）でチェックが入っただけ（実ラベルは未付与）の状態でOFFにしても、何も削除されない**——削除対象は「現に付いている実ラベル」だけのため。夜間実行の判定（`nightlyRejection`）も、この除去予定を先取りして計算する（外したのに`23`/`25`が付いたままの扱いで積めなくならないように）。
