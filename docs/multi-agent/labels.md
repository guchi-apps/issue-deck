...
- **チェックを外すと、既に付いている実ラベルも「実装を開始」を押した時点で外れる**（#2884）。選んだオプションどおりにラベルが付き、その内容で実行されるようにするための対称な動作で、`applyOptionLabels`（`start-implementation-dialog.tsx`）が追加・除去の両方を同じPATCHでまとめて反映する。**種別ラベル由来の既定（「計画が必要」「アーティファクトで見た目を出す」）でチェックが入っただけ（実ラベルは未付与）の状態でOFFにしても、何も削除されない**——削除対象は「現に付いている実ラベル」だけのため。予約実行の判定（`nightlyRejection`）も、この除去予定を先取りして計算する（外したのに`23`/`25`が付いたままの扱いで積めなくならないように）。
