...
| 「実行するデバイス」に端末が1つだけか | `warning` | デバイス欄はそのままチップに出る（`cleanWhereValue`は末尾の括弧を1つ落とすだけ） |
...
| `## 前提条件` の「実行するデバイス」「カレントディレクトリ」「Gitブランチ」 | **全ステップに出したままのチップ**。末尾の括弧書き（補足）は落とす |
...
手順にコマンドが書かれていて、**`## 前提条件`の「実行するデバイス」がサブPC**なら、
...
- サブPC以外（VPS・1Password・GitHub App・ブラウザ）の手作業では、ボタンの代わりに理由が出る。
  **起票の書き方は変えなくてよい**（代行できる手順が増えるだけで、できない手作業はこれまでどおり）
...
- 巡回するのは**`## 前提条件`の「実行するデバイス」がサブPC**で、**確認コマンドが読み取り
