...
残すのは3通りとも同じで、変わるのはラベルと「誰に知らせるか」だけ。**
...
| 引き継ぎ | `11.local`があり、セッションが終了・記録なし | `11.local`も外してから投稿し、無人実行が拾える状態にする |
...
- **セッションへ流すのは固定の1行**（`PR_FIX_SESSION_INSTRUCTION`）で、人が書いた依頼はコメントの
  ...
  **送る全文は押す前に欄の中へ出す**
- **セッションの記録は24時間で消えるため、「終了した」と「記録が無い」は見分けられない。**
  どちらも引き継ぎ側へ倒し、文面だけ言い分ける（`sessionEnded`）
- **送れない理由・送信の失敗は必ず出す。** 前者は押す前に出してボタンを押せなくし
  （`resolveSessionControlRejection`をそのまま使う）、後者は押した後に出してボタンは押せるまま
  にする。**コメントは投稿できているので、ここで黙ると「コメントは増えたのにセッションは知らない」
