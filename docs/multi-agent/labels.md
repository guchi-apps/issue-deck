...
進捗（Project Status）の状態遷移、各種の要否トグル（計画フェーズ・プレビュー・スクリーンショット・マージ前確認）、サブIssue分割、質問セッションからのIssue起票、自動マージ可否の判定、ユーザーの手作業のIssue化。
...
| `30.`〜`70.`・`80.`/`89.` | `30.bug`・`51.improvement`・`70.confirm`・`80.Priority: High` | ○ | 種別・優先度。**本文の内容から決まる**ので推定に向く |
