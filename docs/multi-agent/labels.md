...
ラベルの番号帯の設計、進捗（Project Status）の状態遷移、各種の要否トグル（計画フェーズ・プレビュー・マージ前確認）、サブIssue分割、質問セッションからのIssue起票、自動マージ可否の判定、ユーザーの手作業のIssue化。
...
### 一覧のカードにはGitHubラベルを出さない（#3159。未着手ビューの種類ラベルだけ例外）

**Issue一覧のカードは、GitHubのラベルを描かない**（`src/components/dashboard/issue-list.tsx`）。
**例外は「未着手」ビュー（`view === "not-started"`）の行だけで、種類が分かるラベル（30〜69番台。
判定は`src/lib/issue-kind-labels.ts`の`isKindLabel`）を下段の左に1行で出す**（#3285）。未着手は
着手を選ぶための一覧で、どんな種類のIssueかが行から読めないと選びにくいため。折り返さず、収まらない
ぶんは横スクロールにする（ラベルの多い行だけ背が伸びると一覧が不揃いになる）。スクロールバーは出さない
（行の高さが変わる）。行の当たり判定の上で動かすため、この行だけ`pointer-events-auto`にして
押しは全面ボタン（`data-row-hit`）へ渡す（選択モードなど行の押し方が変わる場面でも同じ動きになる）。**ほかのビューへ広げる場合は、この節と`showKindLabels`の条件を一緒に直す。**
以下は例外を除いた、従来どおりの扱い。
以前は`00.`・`01.check-*`（要対応とその理由。`WorkflowStepBadge`が代弁する）と実装オプション
（#1915。走らせ方で、盤面を眺める手掛かりにならない）だけを除いて出していたが、分類・優先度・
`11.local`の色付きタグが並ぶと、一覧の主役であるリポジトリ・進捗・タイトル・操作ボタンより
目立つため、すべて外した。付いているものをすべて見る・編集するのは**Issue詳細とプロパティ**の役割で、
そちらは従来どおり。ラベルによる絞り込み・確認待ちの判定はデータ側なので影響を受けない。
...
**新しい意味を持つラベルの帯を足しても、一覧には出ない**（未着手ビューの種類ラベルも30〜69番台の固定）。一覧で読み取れるようにしたい場合は、
ラベルを描き戻さず、`WorkflowStepBadge`やエージェントバッジのような専用の表示を足す。なお
`80.Priority: High`・`22.merge-confirm-required`・`23.preview-required`は、他の表示に置き換えが無いため
**一覧からは読み取れない**（#3159でそれを承知のうえ決めた）。カード下段の右端は、引き続き
Remote Controlを開くボタンの場所（#1915）。
...
| PRを作成し、自動マージされなかった | `reusable-claude-review-develop.yml`の`auto-merge`が、`risk-check`（機械的判定・`22.merge-confirm-required`・`23.preview-required`）と`claude-review`（意味的判定）の結果をまとめて付与（後述「自動マージ可否の判定方法」。**判定したジョブ自身は付けない**。#1406・#2136） | 同じ（PR作成後はレビュー・統合側の担当） |
...
| ユーザーが開発環境を確認して承認／修正を依頼した | 上と同じ承認・修正ボタン。ローカルセッションでは`AskUserQuestion`に答えた時点で`PostToolUse`フックが外す |
...
### マージ時に外しそこねた`00.check-user`は巡回が外す（#2335）

上の表の「ユーザーがPRをマージした」で外れるのは、`00.check-user`と理由ラベル
（`01.check-*`・旧名`00.qa-answered`）の**7枚をまとめた1回の`gh issue edit`**。
**そこには再試行が無く、失敗しても警告を出して先へ進む**（進捗の報告を最優先に守る設計。#1861）。

guchi-apps/signaly#200で実際に起きた形。

1. `claude-review-develop.yml`が「Secretsや環境変数の変更」を理由に自動マージを見送り、
   11:27:25に`00.check-user`＋`01.check-merge`を付けた
2. 11:28:28、ユーザーがPR #201をdevelopへマージした
3. `develop-pr-merged`が動き、進捗は`Develop`へ進み、マージ完了のコメントも投稿された。
   しかしラベルを外す`gh issue edit`だけが`502 Bad Gateway`で落ちた（実行ログ 32842492174）
4. 進捗は`Develop`まで進み終えているので、取り残しの巡回（`Develop PR`・`Implementation`が
   対象）にも掛からない

**残り続けるわけではない。** 同じ7枚は`main-pr-merged`とIssueのcloseでも外れるので、
次のmainリリースまでで解消する（signaly#200は11:46:23のリリースPR #203のマージで外れた。
取り残しは18分）。それでも直すのは、**その窓のあいだ盤面の「確認待ち」に押せない札が
積まれる**ため——開いてもマージ済みで、できる操作が無い。リリースまで日をまたげば
Push通知（#838）も飛ぶ。

対策は2つで、**どちらか一方では埋まらない**。

- **ワークフロー側で3回まで再試行する**（`develop-pr-merged`・`main-pr-merged`・
  `main-direct-merged`・`cleanup-on-close`）。一時的な5xxはこれでほぼ吸収できる。
  ただし恒久的な失敗（権限・ラベル未定義）は拾えず、`@workflows/vN`の参照タグを
  配り終えるまでの間は古いままのリポジトリに効かない
- **issue-deck側の進捗巡回（5分ごと）が最後の受け皿になる。** 判定は
...
候補は「`00.check-user`が付いていること」だけで拾い、理由ラベルの除去は`clearCheckUser`の
既存の挙動（付いているものを外す）に任せる。

そのうえで、次のどれかに当たるものには触らない。

- **開いているPRが1件でもある。** それは本物のマージ待ちで、`01.check-merge`が
  指しているものそのもの
- **`issue-<番号>`のマージ済みPRが無い。** マージの記録が無いのに外すと、人が手で付けた
  確認待ちまで黙って消してしまう
- **`00.check-user`が付いたのが最後のマージより後。** 判定が下る前にPRがマージされた場合、
  `reusable-claude-review-develop.yml`は文面だけを事後確認向けに変えて
  `00.check-user`＋`01.check-merge`を**そのまま付ける**（#1968）。取り残しの通知
  （`01.check-blocked`）も人が手で付けたものも同じ形になる。付与の時刻
  （`Issue.checkUserLabeledAt`。ラベルが外れるとnullへ戻る）が最後のマージより前のものだけを
  相手にする。**時刻が分からない（null）ものにも触らない**

baseは`develop`に絞らない——`develop`を持たないリポジトリでは`issue-<番号>`→`main`が
唯一のPRで、そちらにも`main-pr-in-progress`が`01.check-merge`を付ける。

#### GitHub APIを消費しないための絞り込み

候補をDBから引く時点で、**`Develop`・`Release`にいるopenなIssue**に絞る。ここに入るのは
「進捗はマージ後まで進んだのに確認待ちが残っている」ものだけで、平常時は0件になるため、
対象が無ければGitHubへのリクエストは1回も出ない。

- **進捗の報告も一緒に落ちた場合**は`Develop PR`・`Implementation`に留まるが、それは
  取り残しの巡回（`sweepIssue`）が`Develop`へ進めるときに`clearCheckUser`を呼ぶので拾える
- **`Done`はIssueがcloseされる**ので`state: OPEN`の時点で外れ、`cleanup-on-close`と
...
#### マージ待ちの「修正を依頼する」は送り先を切り替える（#2919）

承認欄から「承認」「修正」を外した後も、**マージ待ちの修正依頼欄だけは同じ穴が残っていた。**
あそこが投稿するのも`@claude …`のコメント（`requestPrFixCommentBody`）で、`11.local`が付いて
いれば`reusable-issue-dispatch.yml`は`mode=skip`へ倒し、「`11.local`を外してから、改めて
`@claude`とコメントしてください」という案内を1件返して終わる。**押した人から見ると、修正依頼を
書いて送ったのにコメントが2件増えただけ**になる。

そこで送り先を先に決め（`resolvePrFixRequestRoute`・`src/lib/dispatch/pr-fix-request.ts`）、
**案内とボタンの文言を送り先に合わせる**（`PrFixRouteNotice`）。**#3333以降、修正依頼を送れるのは
PR詳細（`PullRequestFixIssueBar`→`PullRequestFixSessionDialog`）だけ**で、Issue詳細のマージ待ち欄
からは送らない（PRへの操作をPR詳細へ一本化した）。**依頼をIssueコメントとして
残すのは4通りとも同じで、変わるのはラベルと「誰に知らせるか」だけ。**

| 送り先 | いつ | 押すと起きること |
| --- | --- | --- |
| 無人実行 | `11.local`が無い | 今までどおり。`@claude …`のコメントが`claude-issue-dispatch.yml`を起こす |
| セッション | `11.local`があり、セッションが`ALIVE` | コメントを投稿したうえで、そのセッションへ`INSTRUCTION`ジョブで固定の1行を流す |
| 再開 | `11.local`があり、セッションの記録はあるが`ALIVE`でない | コメントを投稿したうえで、既存の「セッションを復旧」（#1830）と同じ起動ジョブで呼び戻す |
| 引き継ぎ | `11.local`があり、セッションの記録も無い | `11.local`も外してから投稿し、無人実行が拾える状態にする |

- **判定材料は`11.local`ラベルそのもの**で、`resolveIssueExecutionTarget`の`expectsActionsRun`は
  使わない。あちらはセッション・ジョブの記録（24時間残る）も見るため、**引き継いだ直後の
  Issueでも`false`**になる——そこは今まさにActionsが拾う状態で、引き継ぎをもう一度やらせる意味が無い
- **終了したセッションは、札を外す前に呼び戻す。** Issue本文の要求が「クローズしていたら再度
  立ち上げる」で、呼び戻す導線は#1830で既にある（`describeSessionRecovery`）。worktreeを消して
...
- **セッションへ流すのは固定の1行**（`PR_FIX_SESSION_INSTRUCTION`）で、人が書いた依頼はコメントの
  ...
  **送る全文は押す前に欄の中へ出す**
- **サブPCへ積む2つの送り先では、`00.check-user`を押した時点で外さない**（#2886で受けたのと同じ
  指摘）。`INSTRUCTION`の送出は非同期で、承認プロンプトの表示中・作業中・入力欄に打ちかけがある
  場合はpollerが見送る。積んだ時点で外すと**何も届いていないのに札だけ消える**。外れるのは
...
- **送れない理由・送信の失敗は必ず出す。** 前者は押す前に出してボタンを押せなくし（判定は送り先
  ごとに既存のものを流用——セッションは`resolveSessionControlRejection`、再開は
  `resolveDispatchTargetRejection`）、後者は押した後に出してボタンは押せるままにする。
  **コメントは投稿できているので、ここで黙ると「コメントは増えたのにセッションは知らない」
  が残る**——このIssueが直そうとした症状そのものになる
...
| `01.check-input` | 質問・確認に答える | 待機 | 質問した／開発環境を提示した |
...
- 一次判定の理由コメントは、そのpush開始時点で既に`00.check-user`が付いていると投稿されない
  （#594）。その場合は`22.merge-confirm-required`・`23.preview-required`から理由を組み立て、
  どれも無ければ「理由の記録が見つかりませんでした」と出す。**理由を推測で埋めない。**
...
- 置き場所は2か所で、**PC・スマホ共通**（どちらも同じコンポーネントを通る）。
  - Issue詳細の上部（`IssueStatusCard`）… 開いた直後に見える。**行き先ボタンが承認欄
    （コメント欄末尾）かRemote Controlを指すときだけ出す**（#2924）。行き先がこのカードの
    すぐ下にある本体パネル（計画パネル・質問パネル・対応PRセクション）を指すときは、その
    本体パネルが既に視界に入っているため出さない（`showApprovalBadge`のバッジだけになる）
...
- **行き先ボタンは1つだけ出す。** 判定は次の順で、`01.check-merge`の「なぜ」（上記#1631）は
  ...
  - 理由が`merge` … 「対応PRへ移動」。**コメント欄の承認カードから見ても目的地は上部の対応PR**
    （#2914）。そこの各行からPR詳細を開き、マージ・修正依頼はPR詳細で行う（#3333）。対応PRのセクションが描かれていないIssueだけは承認欄へ送る
    ...
    その状態では画面のどこにもPR詳細への導線が無い（`IssuePullRequestList`は行が0件なら
...
## 画面のオプション4つの使い分け（#1317）

issue-deckの「実装を開始」ダイアログには、下記4つのラベルをアイコン付きのチップとして出している（定義は`src/lib/github/start-implementation.ts`の`START_IMPLEMENTATION_OPTIONS`）。どれも**エージェントを止める場所が違うだけ**で、迷ったら「どこで自分が見たいか」で選ぶ。

**ダイアログは上が実行先、下がオプション**（#1623）。実行先はアイコン中心のタイルを**枚数に関わらず1行に**並べ（`grid-flow-col`。5枚がスマホでも折り返さない。#3046）、オプションは2列のチップにしている。**オプションの説明はグリッドの下に出さない**（縦に伸びて開始ボタンまで届かないため。#3046）。説明はチップの`title`に残し、予約実行（次の5時間枠）で選べないオプションの理由だけ下に1行出す。表示順は`START_IMPLEMENTATION_OPTIONS`の並びで、実装着手前に承認をもらう「計画を立案」「デザインを提示」が1行目に並ぶ。

**選ぶのは実装を始めるときだけで、「新しいIssueを作成」ダイアログには出さない**（#1580）。4つのうちアーティファクトはローカル実行専用で、**実行先が決まって初めて意味が決まる**ものが混ざっている。起票の時点では実行先も実施時期も未定なため、起票のたびに4項目を読ませても選びようがない。作成画面の「作成+実装開始」からも、続けて開く「実装を開始」ダイアログで選ぶ（`src/components/dashboard/create-issue-dialog.tsx`）。ラベル選択欄からも除外している（`isSelectableLabelName`）ので、作成時にこの4つが付くことはない。

| 選択肢 | ラベル | 止まる場所 | 選ぶ基準 |
|---|---|---|---|
| 計画を立案 | `21.plan-required` | 実装前 | 作り方に選択の余地がある。**新機能・改善・デザインの種別では既定でON**（後述） |
| マージ前に確認 | `22.merge-confirm-required` | developへのマージ前 | 出来上がった差分を自分で読んでから入れたい（issue-deckでは変更の種類では止まらないため、DB・Actions・認証の変更もここで指定する。#2775） |
| 開発環境を起動 | `23.preview-required` | PR作成前 | 動いている画面を触って確かめたい |
| デザインを提示 | `25.artifact-required` | **実装着手前** | **ローカル実行で**、作り始める前に見た目を決めたい（#1473・#1540）。**デザインの種別では既定でON**（後述） |

- **「作る前に方向を合わせる」ものと「出来上がったものを確認する」ものに割れる。** 前者が「計画を立案」（作り方）と「デザインを提示」（見た目。#1540）、後者が残り2つ。バグ修正のように直す場所が決まっているIssueで計画を挟んでも、承認待ちの往復が増えるだけで判断は変わらない。
- **「開発環境を起動」は、サブPC実行・ローカル実行が対象。** `tailscale serve`で開発サーバーそのものをtailnetへ出せる（#1265）ため、外出先のスマホからでも実物を触れる。
- **「デザインを提示」はローカル実行専用**（#1473）。アーティファクトの公開はローカルセッションのツールで無人実行からは作れないため、実行先にGitHub Actionsを選んでいるときだけ隠す。**止まるのは実装着手前**で、`23`とは時間軸が違うため択一ではない（#1540）。`25`が効くのは**実データが無くて実画面が空になる場合**・複数案を見比べたい場合・作り始める前に見た目を固めたい場合で、**「計画を立案」と併用すると計画と見た目を1回のやり取りで承認できる**。
- **「マージ前に確認」は、自分の目で通したい変更を止めるためのもの。** `merge-policy`の既定が`relaxed`のため、**変更の種類（認証・認可、DBスキーマ・マイグレーション、本番環境の設定、GitHub Actions・デプロイ設定、Secrets・環境変数、課金・決済、大規模な依存更新）では自動マージが止まらない**（#2775・#2790。「変更カテゴリで止めるかは`merge-policy`で切り替える」参照）。それらをマージ前に読んでおきたいときは、このラベルを明示的に付ける。ほかに、影響範囲が読みにくいリファクタリング、共通コンポーネント・共通関数の書き換え、Issueの文面だけでは仕様が決まりきっていない実装、他リポジトリへ配る雛形の変更も出番。一度きりの`00.check-user`の手動付与と違い、**PRが何度pushされても毎回止まる**のが実務上の違い。**この緩和はcallerを持つ全リポジトリに効く**ので、どのリポジトリでも同じ基準で選んでよい。
- **チェックを外すと、既に付いている実ラベルも「実装を開始」を押した時点で外れる**（#2884）。選んだオプションどおりにラベルが付き、その内容で実行されるようにするための対称な動作で、`applyOptionLabels`（`start-implementation-dialog.tsx`）が追加・除去の両方を同じPATCHでまとめて反映する。**種別ラベル由来の既定（「計画を立案」「デザインを提示」）でチェックが入っただけ（実ラベルは未付与）の状態でOFFにしても、何も削除されない**——削除対象は「現に付いている実ラベル」だけのため。予約実行の判定（`nightlyRejection`）も、この除去予定を先取りして計算する（外したのに`23`/`25`が付いたままの扱いで積めなくならないように）。
...
- **チップの初期値はIssueの種別ラベルから決める**（#1317・`planRequiredDefaultForLabels`）。`50.feature`・`51.improvement`・`62.design`が付いていれば既定でON、それ以外（`30.bug`・`60.chore`・`65.docs`や種別未選択）は従来どおりOFF。判定は「実装を開始」ダイアログを開いた時点のIssueのラベルに対して行う（`startImplementationOptionsFromLabels`）ため、**起票時に`21.plan-required`を付けておく必要は無い**（#1580）。**あくまで初期値で、自分で押した後は上書きしない。**
  - 「実装を開始」ダイアログを通らない起動経路（カンバンのドラッグ）では、この既定は効かない。ドラッグは`Ready`→`Planning`へ落とせば計画になり、そのときWebhook側が`21.plan-required`を付ける（`src/app/api/webhooks/github/route.ts`）。
...
開発サーバー（`pnpm dev`）のポート割り当て自体はコストがないため、ラベルの有無に関わらず常に`.env.local`に`PORT`（issue-deckなら`4000 + Issue番号`を帯の中で折り返した値。#2478）を設定する。ラベルは「画面確認をPR作成前の承認ゲートにするかどうか」を制御する。
...
- 承認の得方は実行形態により異なる。
  ...
  - **GitHub Actions実行（無人）**: ワークフロー終了と同時にdevサーバーも消えるため、実装エージェント
    ...
    `00.check-user`を付与し停止していたが、確認ゲートをPR作成前ではなくdevelopへのマージ前に移す
    方針で#813にてPR作成をブロックしない方式に変更した。一時期は#832でFly.io Machines上のプレビュー環境（#826・#830・#831）へ接続し、
...
**このラベルはローカル実行専用**（#1473）。アーティファクトの公開は
...
- **チップの初期値はIssueの種別ラベルから決める**（#1956・`artifactRequiredDefaultForLabels`）。
  ...
  - ダイアログを通らない起動経路（カンバンのドラッグ）ではこの既定は効かない。
...
- **`21.plan-required`と併用するのが基本形。** 併用時は**Plan modeに入る前に**公開し、URLを計画本文へ
  含める（Plan modeで書けるのは計画ファイルだけなので、最初の1枚は入る前に作る）。計画と見た目を1回の
  ...
  - **計画のやり取りで見た目の直しを求められたら、計画ファイルの中で差し替える**（#2200）。
    Plan modeで書けるのは相変わらず計画ファイルだけだが（Claude Code 2.1.241のplan mode
    リマインダに`Read-only except plan file (…)`と明記されている）、**その中に置いたHTMLは
    issue-deckが取り出して「アーティファクト」カードへ取り込む**（`src/lib/dispatch/plan-artifact.ts`）。
    書式は`<!-- artifact: <公開したHTMLファイルの絶対パス> -->`の1行＋バッククォート4つと
    `artifact`で開くフェンス。パスを公開時と同じにすると同じカードが差し替わり、計画コメントに
    HTMLは載らない。**差し替わるのは先にissue-deckのカードだけ**で、claude.aiのURLは
    修正前の見た目のまま残る——承認の直後、コードを書く前に同じパスへ書いて`Artifact`を
    呼び直すところまでが1組。**同一判定は`sourcePath`のハッシュだけ**なので、計画ファイルへ
    書くパスが初回の`Artifact`と1文字でも違うと、上書きではなくカードが2枚に増える。
    **#1745の「Plan modeの中では差し替えられない」はこれで解消した**（当時は「公開済みの版は
    修正前であること」を計画本文に断って承認を取るしかなく、承認するまで新しい見た目を
    見られなかった）。**見た目の直しだけを再承認させるゲートは足さない**のは変わらない
    （直しは計画の「修正を送る」の1往復に相乗りするので、承認は1回で済む）。
...
- **見た目案はDesignタイプ（キャンバス）で作らない**（#2984）。`Artifact`ツールの説明が
  「デザインを作るなら最初に`action: "quickstart"`を呼ぶ」と勧めるため、放っておくと
  `intent: "design"`のArtifactタイプ（キャンバス）が選ばれる。**そこから公開すると`file_path`が
  キャンバスの目次（`project/canvas.json`）になり**、issue-deckのカードには
  `{"v": 3, "boards": {...}}`というJSONがそのまま出る（画面ごとの中身は`project/*.dc.html`に
  分かれていて届かない。guchi-apps/asset-manager#454）。**3画面を並べた1枚の自己完結HTMLを
  `file_path`に指定して公開する**のが正で、これは3か所のプロンプト文面に明記してある。
  - **出口も閉じてある。** `scripts/session-notify.sh`の`report_artifact_to_issue_deck`は、
    `file_path`が`.html`/`.htm`以外の公開と、`type_url`を渡す公開（型から器を作る呼び出し）を
    取り込まない。**キャンバスを解釈しにはいかない**——目次の形式はclaude.ai側の都合で変わる
    うえ、1枚のHTMLで出す約束が入口にある
  - この形になると**カードは1枚も出ない**（JSONが出るよりは良いが、原因は画面に出ない）。
    見た目案を公開したのにカードが増えないときは、まずDesignタイプを使っていないかを疑う
- **画面デザインはPC・iPad・スマホの3画面を並べて出す**（#1632・#2460）。1つのアーティファクトの
  中に、PC（デスクトップ幅）・**iPad（横向き = 幅1180px × 高さ820px）**・
  **スマホ（iPhone 15 = 幅393px × 高さ852px）**を、この順（広い順）で並べる。
  issue-deckは外出先のスマホから使う場面が多く、PCの見た目だけを承認しても、
  スマホ幅での崩れは実装後の実画面で初めて分かる。それでは「作る前に見た目を合意する」という
  このラベルの効きどころが半分失われる。減らす場合は理由をアーティファクト本文に書く。
  **この原則はラベルの有無によらない**ため、`25.artifact-required`が付いていないときの文面
  （「作成は不要です」）にも、それでもデザインを出す場合の但し書きとして同じ指示を入れている。
  - **iPadを3枚目に足したのは、その幅がPC・スマホのどちらの1枚にも入らないため**（#2460）。
    スマホ用の画面（`MobileIssueDetail`など）へ切り替わるのは幅768pxまで（`md`・
    [check-user-focus.ts](../../src/lib/check-user-focus.ts)）なので、iPadは横（1180px）でも縦でも
    PCと同じ3カラムが出る。ただし幅はPCよりずっと狭く、カラムが痩せた状態での崩れは
    PCの1枚にもスマホの1枚にも現れない。
  - **横向き（1180×820）だけを必須にする。** 縦向き（820×1180）は幅が`md`帯に入り、
    横とは別の見え方になるが、Issueの要求（#2460）が横画面のため必須には含めていない。
  - **アーティファクトそのものもスマホで開いて読める作りにする**（#2974）。3画面を並べる
    決まりだけでは並べ方が決まらず、枠を実寸（PC 1280px・スマホ393px）のまま
    `overflow-x:auto`に入れたアーティファクトができていた。承認する人は外出先のスマホで
    開くことが多く、その形だとPCの枠を見るには何度も横へ動かす必要があり、**スマホの枠すら
    画面（本文幅361px）に収まらない**。そこで次の3つをプロンプトに入れている。
    - 枠は実寸（PCはデスクトップ幅のまま数値を決めない・iPad 1180px・スマホ393px）で組み、
      閲覧幅に合わせて`zoom`で縮小する（`class="fit" data-w="<枠の幅>"`と
      数行のスクリプト。文面に全文を載せている）。`transform: scale()`は見た目だけ縮んで
      元の高さぶんの余白が残るため使わない。`zoom`はレイアウトごと縮むので高さの補正が要らない
      （issue-deckの画面内で開いたときもスクリプトは動く。iframeが`allow-scripts`付きで、CSPも
      `script-src 'unsafe-inline'`のため。[artifact-document.ts](../../src/lib/artifact-document.ts)）
    - 見出し・説明・注記は折り返し、2列以上の比較や注記の並びは幅600px以下で1列に積む
    - 幅393pxで開いたとき、ページ全体が横にはみ出さず、スマホの枠が画面の幅に収まる
  - **順序を広い順に固定しているのは、カードのサムネイルが上端を写すため。** サムネイルは
    幅1200pxで描いた上端を縮小して出す（[issue-artifact-panel.tsx](../../src/components/dashboard/issue-artifact-panel.tsx)）
    ので、いちばん上にPCを置くと一覧での見分けが3画面にしても変わらない。
...
| | `23.preview-required` | `25.artifact-required` |
| --- | --- | --- |
| 止まる場所 | PR作成前 | **実装着手前** |
| 使える実行先 | ローカル・サブPC実行 | ローカル・サブPC実行 |
| 見せるもの | 動いている実物 | 手で書いた見た目案HTML |
| 忠実度 | 実物そのもの | 近似（実装とずれ得る） |
| 操作できるか | できる | 限定的（自己完結HTMLの範囲） |
| セッション終了後 | 消える | 残る（claude.ai） |
| コスト | 低（起動済み） | 低 |

**`25`と`23`は競合しない。時間軸が違う**（#1540）。`25`は「作る前に見た目を決める」ためのもので、
`23`は「出来上がったものを確かめる」ためのもの。見た目を決めてから作り、出来たものを実物で
...
**いつ選ぶか**（#1317・#2775・#2790）。**`merge-policy`の既定が`relaxed`になったため、変更の種類では
止まらなくなった**（「変更カテゴリで止めるかは`merge-policy`で切り替える」参照）。認証・認可、
DBスキーマ・マイグレーション、本番環境の設定、GitHub Actions・デプロイ設定、Secrets・環境変数、
課金・決済、大規模な依存更新を**自分の目で通したいなら、このラベルを明示的に付ける**。あわせて、
影響範囲が読みにくいリファクタリング、共通コンポーネント・共通関数の書き換え、Issueの文面だけでは
仕様が決まりきっていない実装、他リポジトリへ配る雛形の変更も従来どおりこのラベルの出番。

**この基準はcallerを持つ全リポジトリで同じ**になった（#2790）。例外は、callerに
`merge-policy: strict`と明示したリポジトリと、`merge-policy`入力が入る前の参照タグ
（`workflows/v31`以前）に留まっているリポジトリで、そこでは上のカテゴリがラベル無しでも
機械的判定に止められる。

- ラベル `22.merge-confirm-required` の有無で分岐する。
  - **ラベルなし（デフォルト）**: 従来どおり、`risk-check`（機械的判定）・`claude-review`
    （意味的判定）の結果のみで自動マージ可否を判断する。`merge-policy`の既定が`relaxed`の
    ため、機械的判定は変更カテゴリではマージを止めない（#2775・#2790）。
  - **ラベルあり**: 対応Issueに付いている限り、develop向けPRへのpushのたびに`risk-check`
    ジョブが判定し、変更内容によらず常に該当扱いにして自動マージをスキップする。
    **`merge-policy`の値に関係なく効く**（#2775。緩和の対象はパス・依存関係による
    カテゴリ判定だけで、ラベルによる明示的指定はそのまま残る）
  ...
  - 「実装を開始」ダイアログのオプション（`src/lib/github/start-implementation.ts`の
    `START_IMPLEMENTATION_OPTIONS`）にも「マージ前に確認」として追加済みで、`21.plan-required`
...
- **`merge-policy`の既定が`relaxed`になったため、この穴は塞がっていない**（#2775・#2790）。
  `risk-check`は`.github/workflows/**`の変更を今までどおり検出するが、`relaxed`では
  それを「レビューを実行する理由」の側にしか使わない。そのレビューが上のとおり
  Claudeを実行しないまま`success`で終わるため、**ワークフロー変更PRは機械判定も意味的判定も
  無いままdevelopへ入る**（CIは通る）。`strict`だった頃の「必ず`00.check-user`が付くので
  レビュー結果が無くても人間の確認で代替される」という前提は、もう成り立たない。
  **ワークフローの変更を人の目で通したいときは、対応Issueへ`22.merge-confirm-required`を
  明示的に付ける**（これは`merge-policy`の値に関係なく止まる）。callerを持つ全リポジトリで
  同じ状態であり、#2790で許容すると決めている。**本番へ出るには`develop`→`main`の
  リリースPR（人がマージする）をもう1回通る**のが、残っている確認の機会。
...
無人実行は参照文書`docs/multi-agent/unattended-implementation-reference.md`〈#3024〉）。**雛形を直したら`node scripts/generate-prompt-templates.mjs`を
...
- **`（VPS）`の手順も代行できる**（#2901）。サブPCからTailscale SSHでVPSへ入って実行するので、
  `.env`の追記・PM2の再起動・DBの作成のような実機の操作が「承認して実行」で流れる。ただし
  **`sudo`を含む手順は対象外**（VPSにNOPASSWDの設定が無く、SSH越しでは必ず失敗する）で、
  cwdはVPS側のホームになる（手順はテンプレートどおり自分で`cd`する）。**VPSの設定ファイルの
  変更を手作業Issueへ書いてよくなったわけではない**——`/etc/apache2/`・`/etc/systemd/`などは
  従来どおり`guchi-apps/vps`のIssueへ切り出す（後述「実機の設定ファイル変更は、管理リポジトリの
  Issueへ切り出す」）
- サブPC・VPS以外（1Password・GitHub App・ブラウザ・メインPC）の手順では、ボタンの代わりに
  理由が出る。**判定は手順ごと**なので（#2052）、「手順1はブラウザで人が実行し、手順2はサブPCで
  代行する」が同じ手作業の中で混在できる。端末をまたぐ作業は各手順の文頭に`（サブPC）`のように
  書く。書かれていない手順は`## 前提条件`の「実行するデバイス」を既定値として使い、そこに端末が
...
- 巡回するのは**`## 前提条件`の「実行するデバイス」がサブPCかVPSに決まる**もので、**確認コマンドが
  読み取りだけだと読める**Issueに限る（VPSは#2901で加わった。SSHで到達できるホストが居るときだけ）。
  1Password・ブラウザ・メインPCでの手作業や、確認コマンドが状態を変えうるものは対象外
  （印が出ないだけで、従来どおり人が実行できる）
...
#### 確認が通っていないままのクローズは1回聞き返す（#2256）

**チェックが付いていることは、実施された証拠にならない。** `aide-bot`の立ち上げでは、
1Passwordへの登録がチェック済みのままIssueがcloseされ、初回デプロイが
`DB_NAME: DB_NAME is required`で失敗した（`guchi-apps/aide-bot#8`として起票し直しになった）。
そのIssueの`## 完了の確認方法`は「`dig`がIPを返し、左メニューに並べば完了」で、
**登録されたかを見ていなかった**。そこで次の3つを入れている。

- **確認は手順と1対1のコマンドで書く。** 雛形（[manual-step-body-template.md](manual-step-body-template.md)）の
  `## 完了の確認方法`が、1確認＝1つのコードブロックの形になっている。実行できるコマンドが
  1つも無い本文には、書式検査（`manual-step-body-check.ts`）が`warning`で指摘する
- **人が流した確認の結果も「完了済みの可能性」の印になる。** 手作業アシスタントの代行実行で
  確認コマンドが**すべて**通ると、定期巡回と同じ印が付く（`recordManualStepVerificationPass`）。
  照合は行番号ではなくコマンドの文字列で行うので、確認コマンドを書き換えれば未確認へ戻る
- **印が無いまま「完了してクローズ」を押すと、1回だけ聞き返す。** 未確認の確認コマンドを
  並べて出し、もう一度押せば（「確認せずクローズ」）そのまま閉じられる。**閉じること自体は
  止めない**——管理画面での設定のように、機械的に確かめようがない手作業は実際にあるため。
  確認がコマンドで書かれていない本文では、そもそも聞き返さない
...
### 変更カテゴリで止めるかは`merge-policy`で切り替える（#2775・#2790）

上のカテゴリのうち`develop`→`main`を除く7つは、**共有ワークフローの`merge-policy`入力で「マージを止める理由」として扱うかどうかを切り替えられる。**

| 値 | 挙動 | 使っているリポジトリ |
| --- | --- | --- |
| `relaxed` | **既定。** カテゴリでは止めず、そのまま自動マージへ進む | callerを持つ全リポジトリ |
| `strict` | カテゴリに該当したら`00.check-user`＋`01.check-merge`を付け、自動マージしない | 明示的に`with:`へ書いたリポジトリだけ（現在なし） |

`relaxed`にできるのは、**`develop`はリリース前の統合先**で、本番へ出るには`develop`→`main`のリリースPR（自動マージ不可カテゴリのまま・人がマージする）をもう1回通るため。develop向けPRの側で毎回「確認待ち」の札を積んでも、押すのは同じ人・同じ変更に対する2回目の確認にしかならなかった。

**#2775ではissue-deckのcallerだけに書き、#2790で既定そのものを反転してオプトアウト方式にした。** 配布先18リポジトリの実測（2026-09-03）で、**そのすべてが`release-develop-to-main.yml`と`deploy.yml`を持ち、`deploy.yml`は`push: branches: [main]`でしか動かない**こと——つまりissue-deckで`relaxed`にできた根拠がそのまま当てはまることを確認している。`develop`を持たない`docs`・`claude-config`・`question`・`ideas`はそもそもcallerを持たず、`base: develop`のトリガーが一度も発火しないため影響を受けない。

**配り方として既定の反転を選んだのは、`with:`の追加を運べないため。** 参照タグ更新PR（`.github/scripts/propagate-workflow-tag.sh`）が置換するのは`@workflows/vN`と`prompts-ref`だけで、`merge-policy: relaxed`という行は運べない。しかも**配布先は全て`@workflows/v31`を参照していて、v31には`merge-policy`入力そのものが無い**——callerへ書き足すと未定義入力でワークフローが起動しなくなる。既定を反転すればcallerを1行も触らずに済み、画面の「新しいタグを切って配る」でタグを上げた時点で効く。**タグは`main`の先端から切られる**ので、効き始めるのは`develop`→`main`のリリースを1回通した後。

**`strict`へ戻すのはリポジトリ単位でできる。** そのリポジトリのcallerの`with:`へ`merge-policy: strict`と書く（**入力を持つタグを参照している場合に限る**。`workflows/v31`以前を参照したまま書くと起動しなくなる）。

**配布先の`CLAUDE.md`／`AGENTS.md`は未追従。** 反転したのは共有ワークフロー側だけで、各リポジトリが自前で持っている「自動マージ不可カテゴリ」の節は反転前の文面のまま残っている（2026-09-03時点で15リポジトリが該当）。全アプリ共通の正は共有知識（`guchi-apps/docs`の`agent-rules/review.md`）にあり、そちらの更新を待つ。**食い違ったらcallerの実物を正とする。**

`relaxed`でも止まるのは次の3つで、**どれも「変更の種類」ではなく「実際に何かある」ことを見ている**。

- 対応Issueに`22.merge-confirm-required`・`23.preview-required`のいずれかが付いている（明示的指定）
- `.shared-context/`（共有知識リポジトリのcheckout先）が差分に混入している。これはカテゴリではなく**入ってはいけないものが入っている誤り**の検知なので、緩和の対象外にしてある
- 二次判定（`claude-review`）が「実際に直すべき問題がある」と判定した
- `claude-review`・`auto-merge`ジョブ自体が失敗した（各`*-fallback`ジョブが付ける）
...
**ただし`.github/workflows/**`を変更するPRには穴が残る。** claude-code-actionの検証機構により、そのPRでは`claude-review`がClaudeを実行しないまま`success`で終わる（後述「ワークフローファイルを変更するPRではclaude-reviewが必ずスキップされる」）。`strict`では`risk-check`が止めていたので実害が無かったが、`relaxed`では**機械判定も意味的判定も無いままdevelopへ入る**（CIは通る）。ワークフローの変更を人の目で通したいときは`22.merge-confirm-required`を付ける。**#2790で既定を反転したときも、この穴は配布先でも同じく許容すると決めた**——`relaxed`の意味を「変更の種類では止めない」の一本に保つためで、例外を1つ作るとリポジトリごとに何が止まるかを二重に覚えることになる。塞ぐ手段（`22.merge-confirm-required`）は既にあり、本番へ出るには人がマージする`develop`→`main`のリリースPRをもう1回通る。
