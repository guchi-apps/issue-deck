...
当初の懸念（誰も消せない古い状態が残る）は、解ける経路を3つ持たせることで潰している。
3つ目は**#1353で足りないことが分かって足した**（次節）。
...
- **人が承認プロンプトに答えたことは、どのフックにも現れない。** 飛ぶのは
  `Notification`（止まった時）と`Stop`（turnが終わった時）だけで、その間にある「答えて作業へ
  戻った」瞬間を知らせるものが無い。そのため画面の「入力を待っています」は、答えた瞬間ではなく
  **そのturnが終わった時にしか解けない**。答えた後の作業が長ければ、その間ずっと入力待ちに
  見える（#1353で観測し、#1357へ切り出した。解消には`PostToolUse`のような高頻度のフックと、
  `RESPONDED`とは別の「作業中」を表す値＝enumの追加が要る）
