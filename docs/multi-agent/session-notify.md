...
**現在の段のマスを掃く光**で「今動いている」ことを表す（#2516。円グラフだった頃は外周の
...
- **見た目は「現在の段のマスを掃く光」**（#2516）。円グラフだった頃は「常時見えるトラック＋
  半周の弧」の回転で表していた（#2358。それ以前の2pxの弧が1/4周だけの頃は、18pxのバッジの
  周りで回っていること自体に気付けず、スマホでは「一瞬しか出ていない」と報告された）。
  **掃くのは現在の段だけで、塗り（進捗）は動かさない**——バー全体を光らせると、どこまで
  進んだかを読み取る手がかりまで動いてしまう。起動中（`QueueStepBadge`）だけは掃く先の
  マスが無いため、トラック全体を光が1往復する
