...
- **見た目は「常時見えるトラック＋半周の弧」**（#2358）。以前は2pxの弧が1/4周だけで、18pxの
  バッジの周りでは回っていること自体に気付けず、スマホでは「一瞬しか出ていない」と報告された。
  トラックがあると弧の位置に関わらず輪郭が見えるので、止まっているのか回っているのかを
  視線を止めずに読める
- **確認待ちでも、エージェントが動いている間は回す**（#2358）。`00.check-user`は「人の対応が
