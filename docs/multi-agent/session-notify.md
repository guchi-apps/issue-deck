...
### 計画ファイルの`Write`と同じ応答で呼ぶと、入力が`{}`になる（#3136）

**Claude Codeは`ExitPlanMode`を呼んだ時点で計画ファイルを読み、入力（`plan`・`planFilePath`）へ
詰める。** 上の「実測でも`tool_input`は空」と「2.1.239では入っていた」の食い違いは版差ではなく
これで説明がつく。エージェントが計画ファイルの`Write`と`ExitPlanMode`を1つの応答に並べると、
詰める時点でまだファイルが無いため入力が`{}`になる（2.1.278の実測: 並べた4件は全件`{}`、
別の応答で呼んだ約25件は全件本文入り）。

入力が`{}`のときに起きること:

- **Remote Control（Claudeアプリ）の承認カードに計画が出ない。** アプリは入力の`plan`を描くため、
  計画本文の無い汎用の「ExitPlanModeが許可をリクエストしています」になる
- **フックも計画を送れないことが多い。** 同じ応答の`Write`はフックが走る時点で転記にまだ
  書かれておらず、上の探し方で見つからない。issue-deckのパネルが出ず、エージェントが後から
  手で投稿するまでIssueにも残らない

そこで`session-notify.sh`は、入力に`plan`が無いとき**1回だけ`deny`で差し戻し**、
「`ExitPlanMode`だけを次の応答で呼び直す」よう伝える。呼び直した方は入力に計画が入るので、
アプリにも画面にも計画が出る。計画ファイルの場所は転記の`plan_mode`添付（`planFilePath`。
plan modeに入った時点で書かれる）から引く。

- **差し戻すのは同じ計画（セッション・パス・中身）につき1回だけ。** 印は
  `~/.local/state/issue-deck/sessions/plan-retry/`に置く。呼び直しても入力へ詰めない版に
  当たったときに、差し戻しを繰り返して先へ進めなくなるのを避ける（2回目は従来どおり、
  計画ファイルの中身を送って待つ）
- 差し戻しの時点では承認プロンプトを出していないので、入力待ちの記録も`00.check-user`も付けない
- あわせてプロンプト（`planCommentNote`・`start-issue.sh`・`generic-start-issue.sh`）でも
  並べないよう指示している。差し戻しは指示が守られなかったときの備え
...
- **待ち時間は計画と別の環境変数だが、既定は同じ30分**（`SESSION_QUESTION_WAIT_SECONDS`。
  #2850）。以前は`ExitPlanMode`が1セッションに1回の関門なのに対し**質問はturnの途中で
  何度も起きる常用経路**で、待っている間は端末で答える手段が実質的に無い（`Esc`はturnごと
  打ち切る。上記）ことを理由に計画より短くしていたが、#2822でIssue詳細のセッション行に
  「アプリで答える」トグルが追加され、画面待ちに入るかどうか自体を人が選べるようになった
  ため揃えた。**引く間隔と降りるまでの猶予は計画と共有する**
  （`SESSION_PLAN_POLL_INTERVAL_SECONDS`・`SESSION_PLAN_POLL_GRACE_SECONDS`）——どちらも
  「issue-deckへ何秒おきに引き、届かない状態が何秒続いたら降りるか」という同じ性質の値で、
  分けても片方だけ調整する理由が無い
...
- **効くのはissue-deckのセッションだけではない。** フック設定を書く`run-issue-session.sh`は
  ...
  （待ち切れば端末のフォームへ倒れる）。既定を30分にしてあるのはここの上限でもある
...
- **質問の前提（直前のコメント）をパネルの中に出す**（#2742）。エージェントは
  「上記の計画で実装を進めてよいですか？」のように、**直前に自分が投稿したコメントを指して**
  聞くことが多いが、そのコメントは画面ではコメント欄のずっと下にある。読みに行くと選択肢が
  画面外へ出るため、**スマホでは前提と選択肢を同時に見られない**。選択肢の真上に、その
  コメントを畳んで置く（「全文を表示」で開く。計画の承認パネルが計画本文を枠内に出すのと
  同じ作法）。
  - **質問とコメントを結ぶデータは無い。** 質問はリポジトリとIssue番号でしか紐付いておらず、
    どのコメントを指しているかはツール入力に現れない。選んでいるのは**取得済みコメントの
    末尾から見て、エージェントが書いた最新の1件**でしかないので、**役割（計画ボットなど）と
    投稿時刻を必ず添える**——今の質問と関係のある発言かを判断するのは読む側になる
  - **案内ボット（受付・締め）・通知ボット・回答ボットは前提にしない。** 拾うと
    「対応を開始します」や前回の回答が前提として出る。判定は`findQuestionPremise`
    （`src/lib/dispatch/question-premise.ts`）で、自動投稿かどうかは本文のマーカーだけで
    見る（`isMarkedAutomationComment`。ローカルセッションの`gh`は本人のトークンで動くため
    投稿者名では区別できない）
...
`scripts/prompts/implementation-agent.md`・`generic-implementation-agent.md`の「責務とIssueに残す記録」に
...
- **判定は`scripts/lib/session-tool-call-stall.sh`が持つ。** 転記の最後のやり取りが
  ...
  かつ一定時間（既定3分。`SESSION_TOOL_CALL_STALL_MINUTES`）転記が更新されていない場合に
  検知する
- **固定文面を自動で最大2回送り、それでも直らないときだけ人へ引き上げる**（#2896）。
  #2655の時点では自動再送信をしない設計だった。根拠は研究デスク#41の実例で、「進めて」という
  **曖昧な**再送信のあともモデルが「自分は先にツールを呼び出した」という誤った過去発言を事実と
  誤認し、`ListAgents`で確認しても見つからないのに「まだバックグラウンドで動いている」と誤答して
  再び止まった、というもの。実測し直したところ、**次の入力が入ったセッションはほぼ全件が
  その後のターンでtool_useへ復帰していた**（固定文面19件・人が打った文面19件。後者には
  「進めて」系も含む）。一方、人が画面のボタンを押すまでの放置時間は中央値12分・最長5時間で、
  待たせている時間のほうが大きかった。開ける形はAPIエラー再開（#1971）と同じ3条件——固定文面・
  3段階プロトコル（`deliver_session_instruction`）・条件を確かめたセッションだけ——で、
  [gates.md](gates.md)の例外もそこに揃えている
- **「固定文面だから解ける」わけではない**（#2896の計画レビュー）。復旧させているのは入力が
  入ったこと自体で、固定文面でも2件は同じ形で再発している。**本文を固定にしているのは
  [gates.md](gates.md)の線（実行体に本文を組み立てさせない）を守るためで、効き目のためではない。**
  ただし「進めて」のような曖昧な文面は`ListAgents`で幻のバックグラウンド実行を探しにいく
  遠回りを実測で残すので、事実を明言する文面のままにする
- **回数の記録を消してよいのは、送ったあと実際にツールが呼ばれたときだけ**（#2896の計画レビュー
  指摘1）。「検知しなくなったら消す」にすると上限が一度も効かない——**送出そのものが転記へ
  `queue-operation`レコードを書いてmtimeを更新する**ため、次の巡回では停滞時間が閾値未満になり
  必ず「検知せず」になる。判定に使うのは`.step`（ツールの実行ごとに進む時刻）だけで、`.event`は
  使わない（`Stop`でも進むが、**この現象のターンは`Stop`が正常に発火している**）
- **「進めて」のような曖昧な継続指示は人が送る場合も避ける**（#2675）。上と同じ誤認識を
  そのまま踏む。引き上げコメント（`session-escalation.ts`）には「呼ばれていないことを事実として
  明言し、実際に呼び出し直すよう指示する」具体的な文言を添えている
- **送る本文の正は2か所にあり、同じ1行でなければならない。** pollerが自動で送るのは
  `SESSION_TOOL_CALL_STALL_BODY`（`scripts/lib/session-tool-call-stall.sh`）、画面の停滞パネル
  （#2886）が送るのは`TOOL_CALL_STALL_BODY`（`src/lib/dispatch/session-stall.ts`）。食い違うと
  「自動で送ったもの」と「人が押して送ったもの」の区別が後から付かないため、
  `scripts/session-tool-call-stall.test.mjs`が一致を固定している
...
- 停滞時間の判定は`lib/session-resume.sh`の`session_resume_stalled_seconds`をそのまま使う
  ...
  この現象に気づくまでの時間を縮めてほしいという要望（#2675）を受けて既定10分へ下げ、#2896で
  さらに3分へ下げた。**APIエラーと違い「エラーの直後に自力で書き始めたturnへ割り込む」余地が
  無い**ため（ターンは`Stop`まで終わっており、次に動くのは人か再送信だけ）、待っても判定材料は
  増えない。判定条件そのものが排他（片方は転記末尾がAPIエラー、もう片方はツール呼び出し風の
  テキスト）なので、APIエラー検知とも競合しない。運用で調整したい場合は
  `deploy/subpc/dispatch.env.example`の`SESSION_TOOL_CALL_STALL_MINUTES`を上書きする
- 境界は`scripts/session-tool-call-stall.test.mjs`（判定・回数管理・文面の一致）と
  `scripts/session-tool-call-stall-recovery.test.mjs`（pollerの「送り直す→上限で引き上げる」の
  順序）が固定している

#### 起こしているのは`sonnet-5`の「セッションまるごとforkへ丸投げ」（#2896）

直近45日ぶんの会話転記1,736件を機械的に走査したところ、この形の応答は**31件（29セッション）**
あり、内訳は次のとおりだった。
...
- 実際に成功した`Agent`呼び出しのうち`fork`はsonnetの23件のみで、opusは1件も使っていない。
  一方`Explore`（sonnet 61件・opus 70件）・`Plan`（25件）は両方が正常に呼び出せている

つまり原因は「sonnet-5がキックオフでセッションの作業まるごとをforkへ委任しようとし、その
呼び出しがtool_useにならずテキストで出る」ことに絞れる。sonnetで起動した152セッションの
**約19%**で起きていた。

- **予防はキックオフ文面（`scripts/run-issue-session.sh`）に置く。** 「このIssueの作業そのもの
  （指示ファイルを読む・計画する・実装する）をサブエージェントやforkへ委任しない（調査を
  `Explore`へ委ねるのは構わない）」の1文を必ず足す。**誤出力は最初のターン——プロンプト
  ファイルを読む前——に起きるため、`scripts/prompts/*.md`側に書いても間に合わない**
  （同じ趣旨は2ターン目以降のために両方の実装プロンプトへも書いてある）
- **`--disallowedTools Agent`で機械的に塞がない。** 計画レビュー・コードレビューのセッション
  （`start-plan-review.sh`・`start-code-review.sh`）はそうしているが、実装セッションでは正常に
  使えている`Explore`・`Plan`まで巻き添えになる。止めたいのは「担当する作業そのものを別の
  エージェントへ渡す形」だけ
- **モデルの選択自体は運用の判断として残す。** sonnetを実装に使わない選択もできるが、それは
  画面でIssueごとに選んでいるもので、この現象のために自動で変えることはしない
...
- **受け口は`POST /api/dispatch/session-recovery`と分けてある。** ここだけが`00.check-user`を
  ...
  サーバー側で確かめ直す。任意の本文は従来どおり「追加指示」で送る（そちらは印を外さない）
...
一覧の各行にある進捗バッジ（`WorkflowStepBadge`）は、7マスの横棒（#2867・#2927）で進捗（Project Status）を、
...
- **Push通知も同じ待ちを見て、3分の待ち時間を飛ばす**（`notifications/check-user-push.ts`の
  ...
  **質問の待ち時間は既定30分**（#2850で計画と統一）なので、3分待っても届くのに十分な
  余裕が残る
