...
| `claude-review-develop.yml` | develop向けPRの自動レビュー・自動マージ不可判定（`risk-check`）・Auto-merge有効化を行う。**トリガー定義と`concurrency`のみ**を持ち、本体は`reusable-claude-review-develop.yml`を`uses:`で呼ぶ（#1078） | **コピーではなく薄いcallerを置く。** リポジトリ固有のリスクパスは`risk-paths`、依存関係の判定基準は`dependency-check`、差分規模の閾値は`review-file-threshold`・`review-line-threshold`、除外するlockファイルは`lock-files`で指定する。**変更カテゴリ（DBマイグレーション・ワークフロー・`.env`・認証・依存のメジャー更新）でマージを止めるかどうかは`merge-policy`（`relaxed`＝既定・止めない／`strict`＝止める）で切り替える**（#2775・#2790。既定はcallerを持つ全リポジトリに効き、止めたいリポジトリだけが`strict`と書く。**この入力を持つタグを参照している場合に限る**——`workflows/v31`以前には入力自体が無く、書くと未定義入力で起動しなくなる）。プロンプトは`prompts-ref`に`uses:`と同じタグを指定してissue-deck側を共有する |
...
| `claude-review-fix.yml` | develop向けPRで自動レビューが「要修正」と判定し、レビュー自身が「人の判断が要らない指摘だけ」と印を付けた場合に、その指摘を自動修正する（#3363）。**トリガー定義のみ**を持ち、本体は`reusable-claude-review-fix.yml`を`uses:`で呼ぶ | **コピーではなく薄いcallerを置く。** 指定する入力は`claude-ci-fix.yml`と同じ。`workflow_run`の購読先は`claude-review-develop.yml`の`name:`（`Claude Code Review (develop向けPR)`）なので、**`claude-review-develop.yml`があるリポジトリにだけ置く**。置かれていないリポジトリでは、レビュー側が渡さず従来どおり`00.check-user`で人へ渡る |
...
| **未タグ（`workflows/v31`の次に切るもの）** | 上記 | #2790。`reusable-claude-review-develop.yml`の`merge-policy`の既定を`strict`から`relaxed`へ反転した版。**変更カテゴリ（DBマイグレーション・ワークフロー・`.env`・認証・依存のメジャー更新・`risk-paths`の追加分）でdevelopへの自動マージを止めなくなる**（レビューは従来どおり実行する）。**callerの追加・書き換えは不要**で、参照タグを上げれば効き始める。止めたいリポジトリだけが`with:`へ`merge-policy: strict`と書く。**タグは`main`の先端から切られる**ため、developへ入れただけでは配れない（`develop`→`main`のリリースを1回通す必要がある）。配るまで、対象リポジトリは従来どおりカテゴリ該当で`00.check-user`＋`01.check-merge`が付く（issue-deck自身はローカルパス参照のため配布前から効いている）<br>**`workflows/v29`〜`v31`の行はこの表に無い。** 画面の「新しいタグを切って配る」（#1876・#2476）でタグを切るようになってから、1タグ1行の追記が続いていない。内訳は`git log --oneline <前のタグ>..workflows/vN`で確認する |
...
中身が同じ場合は押せないよう変更した。中身の比較ができない場合（GraphQL取得失敗）は
無効化しない——他の判定と同じく「わからない場合は制限しない」方針。API側
（`createNextWorkflowTag`）にも同じ判定でのガードがあり、UIの無効化を回避されても
タグは切られない。

コミット数と比較ページへのリンクは押す前の補助情報として残す。取得はGraphQLの
`Ref.compare(headRef: "main")`とtree OIDの取得を1本のクエリにまとめて行い、
**RESTの`/compare`は使わない**（あちらは差分のコミットとファイルまで返すため、画面を
開くたび・配布の実行中は10秒ごとに数百KBを運ぶことになる）。
...
| Secret（アプリ） | `SUPABASE_SERVICE_ROLE_KEY`・`APP_GITHUB_APP_PRIVATE_KEY_BASE64`・`APP_GITHUB_WEBHOOK_SECRET`・`APP_GITHUB_USER_TOKEN_ENCRYPTION_KEY`・`APP_GITHUB_OAUTH_CLIENT_ID`・`APP_GITHUB_OAUTH_CLIENT_SECRET`・`APP_GITHUB_BILLING_TOKEN`・`OPENAI_API_KEY`・`TYPESAFE_API_KEY`・`ALLOWED_EMAILS`・`DISPATCH_SECRET`・`IMAGE_UPLOAD_SECRET`・`VAPID_PUBLIC_KEY`・`VAPID_PRIVATE_KEY`・`VAPID_SUBJECT` |
