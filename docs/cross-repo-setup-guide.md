...
| Secret（アプリ） | `SUPABASE_SERVICE_ROLE_KEY`・`APP_GITHUB_APP_PRIVATE_KEY_BASE64`・`APP_GITHUB_WEBHOOK_SECRET`・`APP_GITHUB_USER_TOKEN_ENCRYPTION_KEY`・`APP_GITHUB_OAUTH_CLIENT_ID`・`APP_GITHUB_OAUTH_CLIENT_SECRET`・`APP_GITHUB_BILLING_TOKEN`・`OPENAI_API_KEY`・`OPS_API_TOKEN`・`ALLOWED_EMAILS`・`DISPATCH_SECRET`・`VAPID_PUBLIC_KEY`・`VAPID_PRIVATE_KEY`・`VAPID_SUBJECT` |
