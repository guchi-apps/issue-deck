...
| ステップ | すること | 実装 |
|---|---|---|
| 0. 相談 | 構想メモから仕様案を読み込むか、何を作るかをAIと数往復して固める。仕様案（名前・種別・DB・認証・URL）が下にまとまる | [`lib/claude/new-app-consult.ts`](../src/lib/claude/new-app-consult.ts)・[`lib/new-app/idea-doc.ts`](../src/lib/new-app/idea-doc.ts) |
...
構想の置き場（`guchi-apps/ideas`。#2430）で仕様を固めてある場合、ステップ0の先頭の
「構想から読み込む」で`ideas/<候補名>/README.md`を選べば、雛形（`templates/idea.md`）の
末尾の「仕様案」の表がステップ1・2の入力欄へ入る。**相談は要らない**——読み込むとそのまま
設定ステップへ進む。

- **解析は`lib/new-app/idea-doc.ts`（純粋関数）、取得は`lib/github/ideas-api.ts`**。
  ウィザードのコンポーネントが前者を直接importするので、この分け方は`spec.ts`と同じ制約
- **項目名は括弧書きを外した部分一致で突き合わせる**（`アプリ名（画面に出る名前。日本語可）`
  → `アプリ名`）。人が手で書くファイルなので、完全一致では拾えない
- **雛形のまま触られていない行は取り込まない。** 雛形の値の欄には選択肢そのもの
  （`public / private`・`はい / いいえ`）が入っているため、**選択肢を2つ以上同時に指せる値は
  選ばれていない**と見て「未決」に回す。実物の`templates/idea.md`を通すと、取り込まれるのは
  既定値が書いてある「テーマカラー」1件だけになる
- **1項目でも読めなかったときに全体を捨てない。** `lib/new-app/parse.ts`が「知らない値は
  全体を`null`」にしているのとは方針が逆で、あちらは実際にリポジトリを作る経路、こちらは
  **人が見て直せる入力欄を埋めるだけ**の経路だから。読めた項目・未決の項目・読めなかった値の
  3つを読み込んだ直後に並べ、設定ステップにも「構想から読み込んだ項目」を1枚出す
- **入力欄ごとにバッジは付けない。** 構想由来の値も人が直せる普通の値で、欄ごとに印を付けると
  「直してはいけない値」に見える
- **パスは`ideas/<候補名>/<ファイル>.md`に限る**（`isIdeaDocPath`）。画面から来た文字列を
  contents APIへ渡す経路なので、構想の置き場の他の場所を読める形にしない
- **`guchi-apps/ideas`を読めなくてもエラーにしない。** 一覧が空・権限が無い場合は1行の案内に
  なるだけで、相談から始める人の邪魔をしない

### 構想の正と一覧・削除（#3251）

構想の正は引き続き`guchi-apps/ideas`に置く。立ち上げ前でissue-deckのDB上に対象リポジトリが
無い段階からMarkdownを編集でき、Git履歴も残るためである。issue-deckは本文をDBへ複製せず、
「構想」画面を開いたときだけ一覧と本文を取得する。

削除は`README.md`だけでなく`ideas/<候補名>/`配下を対象にする。ファイルごとのContents APIでは
途中失敗で一部だけ残りうるため、Git Trees APIで削除ツリーを作り、1コミットで反映する。
書き込み開始後の401では処理全体を再試行しない。コミット作成済みか分からない状態から再実行すると、
同じ削除コミットを二重に作る可能性があるためである。
...
サブPCの手作業Issueはclone・対応表登録・シークレット投入に加えて、Claude Codeのフォルダ信頼確認も
持つ。信頼確認だけは対象リポジトリを人が確認して答えるため「あなたが実行」とし、残りは代行できる。
ここを完了条件から外すと、新規リポジトリは申告に載っても最初のローカルセッションが起動前に失敗する。

ここで作られるものが、そのままアプリを終了させるときに片付ける対象になる。逆順の手順は
...
オフライン対応・更新履歴・開発用ログイン**も決めるとしている。ウィザードはこれを
...
| 開発用ログイン | 用意する（認証があるときだけ） | ログイン後の画面をローカル・無人実行から検証する手段（issue-deck自身の`/api/dev/login`と同じ仕組み） |
...
- **認証が無いアプリでは開発用ログインの項目そのものを出さない**（`devLoginBypassEnabled`）。
