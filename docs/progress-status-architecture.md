...
ここを参照する（Issue側に設計を複製しない）。「Ready」〜「Done」の各状態が概念的に何を
意味するか（検討中・レビュー中・確認待ち等との対応）は
[Issue・PRの責任モデル](issue-pr-responsibility-model.md)「状態モデル」も参照。
...
- **本番へデプロイしても、pollerは新しい受け口を叩き始めない**（#2298）。下記「移設は『外す側』
  だけが即座に効く」を参照
- **`schedule`で動くジョブはもう無い。** 他リポジトリのcallerに`cron`が残っていても、reusable側で
  受けるジョブが1つも無いので全ジョブがskipされ、課金は発生しない（skipジョブは課金対象外）

#### 移設は「外す側」だけが即座に効く（#2298）

**「本番へデプロイした」＝「新しい経路が動き出した」ではない。** Actionsの定期実行を常駐プロセスへ
移すと、次の3つが**別々のタイミングで効く**。

1. **外す側（callerの`schedule`削除・reusable側のジョブ削除）** — `develop`へマージした瞬間に効く。
...
- **Issue一覧の進捗バッジ（横棒）はコメントを持たないので判定しない。** もともとチェックを
  出しておらず進んだ段までのマスを塗るだけなので、見分けが付かない問題自体が起きない
