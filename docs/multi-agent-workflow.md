...
### 実装が詰まった状態の検知と、人間による明示的な復旧（issue #112）
...
そのため、詰まった状態を検知してもブランチの自動削除・作り直しはせず`mode=skip`とし、
issue_commentで呼びかけがあった場合は「PRが未作成のまま前回の試行が終了した可能性がある」旨を
コメントで説明し、作り直したい場合は人間が明示的に`git push origin --delete issue-<番号>`等で
ブランチを削除してから改めて`@claude`とコメントするよう案内する。ブランチが削除されれば、次回は
通常のimplementフローとして新規に作り直される。
