...
対策として、`claude-issue-dispatch.yml`の実装プロンプトは、分量が大きく実行時の値に依存しにくい
セクションをステップの`env:`へ切り出し、`prompt:`側は`${{ env.PROMPT_SCREENSHOT }}`のような短い
参照にしている。式テンプレート長は元テキストで判定されるため、これで上限に対する余裕が確保できる。
`env:`（ステップレベル）は`steps.*`コンテキストを参照できるので、切り出したセクション内に
`${{ steps.state.outputs.issue_number }}`が含まれていても問題ない。

余裕は`scripts/check-workflow-expression-length.mjs`（`pnpm check:workflows`）がCIで検査する。
上限超過でCIを落とし、85%超過で警告する。プロンプトを加筆したら、この警告が出ていないかを確認する。
