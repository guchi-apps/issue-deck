...
ラベルから機械的に判定する。`50.feature`（新機能の追加）が1件でも含まれていればminor、それ
以外はpatchとする。大規模な変更やbreaking changeの判断はラベルだけでは表現できないため、
判断が誤っていると思われる場合は人間が生成されたPR上でバージョンを直接修正する想定
（release-to-mainスキルの「迷う場合はユーザーに確認する」に相当）。
