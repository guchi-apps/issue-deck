...
- **ダイアログの段組みを、子の数を数える`grid-rows-*`で組まない**（#2402）。ヘッダー・本文・
  フッターの3段を`grid-rows-[auto_minmax(0,1fr)_auto]`で宣言していると、**条件付きで出る段
  （自動実行の帯など）が1つ増えた瞬間に割り当てが1つずつずれる。** 本文が`auto`の段へ落ちて
  中身の高さぶんを占め、`1fr`の段に入った帯は高さ0まで潰れる（潰れた帯の文字は自分の
  `border-b`に横切られ、次の段の中身に上書きされる）。**縦に余裕があるPCでは気づけず、
  スマホでだけ読めなくなる**——手作業アシスタント（[`manual-step-guide-dialog.tsx`](../src/components/dashboard/manual-step-guide-dialog.tsx)）の
  「失敗したため止まっています」がこれで見えなかった。`flex flex-col`で組み、スクロールする
  本文だけに`flex-1 min-h-0`、他の段に`shrink-0`を付ければ段が増減しても崩れない
  （`DialogContent`の既定の`grid`はtailwind-mergeが落とすので`flex`が効く）。
  高さの話はjsdomでは捕まらないため、テストはクラスの並びを見張る形にする。
...
- **スマホのフッターは`flex-1`の均等割で、枠を増やすと1枠の幅がそのぶん縮む**（#2631）。
  iPhone 15（幅393px）でいまの4枠なら1枠98px、5枠なら78px、3枠なら131px
  （[`mobile-bottom-nav.tsx`](../src/components/dashboard/mobile-bottom-nav.tsx)）。
  **1枠98pxではラベルは4文字までが目安**（#2811。`text-xs`＝12pxの全角4文字で約48px、
...
- **引っ張って更新の`translateY`を包む枠には、`relative`だけでなく`overflow-hidden`も要る**
  （#2885）。一覧を下げるのは`transform: translateY(...)`で、これは**レイアウトを動かさず
  見た目だけをずらす**ため、包む枠が切り抜かないと下がったぶんが枠の外へはみ出し、下に並ぶ
  兄弟の上に重なって描かれる。スマホのIssue一覧では、引っ張るたびに下端の絞り込み行
  （[`mobile/mobile-issue-list-screen.tsx`](../src/components/dashboard/mobile/mobile-issue-list-screen.tsx)）へ
  Issueの行が重なって見えていた——**その行に塗りが無く、ビューのボタンも`bg-primary/10`
  （10%）だった**ので、重なりが「透けて二重に見える」形で出た。`PullToRefreshIndicator`を
  置く4画面（Issue一覧・PR一覧・ブランチ画面・スマホのホーム）は同じ枠の形を共有しているので、
  直すときも4か所そろえる。インジケーター自体は枠の内側（`top-0`＋高さ＝引っ張り量）に
...
- **Issue一覧の上に並ぶ「〜が n件あります。」の入口バーは、`issue-list.tsx`の
  `COUNT_BAR_*`定数を使う**（#2107）。手作業アシスタントなどが
  同じ作りで、**入りきらない幅ではボタン側が次の行へ落ちる**（`flex-wrap`＋テキストの
...
- **作成フォームの項目は「種別 → リポジトリ → タイトル → 内容 → ラベル」の順で、
  ...
  **「画像を添付」は文字を出さず、アイコンだけを点線の四角（48×48px）で囲む**（#3074）。
  「ここに画像を置く枠」に見せるためで、名前は`aria-label`・`title`に残す（アップロード中は
  薄く押せないだけで、名前は「アップロード中」。回るスピナーはサムネイル列の点線枠が持つので、
  ボタンには出さない）。入力欄の部品を共有するコメント欄・質問なども同じ見た目になる。
...
- **画像を拡大して見せるのは[`image-preview-dialog.tsx`](../src/components/dashboard/image-preview-dialog.tsx)だけで、`target="_blank"`で別タブに開かない**（#2065）。
  ...
  **アーティファクトの「別ウィンドウ」（#2210）も同じ扱いで、スマホでは出さない**（PCだけの
  ...
  - **全画面の重ね表示を上へ重ねるときは、同じ`z-50`の層どうしの順番をDOM順に任せない**（#2983、#2993）。
    ...
    （閉じると上の履歴の問題が起きるため、開いている扱いは保つ）。さらに共通Dialogの中で
...
- **一覧を下へ引っ張って更新する操作は[`use-pull-to-refresh.ts`](../src/hooks/use-pull-to-refresh.ts)に集約する**（#1893・#1947・#1958・#2182）。
  ...
  **一覧の先頭に固定するセクション（`IssueList`の`pinnedSection`）も、その枠の中に置く**（#2175）——
  ...
  引いている最中に固定セクションと一覧の境目が割れて見える）。**#3165で固定セクションは
  `<ul>`の中へ移した**ので、いまはスクロール領域の一部でもある（理由は後述の
  「ユーザーの確認待ち」の節）。
...
- **そのダイアログは1画面で、項目はすべて最初から見えている**（#1884）。種別・リポジトリ・内容・
  ...
  **タイトル・ラベルは、空欄があれば送信時に自動で決まる**（`POST /api/issues/suggest`。
  専用の付与ボタンは#2773で廃止）。「作成」「作成+実装開始」を押した送信の直前に、タイトルが
  空ならタイトルとラベルを判定し、タイトルが手入力済みでもラベルが未選択ならタイトルを維持して
  ラベルだけを補う（`ensureTitleAndLabels`・#3295）。人がラベルを選んでいれば判定で上書きしない。
  判定だけをその場で試したい場合はラベル欄の横の「付け直す」を使う（タイトルが空でも押せる）。
  **タイトルも空のときに生成に失敗すると作成を中断する**（空欄のまま自分で書ける状態を保つ）。
  タイトルを手入力済みなら、ラベル判定に失敗してもラベルなしで作成できる。
  ...
  **操作ボタンは「キャンセル → 作成 → 作成+実装開始」の順にDOMへ置き、スマホでも横並びにする**
  （#3123。以前は「キャンセル → 作成+実装開始 → 作成」で、フッター
  〈`DialogFooter`・`WindowFooter`〉の`flex-col-reverse`によりスマホでは縦積みの一番上が主ボタン、
  一番下がキャンセルだった〈#1884〉）。縦に3つ積むとフッターだけで約170px取られて1画面に
  収まらないため、`flex-row justify-end`を渡して横に並べ、左から見た順をそのまま画面の並びにする。
  幅が足りる`sm`以上は右寄せで、「作成」「作成+実装開始」はスマホでだけ`flex-1`で幅を分け合う。
  **先頭に`sm:mr-auto`付きのghostボタンを足さない**（並びの意味が変わる）。
- **作成ダイアログは、見出し・本文・フッターを縦に積み、本文だけがスクロールする**（#3123）。
  スマホで1画面に収め、キーボードが出ていても「キャンセル・作成・作成+実装開始」を常に見せるため
  （実装開始ダイアログの#3119と同じ作り）。**見出しは本文へ入れない**——閉じる・別ウィンドウの
  ボタンは`DialogContent`基準の`absolute`で、本文の中に見出しを入れるとスクロールした項目が
  その下を通って重なる。**内容欄は高さ固定（`h-48`＝192px・`max-h-48`）で、文字が増えても
  伸びず欄の中でスクロールする。** `Textarea`の既定（`field-sizing-content max-h-64`）は
  `field-sizing-fixed`で打ち消し、上限は`max-h-none`ではなく**同じスケールの`max-h-48`**にする
  （tailwind-mergeは`max-h-none`を`max-h-64`と同じグループとして扱わず、両方が残って
  どちらが効くかがCSSの出力順に決まる）。スマホの文字は16pxのまま（16px未満にするとiOSが
  フォーカス時に画面を拡大する・#1442）で、行間だけ`leading-snug`で詰めて表示行数を稼ぐ。
- **ダイアログの中身が横幅を押し広げないよう、`DialogContent`は`grid-cols-[minmax(0,1fr)]`で
  ...
  - **`flex`で組むダイアログでは、この列の指定は効かない**（#2402）。`grid-cols-*`は
    tailwind-mergeの別グループなのでクラス文字列には残るが、`display`が`flex`になった時点で
    ただの残りクラスになる。**横幅を止めているのは`DialogContent`自身の
    `w-full max-w-[calc(100%-2rem)]`（と各ダイアログの`sm:max-w-*`）**で、幅が確定した
    フレックスコンテナは子の中身では広がらないため、`flex`で組んでも#1710の症状は戻らない。
    ただし**幅の指定を消すとどちらの組み方でも戻る**ので、`max-w-*`ごと外さない。
...
- **「未着手」の一覧の上に、着手順をClaudeに決めさせる入口は無い**（#3294で廃止した。かつては
  「順番を決めて開始」の帯とダイアログ・`POST /api/issues/order`・1位の自動開始があった）。
  何から手を付けるかは、一覧の並びと「まとめて予約」（#3284）で人が決める。**着手順の判定を
  Claudeへ戻すときは、母集団を絞り込み前の「未着手」にする**（左メニューの件数と揃える。#1750）。
...
- **「ユーザーの確認待ち」にはIssueだけでなく、ユーザーがマージするしかないPRも出す**（#1613。
  ...
  **PRを数に足す画面と、PRを一覧に出す画面は必ずセットにする**（#1713）。スマホは件数
  ...
  - **先頭に固定するものは、Issueの行と同じスクロール領域（`<ul>`）の中へ置く**（#3165）。
    `<ul>`の外に兄弟として置くと、flexアイテムの`min-height: auto`で枠は縮まない一方、
    `<ul>`は`min-h-0 flex-1`で高さ0まで潰れる。リリースPRを各リポジトリへ一斉に起票した日は
    マージ待ちが10件を超え、**確認待ちのIssueが1件も出ないうえ、枠自身にもスクロールが無いので
    はみ出したカードにも届かなかった**（スマホ・PCの両方で起きる）。**枠の中に別のスクロールを
    作る形では直らない**——引っ張って更新（`use-pull-to-refresh.ts`）は`<ul>`が先頭にいる間の
    下向きのタッチを`preventDefault`するため、入れ子のスクロールは指で動かせない。
    **`<ul>`はIssueが0件でも、固定するものがあれば描く**（`hasPinnedRows`）。
  - **枠が遅れて届いたぶんは、`scrollTop`へ足して打ち消す**（#3165。
    [`lib/issue-list-scroll.ts`](../src/lib/issue-list-scroll.ts)の`computeAboveRowsScrollTop`を
    `use-issue-list-scroll.ts`の`useLayoutEffect`から呼ぶ）。スクロール位置の復元（#773）は
    `scrollKey`ごとに1回しか走らないのに対し、マージ待ちPRはマウント後の取得で遅れて届く。
    枠を`<ul>`の中へ入れた結果、復元で位置を決めた後に枠が先頭へ挿入されると、行だけが枠の
    高さぶん下がって戻ってきた位置がずれるようになった。測るのは**先頭の行の`offsetTop`**
    （＝行より上にあるものの高さ）で、どのIssueが先頭に来ても同じ値になる。
    **ブラウザのスクロールアンカリングには任せない**——iOS Safariは`overflow-anchor`に
    未対応で、報告があったのもiPhoneの画面。
  - **並べるのは既定で先頭3件までで、残りは「他N件を表示」で開く**（#3165。`COLLAPSED_COUNT`）。
    スクロールできるようになっても、10件ぶんのカードを越えないとIssueに届かないのは変わらない。
    **開閉の状態は覚えない**（開いたままにすると、次に来たときまた枠で画面が埋まる）。
    **見出しには並べている数ではなく総件数のバッジを出す**ので、畳んでいても何件あるかは分かる。
    「保留中N件」を開いた中身（`snoozed`を渡した並び）は畳まない——開いたのは中を見るため。
...
- **スマホのフッターは「ホーム／ブランチ／リリース／AI使用量」の4枠で、タブのidは`mscreen`の
  値そのもの**（#1436・#1638・#2631・#2724・#2811）。
  **「リリース」（`mscreen=release-history`）は#2811で3枠目に足した。** リリース履歴は#2726では
  ホームのメニューからのドリルダウンだったが、AI使用量（#2631）と同じ理由——スマホで見たい
  ものが1段掘らないと開けない——でタブへ昇格させた。置き場所を「ブランチ」の隣にしてあるのは、
  マージ待ち→本番へ出た結果、というリリース周りの導線を隣り合わせにするため。
  **あわせてホームのメニューの行は外す**（AI使用量と同じで、同じ画面への入口を2か所に持たない）。
...
- **「ブランチ」画面（`pane=flow`・スマホは`mscreen=flow`＝フッターの4枠目。#1638）は、
  ...
  **どの束に入るかは、リリースPRの「マージ時刻」ではなく「凍結時刻」で決める**（#2489）。
  リリースPRのheadは固定ブランチ`release-main/vX.Y.Z`で、**PRを作った時点でその内容が
  止まっている**（#2117）。develop→mainのマージは人が行うため、PRが開いている窓は数十分〜数日
  あり、その間にdevelopへ入った作業まで「その版で本番反映」と出ていた（実例: PR #2485は
  17:58にdevelopへ入り、17:33に作られたv4.51.0のリリースPRが18:04にマージされたことで本番
  反映と表示されたが、`release-main/v4.51.0`にそのコミットは入っていない）。版の取り違えも
  同じ原因で起きる（PR #2471はv4.50.0のリリースPRが開いている間にdevelopへ入ったので、
  実際に出たのはv4.51.0）。判定は`lib/branch-flow.ts`の`releaseContentFrozenAt`に寄せ、
  **head が`release-main/v`で始まるときだけ作成時刻**、`develop`をheadにする#2117以前の形
  （共有ワークフローの参照タグが古いリポジトリ）は従来どおりマージ時刻にする——あちらはheadが
  ブランチの先端を追い続けるため、作成時刻で切ると「入っているのに未反映」と逆向きに誤る。
  **PR詳細のデプロイ表示（`lib/pull-request-deploy.ts`の`findCarrier`）も同じ関数を通す**
  ——#1814で「2つの画面で結論がずれない」ようにしてあるため、片方だけ直すとそこが割れる。
  厳密な凍結点はバンプPRのhead（＝バンプPRを作った時点のdevelop）で、リリースPRの作成は
  ...
  **`behindBy`（mainにあってdevelopに無いコミット数）は出さない。** develop→mainをマージコミットで
  ...
  と噛み合わないので採らなかった。
  **実行ボタンを押したがまだブランチが無いIssueは「着手中」として流れ図の上流に並べる**（#1704・#2386）。
  レーンはPRのheadブランチと実在する作業ブランチの和集合なので、押した直後のIssueは画面のどこにも
  現れなかった（作業ブランチがGitHubへ現れるのは最初のpushから）。対象は進捗が
  `planning`・`implementation`のopen Issueのうち、どのレーンにも現れていないもの（`lib/branch-flow.ts`の
  `STARTED_ISSUE_PROGRESS_STATUSES`・`collectStartedIssues`）。
  **`ready`（未着手）は着手中へ含めない**（#2386）。含めていたころ（「実装予定」）は畳んだ1行の件数が
  バックログ全体の大きさになり、見ても何も起こせなかった。数えたいのは「いま何本走っているか」なので、
  ...
  「進行中」（レーンの本数）と足して総数になるようにするため**——ブランチが上がったぶんはそちらが数える。
  **除外に使うのはレーンの対応Issueだけ**（`laneMainIssueNumbers`。#2386）。`orphanIssues`が使う
  `linkedIssueNumbers`には、PRのタイトル・本文の`#番号`をそのまま拾った「関連」まで入っており、
  そちらで除くと**走っているIssueが他のPRの本文で言及されているだけで件数から消える**
  ...
  **畳んだ1行の件数（未着手・着手中・進行中・未リリース）はアイコンと数字だけで出す**（#1886・`SummaryCount`）。
  同じ灰色の文字だったため、右端まで読まないとリリース待ちなのか着手中なのか分からなかった。
  破線の丸（未着手・薄い灰。#3163で着手中から移した）／再生マークの丸（着手中・灰）／枝分かれ（進行中・青）／
  上向き矢印（未リリース・紫）と**形でも区別が付く**ようにし、言葉は`title`と`aria-label`に持たせる。
  **破線の意味は「ブランチがまだ無い」**で、未着手も着手中もそれに当たる。流れ図の着手中の点は破線のまま。
  並びはIssueが進む順（手作業・未着手・着手中・進行中・未リリース）。
  **手が要るものではないので、`needsAttention`（ヘッダーの「手が要るもの◯件」）には加えない。**
  枝と点は破線で描き、実在するブランチのレーンと見分けが付くようにする。
  **`orphanIssues`（ブランチもPRも見つからないIssue）とは別物**で、あちらは「developへマージの段階なのに
  PRが見つからない」異常を隠さないための枠（`ORPHAN_ISSUE_PROGRESS_SET`）。**実装中はそこに含めない**
  （#2386）——押した直後は必ずブランチが無い状態を通るため、異常として警告すると起動したセッションの
  ぶんだけ琥珀の枠が出ていた。**平時はほぼ空になるのを承知で残している**（`Implementation` →
  `Develop PR`はpushとPR作成をトリガーに報告されるため、この進捗のIssueはほぼ必ずPRを持つ）。
  取得済みPRの範囲外・報告だけ進んだといった例外を検知する口として置いてあり、出たときは
  実際に見るべき状態になっている。手作業Issue（`71.manual-step`）は実装するものではないため着手中に混ぜない。
...
推定の経路は「新しいIssueを作成」ダイアログの「作成」「作成+実装開始」（タイトルまたはラベルが
空のときに送信前に自動で。#1884・#2773・#3295）と「付け直す」が呼ぶ`POST /api/issues/suggest`だけで、
...
- **初期表示は未確認のリリースだけで、確認済み・対象外は「確認済みも表示」ボタンで戻す**
  （#3170。`ReleaseHistoryPanel`の`showAll`、既定false）。絞り込みは`selectUncheckedReleases`を
  表示の直前に掛けるだけで、件数（「未確認 N件」）は絞り込み前の母集団から数えるため切り替えても
  動かない。**状態は永続化せず、画面を開くたびに未確認のみへ戻る。** 対象リポジトリを1つも
  選んでいないと初期表示は空になるため、案内文の横に同名の入口を置いてある
