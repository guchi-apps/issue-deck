...
- **Issue検索は打ち切る。** `org:guchi-apps knowledge-candidate`は585件ヒットするが、大半は本文に
  マーカーの文字列が出てくるだけのIssue。更新の新しい順に3ページ（300件）で止め、**打ち切ったことは
  `truncated`で返して画面にも出す**（未判定の件数は「+」付きの下限として表示する）。
  **そのため、更新が止まって久しい未判定メモはこの窓から外れる**——滞留の警告は「表示している
  範囲での最古」を根拠にしており、実際の最古とは限らない
- **取得は実測16秒**（GraphQLを最大4回）かかるので、`GET /api/knowledge`がユーザーごとに5分
  キャッシュする。更新ボタンは`?refresh=1`でそれを捨てさせる
