...
- **Issue一覧の進捗は6分割の横棒で出す**（#2516。`components/dashboard/workflow-status-steps.tsx`の
  ...
  - **実行中は現在の段のマスだけを光が掃く**（`.progress-seg-sweep`）。**塗り（進捗）は動かさない**
    ——バー全体を光らせると、どこまで進んだかを読み取る手がかりまで動いてしまう。掃くかどうかの
    判定は円グラフだった頃と同じ`isWorkflowBadgeSpinning`（#1439）
  - **確認待ち（amber）・回答待ち（blue）のアイコンはバーの左隣に出す。** 5px高のバーの中には
    収まらない。色はバーと同じ系統にして、同じことを言っていると分かるようにする
