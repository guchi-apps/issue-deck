...
- **一覧を下へ引っ張って更新する操作は[`use-pull-to-refresh.ts`](../src/hooks/use-pull-to-refresh.ts)に集約する**（#1893・#1947・#1958）。
  ...
  （#1947）・ブランチ画面（`BranchFlowView`。#1958）。**取り直す中身が画面ごとに違うのは
  「ユーザーの確認待ち」だけ**で、そこだけIssueに加えてマージ待ちPRも取り直す（#2175。後述）。**描画を一覧ごとに書かない**——文言・色・
...
- **「ブランチ」画面（`pane=flow`・スマホは`mscreen=flow`＝フッターの4枠目。#1638）は、
  ...
  **畳んだ行のピルは、紫（待てば進む）と琥珀（あなたの番）で書き分ける**（#2038）。リリースが
  ...
  PCヘッダーのリリース状況と同じ言い方にする。琥珀のピルは`AttentionPill`が持ち、「ユーザーの
  マージが必要」「手作業◯」「優先度 高」と同じ見た目にしてある（**この画面の琥珀は「あなたの番」**）。
  **ヘッダーの件数も「手が要るもの◯件」と「待てば進むもの◯件」に分ける**（#2038）。
  `needsAttention`はCI失敗・ユーザーのマージ待ち・リリースのマージ待ち・デプロイ失敗だけになり、
