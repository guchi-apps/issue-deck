...
describe("develop→mainのPRへ引き継ぐバージョンの判断根拠を取得する", () => {
  it("バンプPR本文から`## バージョンの判断根拠`の節（次の見出しまで）を取り出す", () => {
    ...
    // バンプPR向けの固定文言（「このPR上でバージョンを修正してください」）は、mainへのPRでは
    // 案内が食い違うため含まれない
    expect(reason).toBe(
      ["コード差分の内容からminorバージョンと判定しました。", "", "新機能としてダークモードを追加しました。"].join(
        "\n",
      ),
    );
    expect(reason).not.toContain("このPR上でバージョンを修正してください");
  });

  it("バンプPRが見つからない場合は取得できなかった旨の注記を残す", () => {
    const { stdout, reason } = runStep("");

    expect(reason).toBe(
      "バンプPRから判断根拠を取得できませんでした（バンプPR上でバージョンが変更された可能性があります）。",
    );
    expect(stdout).toContain("取得できませんでした");
  });

  it("`## バージョンの判断根拠`の節が無い場合も取得できなかった旨の注記を残す", () => {
