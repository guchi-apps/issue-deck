...
#
# #2896で実測し直したところ、**次の入力が入ったセッションはほぼ全件がその後のターンで
# tool_useへ復帰していた**（固定文面19件・人が打った文面19件で、後者には「進めて」系も含む）。
# **復旧させているのは入力が入ったこと自体で、文面固有の効果ではない**——固定文面でも2件は
# 同じ形で再発している。それでも本文を固定にするのは、gates.mdの線（実行体に本文を組み立て
# させない）を守るためで、効き目のためではない。**ただし「進めて」のような曖昧な文面は
# `ListAgents`で幻のバックグラウンド実行を探しにいく遠回りを実測で残す**（#2655・#2675）ので、
# 事実を明言する文面のままにする。
#
# 一方、人が押すまでの放置時間は中央値12分・最長5時間で、待たせている時間のほうが大きい。
# そこで、APIエラーの自動再開（#1971）と
...
# 送り直したあと、そのセッションが**実際にツールを呼んだ**か（＝本当に復旧したか。#2896）。
#
# **回数の記録を消してよいのはここが真のときだけ。** 「検知しなくなったら消す」にすると
# 上限が効かない——送出そのものが転記へ`queue-operation`レコードを書いて**mtimeを更新する**ため、
# 次の巡回では停滞時間が閾値未満になり必ず「検知せず」になる。そこで消すと回数が毎回0へ戻り、
# 同じ形で止まり続けるセッションへ延々と送り続けることになる（計画レビューの指摘1）。
#
# **判定に使うのは`.step`（ツールの実行ごとに進む時刻）だけで、`.event`は使わない。**
# `.event`は`Stop`でも進むが、**この現象のターンは`Stop`が正常に発火している**ので、それを
# 「動き出した」と読むと送った直後に必ず消えてしまう。画面の停滞パネル（#2886）が
# `stepSeenAt`・`activityAt`の追い越しで判定しているのと同じ考え方で、こちらは`Stop`が飛ぶぶん
# 条件をツールの実行だけに絞る。
#
# **読めなければ「復旧していない」を返す**（＝記録を残す）。取りこぼしても、次に同じ現象で
# 止まったときに1回早く人へ渡るだけで、送り続ける側には倒れない。
session_tool_call_stall_recovered() {
  local session="$1" state last attempts step seen
  state="$(session_tool_call_stall_read_state "$session")"
  read -r last attempts _ <<<"$state"
  # 一度も送っていなければ、消しても失うものが無い（記録そのものが無い）。
  ((attempts > 0)) || return 0
  step="$(session_state_read_step "$session" 2>/dev/null || true)"
  [[ "$step" =~ ^([0-9]+)[[:space:]]+[A-Za-z_]+[[:space:]]+([0-9]+)$ ]] || return 1
  seen="${BASH_REMATCH[2]}"
  ((seen > last))
}
