...
# **チャンク化する前に終了時刻の新しい順へ並べ替える**（#2870）。呼び出し側
# （`report_session_usage`）はチャンクを先頭から順にPOSTし、途中の1件が失敗したら
# 残りを送らずに諦める（次の巡でまたやり直す設計）。転記を開く順（古い順・
# `session_usage_transcripts`）のままだと、埋め戻しのように送るチャンクが複数に
# 分かれる状況で、直近のセッションほど後ろのチャンクに回り、途中で詰まるたびに
# 一度も送られないまま取り残される。**この並べ替えは送信順だけの話で、集計側
# （`session_usage_aggregate`）が転記を読む順（重複除去の基準）はここでは変えない。**
#
# **#2870で実際に埋め戻しが完走しなかった主因は、この送信順ではなく
# `api_call`側で本文が長すぎて`curl`が起動できなかったこと（`E2BIG`）だった**
# （そちらは`scripts/subpc-dispatch-poller.sh`の`api_call`で直した）。この並べ替えは
# それとは独立に効く保険で、将来また何らかの理由でチャンクの途中が失敗しても、
# 新しいセッションだけは先に届くようにする。
sessions.sort(key=lambda s: s["endedAt"], reverse=True)
