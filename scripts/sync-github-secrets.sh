...
#
# **出力の書式は変えないこと。** 次の4つは reusable-sync-secrets.yml がパースして
# issue-deck の画面へ返す（#1309・#2022）。
#   - `ok     <KEY> -> ...`   同期できた項目（画面の「内訳」に出る）
#   - `skip   <KEY>（理由）`  スキップした項目（同上）
#   - `FAIL   <KEY>...`       失敗した項目
#   - 末尾の `同期=N スキップ=M 失敗=K`
# **このスクリプトは共有ワークフローのタグでは配られず、リポジトリごとに実体を持つ。**
# 書式を変えると、変えたリポジトリだけ内訳が空で返る（件数は正しいまま）。
