...
#   （`MANUAL_STEP`）が持つ「本文に書かれたコマンドしか実行しない（照合2回・`body_changed`・
#   5分の失効）」の歯止めは持たない。
#
#   **本文に書かれた手順は、画面の「セッションを起動」を押した1回で最後まで流す**（#2830）。
#   止まるのは4つ——代行できない手順・実行の失敗・**本文に無いコマンドが要るとき**・クローズの
#   可否。3つ目だけがこのセッションの歯止めで、実行体が組み立てた文字列は読み取り専用でも
#   全文を`AskUserQuestion`で示してから実行する。**担保はプロンプト
#   （scripts/prompts/manual-step-agent.md）だけで、機械的には縛っていない**
#   （docs/multi-agent/gates.md の例外5）。
#
# 環境変数:
