...
# 載せ替えでも変わらない・#1791）。必要なセッションだけ中で起動する。
#
...
if [[ "$DEV_SERVER_ENABLED" == "0" ]]; then
  ...
  #
  # **リダイレクトは`>`ではなく`>>`（O_APPEND）で開く（#1679）。** このログには開発サーバー自身の
  # 出力と、停止理由を書く`dev_server_log_event`（`>>`で開く）の行が同居する。`>`で開いたfdは
  # 自分のオフセットを持つため、回収側が末尾へ理由を追記した直後に、死にゆく開発サーバーが最後の
  # 出力（`ELIFECYCLE`など）を**自分のオフセット位置＝理由の行より手前**へ書き、理由の行を
  # 上から潰す。実際に`.dev-servers/issue-1523.log`でマルチバイト文字の途中から壊れた行が残った。
  # 両者をO_APPENDに揃えると、書き込みのたびに末尾へ位置づけられるのでこの競合が起きない。
  #
  # **起動時にログを切り詰めない。** 切り詰めると、この直前に書かれた「セッションの再開に伴い、
  # 前回の開発サーバーを停止します」（上部の再開処理・`dev_server_stop_by_port`）が消える。
  # 消えて困るのがまさにこの理由の行なので、伸びるのを受け入れて残す側に倒す。
...
#
# 質問セッションは「調べる過程で見つけた別件を`70.needs-decision`付きで起票してよい」運用（#1528）だが、
...
# （入れない理由の一覧は`lib/agent-allowed-tools.sh`）。暴発の歯止め（`70.needs-decision`を付ける・
...
#
#   - 添付画像の保存先（ディレクトリ・#3039）——**Issueに画像が貼られていれば読む。**
#     `fetch-issue-images.sh`（#2967）の保存先で、画像を開くたびに承認待ちになっていた。
#     ファイル名が起動時に決まらないので、ここだけディレクトリ単位（末尾`/`）で渡す。
#     **保存先を`ISSUE_DECK_IMAGE_DIR`としてセッションへ明示する。** スクリプトの既定値は
#     `TMPDIR`で変わるため、規則とスクリプトが別の場所を指して規則が外れるのを防ぐ
#
...
SESSION_EXTRA_READ_FILES=("$PROMPT_FILE" "${ISSUE_DECK_IMAGE_DIR%/}/")
...
# セッションの状態をissue-deckへ報告するフック（#1219）、提示した計画をIssueへ残すフック（#1342）、
# 承認に答えて作業へ戻ったことをissue-deckへ伝えるフック（#1357）。
#
# **`--settings` で渡すことで、このスクリプトから起動したセッションにだけ適用する。**
# `~/.claude/settings.json` に書くとメインPCの対話セッションでも報告が飛んで邪魔になる。
...
# **丸投げを禁じる1文を必ず足す**（#2896）。ここが「ツールを呼んだつもりで呼んでいないまま
# 停滞する」現象の予防にあたる。
#
# 直近45日ぶんの転記を機械的に走査したところ、この形の停滞31件は**全件が`claude-sonnet-5`**・
# **全件が`Agent`ツール**で、27件がこのキックオフ文面への最初の応答だった。`subagent_type`は
# 26件が`fork`——つまり「セッションの作業まるごとをforkへ委任しようとして、その呼び出しが
# tool_useにならずテキストで出る」形に揃っている（`claude-opus-5`は1,089セッションで0件で、
# そもそも`fork`を使っていない）。**丸投げする動機そのものを消すのが、いちばん確実な予防。**
#
# **文面に入れる場所はここしかない。** 誤出力は最初のターン——プロンプトファイルを読む前——に
# 起きるため、`scripts/prompts/*.md`側に書いても間に合わない。
#
# **`Explore`・`Plan`は禁じない。** 調査を委ねるぶんには正常に呼び出せており（実測でsonnet 85件・
# opus 71件）、`--disallowedTools Agent`で機械的に塞ぐとそちらまで巻き添えになる。
# 止めたいのは「このセッションが担当する作業そのもの」を別のエージェントへ渡す形だけ。
#
# 再開（`--continue`）の文面にも同じ1文を足す。**再開したセッションも最初のターンは同じ形**で、
# 丸投げされて困ることも変わらない。
#
# **独立した段落として置く。** 上の1文へ続けて書くと「着手のしかた」の但し書きに埋もれる。
KICKOFF_PROMPT+=$'\n\n'"このIssueの作業そのもの（指示ファイルを読む・計画する・実装する）を Agent ツールのサブエージェントや fork へ委任せず、このセッションで行ってください（調査を Explore へ委ねるのは構いません）。"
