...
handle_decision() {
  ...
  case "$status" in
    ...
    REVISION_REQUESTED)
      ...
      # Codexは終了コードが0以外のコマンドを失敗として扱うことがある。修正本文を標準エラー
      # だけへ出すと、終了理由だけが次のターンへ渡り、画面から送った修正が計画へ反映されない。
      # 終了コード2は維持しつつ、次の計画を作るための情報は通常のコマンド出力にも載せる。
