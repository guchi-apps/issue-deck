  ...
  while IFS= read -r line; do
    ...
    if ! api_call POST /api/dispatch/session-usage "$line"; then
      case "$API_RESPONSE_STATUS" in
        ...
        # 持たない旧デプロイへの後方互換）。
        404) ;;
        # **000（タイムアウト・接続不可）だけは黙らない**（#2870）。以前はここも404と
        # 一緒に黙って見送っていたが、大量データ（埋め戻し）を複数チャンクに分けて送る
        # うち1つがサーバー側の処理時間で`--max-time 30`に達すると、ここで静かに諦めて
        # 残りのチャンク（＝より新しいセッション）が送られないまま次の巡でも同じ場所で
        # 詰まり続けていた。原因追跡のためログにだけは残す（`report_api_failure`は
        # `report_api_failure`のcase内で000も文言を持っている）。
