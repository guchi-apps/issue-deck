...
# **#3169でCodexの行にもフェーズの内訳が入るようになった。** Claude側の印はどれも既に
# 置かれているため、Codexの過去ぶんを埋め直すには別の印が要る（置かないと、入れ替えた時点で
# 3日以上前に終わったCodexのセッションは30日ぶん「実装（フェーズ未集計）」のまま残る）。
SESSION_USAGE_CODEX_PHASE_BACKFILL_STAMP="${XDG_STATE_HOME:-$HOME/.local/state}/issue-deck/session-usage-codex-phase-backfill.stamp"
...
report_session_usage() {
  ...
  if [[ -f "$SESSION_USAGE_BACKFILL_STAMP" && -f "$SESSION_USAGE_CODEX_BACKFILL_STAMP" &&
    ...
    -f "$SESSION_USAGE_VERIFY_BACKFILL_STAMP" && -f "$SESSION_USAGE_CODEX_PHASE_BACKFILL_STAMP" ]]; then
  ...
  if ((backfill)); then
    ...
    # Codexの集計が落ちた巡では置かない（Claudeの印と違い、埋め直す相手がCodexの行だけのため）。
    ((codex_backfill_ok)) && touch "$SESSION_USAGE_CODEX_PHASE_BACKFILL_STAMP" 2>/dev/null || true
...
# --- リリース完了の巡回通知 -----------------------------------------------------
# 新しく出たリリース（本番反映）を、issue-deckに巡回して見つけさせ、Push通知させる（#2725）。
#
# **これまでリリース通知はSignalyへのwebhookにしか流れていなかった**
# （`.github/scripts/signaly-notify.sh`を`deploy.yml`の`notify-release`が呼ぶ）。
# issue-deckのPWAには届かず、フリートのリリースだけ別のアプリで受け取る形になっていた。
#
# 上の巡回と同じ形で、**pollerがやるのは「呼ぶ」ことだけ**。実際に巡回するかどうかも、
# 鳴らすかどうか（記録済みのタグと違うか）もissue-deck側が決める。失敗しても1巡を止めない。
sweep_releases() {
  if ! api_call POST /api/repositories/release-push-sweep '{}'; then
    case "$API_RESPONSE_STATUS" in
      # 404と接続不可を黙って見送る理由はコンフリクトの巡回検知と同じ。
      404|000) return 0 ;;
      *) report_api_failure "リリース完了の巡回通知に失敗しました" ;;
    esac
    return 0
  fi
  ...
  # 鳴らしたときだけ出す（毎巡「異常なし」を積まない）。
  printf '%s' "$API_RESPONSE_BODY" |
    jq -r '.notified[] | "リリースを通知しました: \(.repositoryFullName) \(.tagName)"' 2>/dev/null ||
    true
  return 0
}
...
run_job() {
  ...
  case "$codex_model" in
    auto | gpt-6-astra | gpt-6-sol | gpt-5.6-terra | gpt-6-luna | gpt-5.5 | gpt-5.4) ;;
    *)
...
run_once() {
  ...
  if [[ "$DRY_RUN" -eq 0 ]]; then
    ...
    # 外向きの副作用があるため）。
    sweep_release_merges
    # リリース完了の巡回通知（#2725）。**dry-runでは呼ばない**（Push通知という
    # 外向きの副作用があるため）。
    sweep_releases
