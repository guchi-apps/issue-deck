...
if "25.artifact-required" in label_names:
    artifact_instructions = (
        ...
        "- **画面デザインは原則PC・スマホの2画面を並べて提示してください**（#1632）。1つの"
        "アーティファクトの中に、PC（デスクトップ幅）と**スマホ（iPhone 15 = 幅393px × 高さ852px）**"
        "の両方の見た目を並べます。どちらか一方だけにする場合は、その理由をアーティファクト本文に"
        "書いてください\n"
        ...
        "承認済みの記録として残ります\n"
        "- **計画のやり取りで見た目の直しを求められても、Plan modeの中では差し替えないでください**"
        "（#1745・#2110）。Plan modeで書けるのは計画ファイルだけで、アーティファクトのHTMLは"
        "書けません。順序は次の2つです。(1) 直す内容を計画本文へ書き、"
        "「公開済みの版は修正前の見た目です」と明記したうえで承認を取る (2) 承認の直後、コードを"
        "書く前に**最初と同じファイルパス**で`Artifact`を呼び直して再公開する。パスが同じなら同じ"
        "URLへ再デプロイされるため、計画コメントに残ったリンクはそのまま新しい見た目を指します。"
        "**見た目の直しだけを再承認させるゲートは足しません**（承認の往復が増えるわりに、直す内容は"
        "計画に書いてあるので判断材料は増えません）\n"
    ...
    artifact_instructions = (
        ...
        "**PC（デスクトップ幅）とスマホ（iPhone 15 = 幅393px × 高さ852px）の2画面を1つの"
