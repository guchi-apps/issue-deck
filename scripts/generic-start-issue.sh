...
#   ISSUE_DECK_CLAUDE_SUBAGENT_MODEL    サブエージェントのモデル（既定は sonnet。inherit で親を引き継ぐ。#3118）
...
if "25.artifact-required" in label_names:
    artifact_instructions = (
        ...
        "アーティファクト本文に書いてください\n"
        "- **アーティファクトそのものも、スマホで開いて読める作りにしてください**（#2974）。承認する人は外出先のスマホ"
        "で開くことが多く、枠を実寸のまま横スクロールに入れると、PCの枠もスマホの枠も横へ動かさないと見えません。(1) 各画面"
        "の枠は実寸（PCはデスクトップ幅・iPad 1180px・スマホ393px）で組み、`overflow-x:auto`の横"
        "スクロールに頼らず、閲覧幅に合わせて`zoom`で縮小します。枠に`class=\"fit\" data-w=\"1440\"`"
        "（数値は枠の幅）を付けて親を幅100%にし、ページ末尾に`<script>const fit=()=>document."
        "querySelectorAll('.fit[data-w]').forEach(f=>(f.style.width=f"
        ".dataset.w+'px',f.style.zoom=Math.min(1,f.parentElement.clie"
        "ntWidth/f.dataset.w)));addEventListener('resize',fit);fit();"
        "</script>`を置けば足ります (2) 見出し・説明・注記は画面幅で折り返し、2列以上の比較や注記の並びは幅600"
        "px以下で1列に積みます (3) 幅393pxで開いたときに、ページ全体が横にはみ出さず、スマホの枠が画面の幅に収まるよ"
        "うに仕上げます\n"
...
if skip_shared_context == "1":
    shared_context_instructions = (
        ...
        "- 参照文書「実装中に得た知見の記録」にある「共有知識リポジトリを直接編集してはいけません」は、"
        "**実装対象がこのリポジトリ自身である今回は、worktree内の編集に限って当てはまりません**。"
        "共通の知見はこのPull Requestに同梱して構いません（ただしIssueの要求から外れる変更は入れないこと）"
    )
else:
    shared_context_instructions = (
...
# PR直前の計画レビュー待ちを丸ごと載せない（#3023。issue-deck自身のひな形は#3021）。ひな形側で
...
# 同じ処理が scripts/start-issue.sh と src/lib/prompts/build-implementation-prompt.ts にもある。
