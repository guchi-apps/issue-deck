...
build_env_prefix() {
  ...
  # **`--continue`では引き継がない**（#1648）。質問セッションの cwd は**質問Issueごとではなく
  # リポジトリごと**に固定されている（#1529）ため、`--continue`が拾う「そのcwdで最後に動いた会話」
  # は同じ質問のものとは限らず、「別の質問の続き」として始まる事故が起きる。
  # 代わりに run-issue-session.sh が、種別（上のISSUE_DECK_SESSION_KIND=question）を見て、
  # `SessionStart`フックが質問Issueごと（tmuxセッション名）に控えたsessionIdだけを
  # `--resume`へ渡す（#3033）。控えが無ければ新しい会話で始まる。
  # ISSUE_DECK_CLAUDE_RESUME=0 を渡せば、控えがあっても新しい会話にできる（この起動では渡さない）。
