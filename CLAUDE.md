...
- **tmuxセッションへの文字列・確定キーの送出（`send-keys`）。** 選択フォームの表示中に本文＋Enterを送り、1問目が既定の選択肢で勝手に回答済みになった事故があるため、判断して入力を送る実行体を作らない。追加指示の投入が必要になった場合は#1012で3段階プロトコル（状態確認→本文のみ送出→入力欄への反映を再確認→確定キーを別送）として別途設計する。**例外は画面の「停止」（#1332）**で、人が押した1回の操作として固定の`C-c`だけを送る（答えを選ばせも埋めもしないため同じ事故は起こせない）
