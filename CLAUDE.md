...
- **実行体が判断して組み立てた文字列・確定キーのtmuxセッションへの送出（`send-keys`）。** 選択フォームの表示中に本文＋Enterを送り、1問目が既定の選択肢で勝手に回答済みになった事故があるため、状況を読んで返事を組み立てる実行体を作らない。**例外は4つで、いずれも本文を実行体が決めない。** 最初の3つは「人が押した1回の操作」で、本文を決めるのは人か、画面が用意した固定文面。**固定文面を1クリックで送るものは、新しい例外を立てず3つ目の中へ足す**（担保が同じものを別枠にすると、どこまで許されているのかが読み取りにくくなるだけ）。
  - **画面の「停止」（#1332）**: 固定の`C-c`だけを送る（答えを選ばせも埋めもしないため同じ事故は起こせない）
  - **画面の「追加指示」（#1012）**: 人が書いた1行を3段階プロトコル（状態確認→本文のみ送出→入力欄への反映を再確認→確定キーを別送）で送る。承認プロンプト・選択フォームの表示中は送らず、反映を確認できなければ**Enterを送らずに終える**。CI失敗などを見て自動で送る経路は作らない
  - **画面が用意した固定文面を1クリックで送る（#2886・#2919）**: 共通するのは、**本文の正がコードにあり、押す前に全文が画面に出て、送り方は「追加指示」と同じ3段階プロトコル（`INSTRUCTION`ジョブ）**であること。**線引きは「固定文面の送信は可、選択肢の確定は不可」**——承認プロンプト・選択フォームの表示中はここでも送らない。受け口は`POST /api/dispatch`と分けてあり、**本文がその固定文面であること**と**今が送ってよい状態であること**をサーバー側で確かめる（任意の本文を送れるうえに`00.check-user`まで外れる操作にすると、追加指示と分けた意味が消えるため）。どちらも`00.check-user`を外すのは**`succeeded`の報告が届いてから**（積んだ時点で外すと、見送られたときに札だけ消える）
    - **「セッションが停滞しています」からの復旧（#2886）**: 引き上げ（`supervisor:session-*`）が出ているセッションへ、**原因ごとに決まった固定文面**を送る。文面の正は`src/lib/dispatch/session-stall.ts`。受け口（`POST /api/dispatch/session-recovery`）は、セッションが**今も停滞していること**を確かめる
    - **実行状況パネルの「再開」から、一括停止で止まったセッションを動かす（#3045）**: 「エージェントを一括操作」の「再開」を押すと、新規実行のブロックを解除したうえで、**一括停止（C-c）で止まっているセッションへ固定の1行だけ**を送る（`src/lib/dispatch/agent-resume.ts`）。受け口（`POST /api/dispatch/agent-resume`）は本文を受け取らず、対象をサーバー側の記録（成功済みの`INTERRUPT`ジョブ＋動き出していないこと＋中断の時点で作業中だったこと。質問・承認待ちで止まっていたものは外す）から選び直す。押す前に固定の1行の全文をダイアログへ出す。**`00.check-user`は外さない**。トグルだけをONにする操作はブロック解除のみで、セッションへは何も送らない
    - **マージ待ちの「修正を依頼する」・PR詳細の「修正Issueを起案」からローカルセッションへ知らせる（#2919・#3009）**: 人が書いた修正依頼はIssueコメントとして投稿し、セッションへ流すのは「そのコメントを読め」という**固定の1行だけ**（`src/lib/dispatch/pr-fix-request.ts`）。受け口（`POST /api/dispatch/pr-fix-notify`）は、セッションが**今も生きていること**を確かめる。**終了しているセッションへは送らず**、起動ジョブで呼び戻す（`send-keys`を使わない#1830の経路）。**押せる入口は2つ**——Issue詳細のマージ承認欄（マージ待ちに限る）と、PR詳細の「修正Issueを起案」帯（対象PRが未マージ〈`open`かつ`!merged`〉で元Issueが1件に絞れる場合。#3009）。どちらも送る本文・受け口・`00.check-user`の外し方は同じ1つの経路を共用する
  - **中断・停滞したセッションの自動再開（#1971・#2896）**: 人の操作を挟まない唯一の経路。対象は2つで、どちらも**固定の1行だけ**を送り、状況を読んで本文を変えない。上限回数を使い切ったら送るのをやめ、issue-deckへ引き上げて人へ渡す（Issueコメント＋`00.check-user`＋`01.check-blocked`が付き、Push通知が鳴る）
    - **APIエラーでの中断（#1971）**: Claude CodeがAPIの一時エラー（529等）を再試行しきるとturnが打ち切られ、**`Stop`フックが飛ばないまま**止まる（通知も出ず、回収も「追加指示」も効かない）。本文は`scripts/lib/session-resume.sh`が持つ。送ってよいのは**転記の末尾がAPIエラーで、一定時間更新が止まっている**と確かめられたセッションに限り、送り方は「追加指示」と同じ3段階プロトコル。上限は既定3回
    - **ツール呼び出しの空振り（#2655・#2896）**: ツールを呼んだつもりでtool_useにならずテキストを出しただけでターンを終えた形。本文は`scripts/lib/session-tool-call-stall.sh`が持ち、**画面の停滞パネルが送るものと同じ文面**。送ってよいのは転記の末尾がツール呼び出し風のテキストだと確かめられたセッションだけで、送り方も同じ3段階プロトコル。上限は既定2回。#2655では自動送信を開けていなかったが、実測では次の入力が入ったセッションはほぼ全件がその後のターンでtool_useへ復帰しており（放置時間は中央値12分・最長5時間だった）、**復旧させているのは入力が入ったこと自体で、文面固有の効果ではない**（本文を固定にするのは、この例外の条件を満たすため）。**回数の記録を消してよいのは送ったあと実際にツールが呼ばれたときだけ**で、「検知しなくなったら消す」にすると送出自体が転記のmtimeを更新するため上限が一度も効かない
...
「実装を開始」ダイアログで実行先に「次の5時間枠」を選ぶと、いまは起動せず**予定として積む**。積んだものは窓が開いた時点でサブPCへ順に起動され、いつもの経路（PR→自動レビュー→developへ自動マージ）で「本番反映待ち」まで進む。結果は左メニュー「予約実行」で5つ（本番反映待ち／確認が必要／実行中／止まった／見送り）に分けて見る。契機はpollerの`POST /api/dispatch/claim`への相乗りで、**窓の判定はサーバー側の純関数**（`src/lib/next-window-run.ts`）が持つ（設計は[docs/multi-agent/subpc-dispatch.md](docs/multi-agent/subpc-dispatch.md)「次枠実行」。かつては同じ画面に「今夜の夜間実行」〈時計で決まる窓〉もあったが#3019で削除した）。

- **次枠実行**はClaudeのプラン枠で決まる窓（既定OFF）。**いまの5時間枠の残りが既定60分を切ってから、既定10分おきに1件ずつ**起動する。5時間枠は時計の境界ではなく**最初のリクエストで始まる**ので、終わり際に起こすと枠の使い残しを消費しつつ、セッションが枠をまたいで走り、**次の枠のカウントがそこから始まる**（使われないまま消える枠が減る）。枠が動いていないときは待たずに起動する。**5時間枠・週間枠の残りが設定した下限（既定は制限なし）を下回っている間は起動を見送る**（#3100。予定は残り、上回れば再開する。判定は同じ純関数）
- **枠の状況を取れるのはAPIのレスポンスヘッダだけ**（`src/lib/claude/usage.ts`）。Claude Codeの転記JSONLには使用率もリセット時刻も入っていない。取得は最小の推論リクエスト1本で、**送信そのものが5時間枠を開始する**ため、**次枠実行としては、予定が1件も無いとき・OFFのときは呼ばない**。例外は下の「5時間枠を開けておく」で、ONかつ時間帯の中で枠が止まっているときだけ**意図して1本送る**（画面のメーターはその設定の都合だけでは送信せず、最後に見た値を読む）
- **承認・回答を代わりに押す経路は作らない。** 「計画を立案」のIssueは計画の投稿で止まり、人が承認する（上の「監視・計画レビューを行う実行体の禁止事項」と同じ線）
- **人が居ないと進まないオプション（`23.preview-required`・`25.artifact-required`）が付いたIssueは積めない。** 判定は積む時点と起動する時点の両方で、Issueの実ラベルを見る（起動が深夜になるか昼になるかを積む時点では決められないため）
- **確認待ちPushは止めない**（昼に起動することがあり、伏せる理由が無い）
- **5時間枠を開けておく**（#3032・既定OFF）: 同じ画面の設定で、指定した時間帯（既定7:00〜23:00）のあいだ枠が止まっていれば、pollerの巡回ついでに最小の探りを1本送って枠を開ける。作業前に枠を始めておくとリセットが早く来る（週間枠の総量は増えない）。**枠が動いている間は送らない**（最後に見たリセット時刻を過ぎるまで待つ）。判定は`src/lib/claude-window-keepalive.ts`
...
- **`（サブPC）`と`（VPS）`の手順は代行実行できる**（#2901）。VPSの手順はサブPCからTailscale SSHで実行するので、`.env`の追記・PM2の再起動・DBの作成のような実機の操作は「承認して実行」で流れる。**ただし`sudo`を含むVPSの手順は代行できない**（NOPASSWDの設定が無く必ず失敗する）ので、可能なら`sudo`の要らない形で書く。カレントディレクトリはVPS側のホームになるため、手順の中で`cd`する。**代行できるようになったのはリポジトリ管理外の実機操作だけで**、`/etc/apache2/`・`/etc/systemd/`などの設定ファイルは従来どおり後述の管理リポジトリのIssueへ切り出す
...
Issue／PRの状態モデル、担当ごとの責任は[docs/issue-pr-responsibility-model.md](docs/issue-pr-responsibility-model.md)を参照する（#3334）。要点は次の2つ。

- **Issueは「何を・なぜ作るか」の契約、PRは「どのように実現したか」の証拠。** PRがマージされたことと
  Issueの完了は同義にしない（完了は「Issueの進捗の状態遷移」節のとおり`Done`＝mainへの反映で判定する）
- 1 Issueに複数PRがある場合、それぞれのPRが「このIssueを閉じるPR」か「途中PR」かをPR本文へ明示する
  （下記PR本文テンプレート参照）。このリポジトリはGitHubの`closes`/`fixes`キーワードを使わないため、
  この明示が「Refs／Closesの使い分け」に相当する

### PR本文テンプレート

`develop`宛のPRには以下を記載する。「実装上の判断と採用理由」「画面変更・動作証跡」「デプロイ・移行・ロールバック上の注意」は該当する場合のみ書けばよい。

- 対応Issue（`closes #番号`/`fixes #番号`は使わず`#番号`のみ記載する。developマージ時点ではissueをcloseしない運用のため）
- Issueを閉じるPRか、途中PRか（このIssueに対応する最後のPRなら`<!-- issue-deck-pr-role:closing -->`、
  後続PRが必要ならまだ`<!-- issue-deck-pr-role:interim -->`をこの項目の末尾へ書く。Issue詳細の対応PR
  一覧に「最終PR」「途中PR」のバッジとして表示される）
- 実装内容
- 実装上の判断と採用理由（複数の実装方針を検討した場合・Issue本文の指示から外れた判断をした場合）
- テスト内容
- 画面変更・動作証跡（画面に関わる変更がある場合。URLと確認手順は「確認方法」、見た目の変化はここに書く）
- 影響範囲
- 未対応事項（Issueの完了条件のうち、このPRでは満たしていないものがあれば書く。無ければ「無し」）
- デプロイ・移行・ロールバック上の注意
- 確認方法
- 注意点
