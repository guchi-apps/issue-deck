...
- **実行体が判断して組み立てた文字列・確定キーのtmuxセッションへの送出（`send-keys`）。** 選択フォームの表示中に本文＋Enterを送り、1問目が既定の選択肢で勝手に回答済みになった事故があるため、状況を読んで返事を組み立てる実行体を作らない。**例外は4つで、いずれも本文を実行体が決めない。** 最初の3つは「人が押した1回の操作」で、本文を決めるのは人か、画面が原因ごとに用意した固定文面。
  ...
  - **画面の「セッションが停滞しています」からの復旧（#2886）**: 引き上げ（`supervisor:session-*`）が出ているセッションへ、**原因ごとに決まった固定文面**を1クリックで送る。文面の正は`src/lib/dispatch/session-stall.ts`で、状況を読んで組み立てる余地は無く、押す前に全文が画面に出る。送り方は「追加指示を送る」と同じ3段階プロトコル（`INSTRUCTION`ジョブ）で、**線引きは「固定文面の送信は可、選択肢の確定は不可」**——承認プロンプト・選択フォームの表示中はここでも送らない。受け口（`POST /api/dispatch/session-recovery`）が、本文が固定文面のどれかであることと、セッションが今も停滞していることをサーバー側で確かめる。`00.check-user`を外すのは**`succeeded`の報告が届いてから**（積んだ時点で外すと、見送られたときに札だけ消える）
