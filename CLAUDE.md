...
### 予約実行（夜間実行 #2772・次枠実行 #2995）

「実装を開始」ダイアログで実行先に「今夜の夜間実行」「次の5時間枠」を選ぶと、いまは起動せず**予定として積む**。積んだものは窓が開いた時点でサブPCへ順に起動され、いつもの経路（PR→自動レビュー→developへ自動マージ）で「本番反映待ち」まで進む。結果は左メニュー「予約実行」で5つ（本番反映待ち／確認が必要／実行中／止まった／見送り）に分けて見る。契機はどちらもpollerの`POST /api/dispatch/claim`への相乗りで、**窓の判定はサーバー側の純関数**（`src/lib/nightly-run.ts`・`src/lib/next-window-run.ts`）が持つ（設計は[docs/multi-agent/subpc-dispatch.md](docs/multi-agent/subpc-dispatch.md)「夜間実行」「次枠実行」）。

- **夜間実行**は時計で決まる窓（既定01:00・日本時間から3時間、既定OFF）。**窓を過ぎた予定は翌夜へ持ち越さず「見送り」にする**
...
- **枠の状況を取れるのはAPIのレスポンスヘッダだけ**（`src/lib/claude/usage.ts`）。Claude Codeの転記JSONLには使用率もリセット時刻も入っていない。取得は最小の推論リクエスト1本で、**送信そのものが5時間枠を開始する**ため、**予定が1件も無いとき・次枠実行がOFFのときは呼ばない**
- **承認・回答を代わりに押す経路は作らない。** 「計画が必要」のIssueは計画の投稿で止まり、人が承認する（上の「監視・計画レビューを行う実行体の禁止事項」と同じ線）
- **人が居ないと進まないオプション（`23.preview-required`・`25.artifact-required`）が付いたIssueは積めない。** 判定は積む時点と起動する時点の両方で、Issueの実ラベルを見る。次枠実行も同じ（起動が深夜になるか昼になるかを積む時点では決められないため）
- **夜間実行で起動したIssueの確認待ちPushは翌朝7:00まで送らない**（`decideCheckUserPush`の`holdUntil`）。**次枠実行では止めない**（昼に起動することがあり、伏せる理由が無い）
