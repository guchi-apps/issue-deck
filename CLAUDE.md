...
- **実行体が判断して組み立てた文字列・確定キーのtmuxセッションへの送出（`send-keys`）。** 選択フォームの表示中に本文＋Enterを送り、1問目が既定の選択肢で勝手に回答済みになった事故があるため、状況を読んで返事を組み立てる実行体を作らない。**例外は4つで、いずれも本文を実行体が決めない。** 最初の3つは「人が押した1回の操作」で、本文を決めるのは人か、画面が原因ごとに用意した固定文面。
  ...
  - **中断・停滞したセッションの自動再開（#1971・#2896）**: 人の操作を挟まない唯一の経路。対象は2つで、どちらも**固定の1行だけ**を送り、状況を読んで本文を変えない。上限回数を使い切ったら送るのをやめ、issue-deckへ引き上げて人へ渡す（Issueコメント＋`00.check-user`＋`01.check-blocked`が付き、Push通知が鳴る）
    ...
    - **ツール呼び出しの空振り（#2655・#2896）**: ツールを呼んだつもりでtool_useにならずテキストを出しただけでターンを終えた形。本文は`scripts/lib/session-tool-call-stall.sh`が持ち、**画面の停滞パネルが送るものと同じ文面**。送ってよいのは転記の末尾がツール呼び出し風のテキストだと確かめられたセッションだけで、送り方も同じ3段階プロトコル。上限は既定2回。#2655では自動送信を開けていなかったが、根拠だった失敗は「進めて」という曖昧な文面のもので、現行の固定文面は実測29セッション中27件で復旧している（放置時間は中央値12分・最長5時間だった）
